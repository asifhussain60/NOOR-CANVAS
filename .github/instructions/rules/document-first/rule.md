...
### Step 2a: Update KDS Documentation (if applicable) ✅

**Action:** Update affected KDS files if modifying governance, prompts, or rules

**When Required:**
- Modifying `.github/` governance files
- Creating/updating prompts
- Changing KDS rules or protocols

**What to Update:**
- Affected rulebook sections
- Related prompt files
- Handoff protocol schemas

---

### Step 2b: Add Test Reverse-Engineering Metadata (UI/API Code) ✅

**Action:** Auto-generate PLAYWRIGHT TEST METADATA block for testable code

**When Required:**
- Creating/modifying `.razor` components
- Creating/modifying `Controllers/*.cs` files
- Creating/modifying `Hubs/*.cs` files
- Any file with API calls, database operations, or SignalR
...
//   - POST /api/host/validate-token → canvas.Sessions lookup (canvas + KSESSIONS)
//
// Database Connections:
...
**Insertion Location:** Top of file (after usings/imports)

**Trigger Modes:**

1. **Automatic (during task execution):**
   - `task.prompt.md` Step 6.5 detects UI/API file creation
   - Auto-generates metadata from code analysis
...
3. **Validation (healthcheck):**
   ...
   ```
   - Scans codebase for missing metadata
   - Reports violations with suggested fix

**Benefits:**
- **Reverse-Engineering:** Enables automated Playwright test generation
- **API Discovery:** Quick reference for all endpoints a component uses
