...
$routesLastScan = (Get-Content .github/context/routes.json | ConvertFrom-Json).last_scan
$dbLastScan = (Get-Content .github/context/database.json | ConvertFrom-Json).last_scan
$uiLastScan = (Get-Content .github/context/ui-components.json | ConvertFrom-Json).last_scan
...
if ($scanAge -lt 5) {
    ...
    & .github/scripts/sensors/run-all-sensors.ps1 -Mode Incremental
}
```
...
**Output:** Updated context files in `.github/context/`
...
$keywords = & .github/scripts/sensors/activate-context.ps1 `
...
$context = & .github/scripts/sensors/activate-context.ps1 `
    -UserRequest "I want to add a save button to the canvas" `
...
1. **Routes Query**
   ...
   $routes = Get-Content .github/context/routes.json | ConvertFrom-Json
...
2. **Database Query**
   ...
   $db = Get-Content .github/context/database.json | ConvertFrom-Json
...
3. **UI Components Query**
   ...
   $ui = Get-Content .github/context/ui-components.json | ConvertFrom-Json
...
4. **Knowledge Graph Query**
   ...
   . .github/scripts/sensors/query-knowledge-graph.ps1
...
- Pattern File: `.github/knowledge/workflows/canvas-save-flow.md` (if exists)
...
```yaml
# .github/context/activation-config.json
{
  "confidence_threshold": 0.7,
  "max_nodes": 10,
...
status: "no_data"
context: null
message: "Knowledge graph not yet populated. Run: .github/scripts/sensors/run-all-sensors.ps1"
fallback_mode: true
```
