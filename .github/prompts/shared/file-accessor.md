...
**Implementation:** ✅ 100% local (.github/scripts/file-operations.ps1)
...
content = read_file(".github/governance/rules.md")  # Direct I/O
...
```yaml
prompts: .github/prompts/{subcategory}/{file}
governance: .github/governance/{file}
sessions: .github/sessions/{file}
knowledge: .github/knowledge/{subcategory}/{file}
tests: Tests/{subcategory}/{file}
scripts: .github/scripts/{subcategory}/{file}
```

### Examples
```python
# Instead of hardcoded paths
read_file(".github/prompts/internal/work-planner.md")
read_file(".github/governance/rules.md")
read_file(".github/sessions/current-session.json")
...
### Path Independence
- ✅ Relocate .github folder without changing agents
...
design = read_file(".github/KDS-DESIGN.md")
...
write_file(".github/sessions/current-session.json", session)
...
```markdown
# Old (Direct)
patterns = glob(".github/knowledge/test-patterns/*.md")
...
```json
{
  ...
  "resolved_path": ".github/prompts/internal/work-planner.md",
...
```json
{
  "file_accessor": {
    ...
    "auto_backup": true,
    "backup_location": ".github/.backups",  // LOCAL directory
    "max_backups_per_file": 5,
    "implementation": ".github/scripts/file-operations.ps1"  // LOCAL script
  }
}
```
...
- ✅ Backups stored in .github/.backups (local)
