...
async function loadKDSDesign(): Promise<KDSDesign> {
    const content = await readFile('.github/KDS-DESIGN.md');
...
async function loadKDSRules(): Promise<KDSRule[]> {
    const content = await readFile('.github/governance/rules.md');
...
async function loadSession(): Promise<SessionState | null> {
    try {
        const content = await readFile('.github/sessions/current-session.json');
...
async function saveSession(state: SessionState): Promise<void> {
    ...
    await writeFile('.github/sessions/current-session.json', content);
}
```
...
```typescript
const CONFIG_PATHS = {
    KDS_DESIGN: '.github/KDS-DESIGN.md',
    KDS_RULES: '.github/governance/rules.md',
    CURRENT_SESSION: '.github/sessions/current-session.json',
