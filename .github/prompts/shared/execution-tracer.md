...
**Location:** `.github/kds-execution.log`
...
**Location:** `.github/sessions/{session-id}.json`
...
# 1. Find KDS session
Select-String -Path ".github/kds-execution.log" -Pattern "a3f9c1b2" | Select-Object -First 1

# Output shows session: fab-button-animation

# 2. Load session file
Get-Content ".github/sessions/fab-button-animation.json" | ConvertFrom-Json
...
(Select-String -Path ".github/kds-execution.log" -Pattern "\[$today.*Session created").Count

# Average task completion time (requires timestamp parsing)
# See .github/scripts/analyze-kds-performance.ps1
```
...
```markdown
User: #file:.github/prompts/user/kds.md
