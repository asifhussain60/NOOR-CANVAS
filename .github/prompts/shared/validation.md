...
const KDS_ERRORS = {
    SESSION_NOT_FOUND: createError(
        ...
        'error',
        'Create a new session: #file:.github/prompts/user/plan.md'
    ),
    ...
    TESTS_FAILED: createError(
        ...
        'error',
        'Fix failing tests or use #file:.github/prompts/user/correct.md'
    )
};
```
...
function validateKDSPath(path: string): ValidationResult {
    // Ensure path is within .github/ directory
    if (!path.includes('\\.github\\')) {
        return {
            valid: false,
            errors: [`Path must be within .github/ directory: ${path}`]
        };
    }
