...
def step_3_cleanup():
    ...
    empty_dirs = find_empty_directories('.github/')
    for dir in empty_dirs:
        if dir not in ['.github/keys', '.github/knowledge']:  # Keep structure
            try:
                rmdir(dir)
...
def step_4_reorganization():
    ...
    # Find misplaced .md files in .github root
    root_md_files = glob('.github/*.md')
    allowed = ['.github/README.md', '.github/KDS-DESIGN.md']
...
def determine_doc_location(file):
    ...
    if any(kw in filename for kw in ['design', 'architecture', 'pattern']):
        return f".github/docs/architecture/{os.path.basename(file)}"
    
    # Database docs
    if any(kw in filename for kw in ['database', 'schema', 'migration', 'sql']):
        return f".github/docs/database/{os.path.basename(file)}"
    
    # Testing docs
    if any(kw in filename for kw in ['test', 'playwright', 'spec']):
        return f".github/docs/testing/{os.path.basename(file)}"
    
    # API docs
    if any(kw in filename for kw in ['api', 'endpoint', 'contract']):
        return f".github/docs/api/{os.path.basename(file)}"
    
    # Default: guides
    return f".github/docs/guides/{os.path.basename(file)}"
```
...
def check_performance():
    ...
    prompt_count = len(glob('.github/prompts/**/*.md'))
    file_count = len(glob('.github/**/*'))
...
def check_consistency():
    ...
    internal_agents = glob('.github/prompts/internal/*.md')
    ...
    user_prompts = glob('.github/prompts/user/*.md')
...
def step_6_living_docs_update(task_result):
    ...
    if task_result.get('structure_changed'):
        update_metrics('.github/KDS-DESIGN.md')
        updates.append('KDS-DESIGN.md updated (metrics)')
    
    # Increment version if rules modified
    if task_result.get('rules_modified'):
        increment_version('.github/KDS-DESIGN.md')
        increment_version('.github/governance/rules.md')
        updates.append('Version incremented (v4.2.0 → v4.3.0)')
    
    # Update timestamp
    update_timestamp('.github/KDS-DESIGN.md')
    update_timestamp('.github/governance/rules.md')
...
✅ Step 3: Cleanup
   ...
   - .github/old-file.md.old
   - .github/archive/ (folder removed)
   - .github/temp-backup.md.backup
...
**Next Command:**
@workspace /execute #file:.github/keys/user-dashboard/handoffs/phase-1-task-3.json
...
**Next Command:**
@workspace /execute #file:.github/keys/user-dashboard/handoffs/fix-build.json
```
