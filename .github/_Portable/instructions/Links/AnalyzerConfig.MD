...
### {{ANALYZER_TOOLS}} Code Quality Analysis
**Location:** `Workspaces/CodeQuality/{{ANALYZER_TOOLS}}/`
- **Execution Script:** `run-{{ANALYZER_TOOLS}}.ps1` with automated report generation
- **Configuration:** `Config/{{ANALYZER_TOOLS}}.config`
- **Output Format:** GitLab Code Climate JSON for CI/CD integration
- **Artifacts:** Timestamped reports in `Reports/`, logs in `Logs/`, documentation in `Workspaces/Documentation/{{ANALYZER_TOOLS}} DOCS/`
- **VS Code Tasks:** "run-{{ANALYZER_TOOLS}}-analysis" and "run-{{ANALYZER_TOOLS}}-analysis-and-open"
- **Health Metrics:** Quantitative scoring system with trend analysis

### JavaScript/TypeScript Linting
**Primary Configuration:** `config/testing/eslint.config.js`
- **Target:** `{{TEST_FRAMEWORK}}/**/*.{ts,tsx}` files
- **Parser:** `@typescript-eslint/parser`
- **Plugins:** `@typescript-eslint/eslint-plugin`, `eslint-plugin-{{TEST_FRAMEWORK}}`
- **Key Rules:**
  - `@typescript-eslint/no-unused-vars: warn`
  - `@typescript-eslint/no-explicit-any: warn`
  - `no-duplicate-imports: error`
  - `{{TEST_FRAMEWORK}}/no-skipped-test: warn`
...
- On `/refactor`:  
  - Run {{ANALYZER_TOOLS}} and all analyzers listed here.  
