...
- When user mentions "database", they mean **{{DATABASE_NAME}}**
...
**When user mentions "{{TEST_FRAMEWORK}} test" or "pwtest":**

Create automated end-to-end visual regression tests using {{TEST_FRAMEWORK}}, Percy, and any other configured libraries to reproduce and fix the issue. Ensure that the application is launched in a separate PowerShell window (not a Terminal window) before executing any headed tests to maintain proper environment isolation and visual consistency.

- Test Location: `{{TEST_FRAMEWORK}}/{{TEST_PATH}}`, `{{TEST_PATH}}UI/`, or `Workspaces/TEMP/` (temporary)
- Configuration: `config/testing/{{TEST_FRAMEWORK}}.config.cjs`
- Test Data: Session 212 (tokens: KJAHA99L user / PQ9N5YWW host)
- Base URL: `https://localhost:9091`
- Execution: `npx {{TEST_FRAMEWORK}} test` or `$env:PW_MODE='standalone'; npx {{TEST_FRAMEWORK}} test`

**See**: `{{TEST_FRAMEWORK}}QuickRef.md` for complete testing documentation
...
- **Architecture.md** - Full system design (controllers, services, {{REALTIME_TECH}} hubs, database schema)
- **InfrastructureQuickRef.md** ⭐ - DB connections ({{DATABASE_NAME}} rules), API endpoints, test data
...
- **AnalyzerConfig.MD** - Code quality tool configurations ({{ANALYZER_TOOLS}}, StyleCop, ESLint)

### Testing
- **{{TEST_FRAMEWORK}}QuickRef.md** ⭐ - Complete {{TEST_FRAMEWORK}} testing guide (test creation, execution, patterns)
- **{{TEST_FRAMEWORK}}Config.MD** - Detailed configuration reference
- **{{TEST_FRAMEWORK}}TestPaths.MD** - Canonical test patterns and test data
...
- **feature.prompt.md** - Feature Planning Agent ⭐ **NEW**
  ...
  - **KEY FEATURES**:
    ...
    - System Context Pack (APIs, database schemas, {{REALTIME_TECH}} hubs, test data)
...
- **task.prompt.md** - Canonical task executor
  ...
  - Automatic test generation using {{TEST_FRAMEWORK}}, Percy, and configured libraries (Step 6.1)
  - Functionality registry validation (Step 8.2)
  - **MUST** consult InfrastructureQuickRef.md for database rules
  - **MUST** consult {{TEST_FRAMEWORK}}QuickRef.md for test creation
...
- **question.prompt.md** - Application knowledge agent
  ...
  - **MUST** reference {{TEST_FRAMEWORK}}QuickRef.md for test-related questions
...
- **test-generation.prompt.md** - Automated end-to-end visual regression test generator
  - Creates tests using {{TEST_FRAMEWORK}}, Percy, and configured libraries
...
- {{REALTIME_TECH}} hub functionality changes
