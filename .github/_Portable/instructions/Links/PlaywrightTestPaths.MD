...
---
# {{TEST_FRAMEWORK}} Test Paths — Canonical Test Data Reference
...
// Verify {{PROJECT_NAME}} branding loads
...
#### Configuration File
**Path**: `config/testing/{{TEST_FRAMEWORK}}.config.cjs`
**webServer Settings**:
- `command: '{{RUN_COMMAND}}'`
- `cwd: '../../{{SOURCE_PATH}}'`
- `port: 9091`
...
- `{{TEST_PATH}}UI/assetlookup-database-detection.spec.ts` ✅ (Asset detection)
- `{{TEST_PATH}}UI/focused-assetshare-test.spec.ts` ✅ (Asset sharing)
- `{{TEST_FRAMEWORK}}/{{TEST_PATH}}assetlookup-api-test.spec.ts` ✅ (API integration)
- `{{TEST_FRAMEWORK}}/{{TEST_PATH}}multi-browser-participant-isolation.spec.ts` ✅ (API-based multi-browser isolation - ALL TESTS PASS)
- `{{TEST_FRAMEWORK}}/{{TEST_PATH}}simple-session-test.spec.ts` ✅ (Session canvas validation)
