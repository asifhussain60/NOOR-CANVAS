...
1. **Catalog API Response Models**
   ...
   - Note the complete namespace path (e.g., `{{PROJECT_NAME}}.Controllers.CreateSessionResponse`)
...
var response = await httpResponse.Content.ReadFromJsonAsync<{{PROJECT_NAME}}.Controllers.CreateSessionResponse>();
```
