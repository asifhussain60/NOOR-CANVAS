...
---
# {{TEST_FRAMEWORK}} Testing Quick Reference

**Version**: 2.0.0  
**Last Updated**: 2025-10-16  
**Purpose**: Authoritative {{TEST_FRAMEWORK}} testing reference - eliminates ambiguity in test creation and execution

---

## ⚠️ CRITICAL: Server Management Rules

**{{TEST_FRAMEWORK}} MANAGES THE SERVER AUTOMATICALLY - DO NOT START IT MANUALLY!**

Port policy: The application must bind only to HTTPS on 9091. Do not bind to 9090.

### ❌ NEVER Do This for {{TEST_FRAMEWORK}} Tests:
```powershell
# WRONG - Don't manually start the app
{{RUN_COMMAND}}
./Workspaces/Global/nc.ps1
./Workspaces/Global/ncb.ps1
Start-Process powershell -ArgumentList "{{RUN_COMMAND}}"
```

### ✅ ALWAYS Do This:
```bash
# RIGHT - Let {{TEST_FRAMEWORK}} manage everything
PW_MODE=standalone npx {{TEST_FRAMEWORK}} test

# {{TEST_FRAMEWORK}} automatically:
# 1. Starts {{RUN_COMMAND}} (background subprocess)
...
- **NEVER**: For running automated {{TEST_FRAMEWORK}} tests
...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd 'D:\PROJECTS\{{PROJECT_NAME}}\SPA\{{PROJECT_NAME}}'; $env:ASPNETCORE_URLS='https://localhost:9091'; {{RUN_COMMAND}}" -WindowStyle Normal
...
```powershell
# Run {{TEST_FRAMEWORK}} tests in the CURRENT terminal window
npx {{TEST_FRAMEWORK}} test path/to/test.spec.ts --headed --reporter=list
```
...
$appProcess = Get-Process powershell | Where-Object { 
    $_.MainWindowTitle -like "*{{RUN_COMMAND}}*" -or 
    $_.CommandLine -like "*{{PROJECT_NAME}}*{{RUN_COMMAND}}*" 
}
...
# 3. Run tests
npx {{TEST_FRAMEWORK}} test Workspaces/TEMP/share-transcript-navigation.spec.ts --headed
...
- **❌ NEVER use**: For standard CI/CD tests in `{{TEST_FRAMEWORK}}/{{TEST_PATH}}`
- **❌ NEVER use**: When `webServer` configuration is available in `{{TEST_FRAMEWORK}}.config.cjs`

---

## 🎯 Testing Strategy Overview

**{{PROJECT_NAME}} uses three complementary testing approaches:**

### 1. **Functional E2E Tests** ({{TEST_FRAMEWORK}} - Standard)
- **Purpose**: Verify user interactions and business logic
- **When**: User flows, API integration, multi-user scenarios, {{REALTIME_TECH}} broadcasts
- **Tools**: {{TEST_FRAMEWORK}} `@{{TEST_FRAMEWORK}}/test`
- **Example**: Submit question, vote on question, participant registration

### 2. **Visual Regression Tests** (Percy + {{TEST_FRAMEWORK}})
- **Purpose**: Catch visual/CSS rendering issues
- **When**: CSS changes, layout modifications, responsive design, color scheme updates
- **Tools**: Percy `@percy/{{TEST_FRAMEWORK}}` + {{TEST_FRAMEWORK}}
...
## 🎯 When User Says "{{TEST_FRAMEWORK}} Test" or "pwtest"

**Copilot Should Know**:
1. **Test Location**: `{{TEST_FRAMEWORK}}/{{TEST_PATH}}` or `{{TEST_PATH}}UI/` or `Workspaces/TEMP/` (for temporary tests)
2. **Configuration**: `config/testing/{{TEST_FRAMEWORK}}.config.cjs`
...
### Test Location Rules
1. **Permanent Tests**: `{{TEST_FRAMEWORK}}/{{TEST_PATH}}` or `{{TEST_PATH}}UI/`
...
|------------|--------------|-----|---------|
| **User Workflow/Behavior** | {{TEST_FRAMEWORK}} (Functional E2E) | Tests actual user interactions, API calls, {{REALTIME_TECH}} updates | User joins session, submits question, votes |
...
| **Component Refactor** | Percy + {{TEST_FRAMEWORK}} | Percy validates visual consistency, {{TEST_FRAMEWORK}} validates behavior | Refactoring question card component |
| **API Change** | {{TEST_FRAMEWORK}} (Functional E2E) | Tests API contract, response format, error handling | New participant endpoint, {{REALTIME_TECH}} hub changes |
...
| **Accessibility Fix** | {{TEST_FRAMEWORK}} (Functional E2E) | Tests ARIA attributes, keyboard navigation, screen reader support | ARIA labels, focus management, semantic HTML |
...
Did you change...
│
├─ User flows/interactions? → {{TEST_FRAMEWORK}} Functional Test
│  ├─ Examples: Login, navigation, form submission, voting
│  └─ File: {{TEST_PATH}}UI/*-functional.spec.ts
│
├─ Visual appearance/CSS? → Percy Visual Test + Stylelint
│  ├─ Examples: Button colors, card layouts, spacing, themes
│  ├─ Visual Test: {{TEST_PATH}}UI/*-visual.spec.ts (Percy)
│  └─ CSS Quality: npm run lint:css (Stylelint)
│
├─ API/{{REALTIME_TECH}} contract? → {{TEST_FRAMEWORK}} Functional Test
│  ├─ Examples: New endpoints, hub methods, response format
│  └─ File: {{TEST_PATH}}UI/*-api.spec.ts
│
├─ Component internals (no UI change)? → {{TEST_FRAMEWORK}} Functional Test
...
└─ Multi-viewport responsive design? → Percy Multi-viewport Test
   ...
   └─ File: {{TEST_PATH}}UI/*-responsive.spec.ts
```

### Quick Reference Commands

```bash
# Functional E2E Tests ({{TEST_FRAMEWORK}})
npx {{TEST_FRAMEWORK}} test {{TEST_PATH}}UI/feature-name-functional.spec.ts --headed

# Visual Regression Tests (Percy + {{TEST_FRAMEWORK}})
npm run test:percy:visual -- {{TEST_PATH}}UI/feature-name-visual.spec.ts

# CSS Quality Check (Stylelint)
npm run lint:css -- {{SOURCE_PATH}}/Components/**/*.razor

# Full Test Suite (All 3 Approaches)
npx {{TEST_FRAMEWORK}} test && npm run test:percy && npm run lint:css
```
...
### Primary Config: `config/testing/{{TEST_FRAMEWORK}}.config.cjs`
...
```javascript
{
  ...
  testMatch: [
    ...
    '**/{{TEST_PATH}}UI/**/*.{test,spec}.{js,ts,jsx,tsx}',
...
**Usage**:
```bash
$env:PW_MODE='standalone'; npx {{TEST_FRAMEWORK}} test
```

**Features**:
- Starts .NET app automatically (`{{RUN_COMMAND}}` in `{{SOURCE_PATH}}`)
- Port: 9091
...
- Report: `Workspaces/TEMP/{{TEST_FRAMEWORK}}-report/standalone-html`
...
**Usage**:
```bash
$env:PW_MODE='temp'; npx {{TEST_FRAMEWORK}} test
```
...
```bash
npx {{TEST_FRAMEWORK}} test {{TEST_PATH}}UI/session-canvas.spec.ts
```

### Run Tests in Headed Mode
```bash
npx {{TEST_FRAMEWORK}} test --headed
```

### Run Tests with UI Mode (Interactive)
```bash
npx {{TEST_FRAMEWORK}} test --ui
```
...
```bash
npx {{TEST_FRAMEWORK}} test --project=chromium
npx {{TEST_FRAMEWORK}} test --project=firefox
npx {{TEST_FRAMEWORK}} test --project=webkit
```

### Debug Mode
```bash
npx {{TEST_FRAMEWORK}} test --debug
```

### Generate Report
```bash
npx {{TEST_FRAMEWORK}} show-report
```
...
### 4. {{REALTIME_TECH}} Real-Time Test
```typescript
test('should receive {{REALTIME_TECH}} updates', async ({ page }) => {
  await page.goto('https://localhost:9091/session/canvas/KJAHA99L');
  
  // Trigger action that should broadcast
  await page.click('[data-testid="vote-button"]');
  
  // Wait for {{REALTIME_TECH}} update to reflect
...
- Configured in `{{TEST_FRAMEWORK}}.config.cjs`
...
### 4. Trace Viewer
```bash
npx {{TEST_FRAMEWORK}} show-trace test-results/trace.zip
```
...
```json
{
  "compilerOptions": {
    ...
    "skipLibCheck": true,
    "strict": true,
    "types": ["node", "@{{TEST_FRAMEWORK}}/test"]
  },
  "include": [
    "../../{{TEST_FRAMEWORK}}/**/*.ts",
    "../../{{TEST_PATH}}UI/**/*.ts",
...
Remove-Item "Workspaces/TEMP/{{TEST_FRAMEWORK}}-artifacts/*" -Recurse -Force
```
...
**Location**: `{{TEST_FRAMEWORK}}-report/` or mode-specific folder

### JSON Report
```bash
npx {{TEST_FRAMEWORK}} test --reporter=json
```

### Custom Reporter
Configured in `{{TEST_FRAMEWORK}}.config.cjs` per mode
...
### With Task Agent
- **Step 6.1**: Automatic {{TEST_FRAMEWORK}} test generation for UI tasks
...
- Level 5: E2E Testing with {{TEST_FRAMEWORK}}
...
- **{{TEST_FRAMEWORK}}Config.MD** - Detailed configuration reference
- **{{TEST_FRAMEWORK}}TestPaths.MD** - Canonical test patterns and Session 212 data
- **InfrastructureQuickRef.md** - API endpoints for test assertions
- **Architecture.md** - {{REALTIME_TECH}} hubs and real-time testing
...
- Configuration changes in `{{TEST_FRAMEWORK}}.config.cjs`
- New test data added (beyond Session 212)
- New environment modes added
- Test execution strategy changes

**Verification**:
- Compare with `config/testing/{{TEST_FRAMEWORK}}.config.cjs`
- Validate Session 212 test data still valid
- Ensure test commands match actual npm scripts
- Cross-reference with `{{TEST_FRAMEWORK}}Config.MD` and `{{TEST_FRAMEWORK}}TestPaths.MD`

---

**Version History**:
- **1.0.0** (2025-10-12): Initial creation - Consolidated {{TEST_FRAMEWORK}} testing knowledge
