...
- **File Watch**: 
  - `{{SOURCE_PATH}}/Pages/UserLanding.razor`
  - `{{SOURCE_PATH}}/Services/AuthService.cs`
- **Method Watch**:
...
- **Automated Tests**:
  - `{{TEST_PATH}}UI/user-auth-flow.spec.ts` - Covers behaviors 1, 2, 3
- **Manual Validation**:
...
```bash
npm test -- {{TEST_PATH}}UI/user-auth-flow.spec.ts
...
### Related Test Coverage
- **Automated Tests**:
  - `{{TEST_PATH}}UI/feature-workflow.spec.ts` - Covers behaviors 1, 2, 3
  - `{{TEST_PATH}}Unit/ServiceTests.cs` - Unit test coverage
- **Manual Validation**:
...
**Priority Order**:
1. **Automated E2E Tests** ({{TEST_FRAMEWORK}}) - Best coverage, run automatically
2. **Automated Unit Tests** - Fast, cover specific logic
3. **Manual Validation Checklist** - Fallback when no automated tests exist

**Recommendation**: Create {{TEST_FRAMEWORK}} tests for all core behaviors over time.
...
### Related Test Coverage
- **Automated Tests**: None (recommended: create {{TEST_FRAMEWORK}} test)
- **Manual Validation**:
...
### Breaking Change Detection
- **File Watch**:
  - `{{SOURCE_PATH}}/Pages/UserLanding.razor` - Main UI component
  - `{{SOURCE_PATH}}/Controllers/ParticipantController.cs` - Validation API
  - `{{SOURCE_PATH}}/Services/SimplifiedTokenService.cs` - Token logic
- **Method Watch**:
...
### Breaking Change Detection
- **File Watch**:
  - `{{SOURCE_PATH}}/Pages/SessionCanvas.razor` - Canvas rendering
  - `{{SOURCE_PATH}}/wwwroot/css/session-transcript.css` - Asset styling
- **Method Watch**:
...
### Phase 2 (Future)
- Automated {{TEST_FRAMEWORK}} test execution when File Watch triggers
