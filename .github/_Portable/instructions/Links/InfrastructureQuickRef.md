...
**When user mentions "database", they mean {{DATABASE_NAME}} unless specified otherwise.**

**Connection Details**:
- **Database Name**: `{{DATABASE_NAME}}`
- **Server**: `{{DATABASE_SERVER}}`
- **Authentication**: {{DATABASE_TYPE}} authentication (credentials in {{CONFIG_PATH}}/appsettings.json)
- **Connection Key**: `"{{CONNECTION_STRING_KEY}}"` or `"KSessionsDb"`
- **Configuration Location**: `{{SOURCE_PATH}}/{{CONFIG_PATH}}/appsettings.json` → `ConnectionStrings` section
...
- `dbo.*` **Stored Procedures** - **READ-ONLY**
  ...
  > **NOTE**: Tables verified against {{DATABASE_NAME}} codebase on 2025-10-12.
  > Only tables with {{FRAMEWORKS}} models or direct SQL usage are listed.
...
```csharp
// Correct approach
_configuration.GetConnectionString("{{CONNECTION_STRING_KEY}}")
_configuration.GetConnectionString("KSessionsDb")
_configuration.GetConnectionString("KQurDb")

// WRONG - Never do this
var connString = "Server={{DATABASE_SERVER}};Database={{DATABASE_NAME}};..."
```
...
### Required Services
1. **{{DATABASE_TYPE}}** ({{DATABASE_SERVER}} server)
...
3. **{{REALTIME_TECH}} Hubs** (Real-time communication)
...
### Configuration Files
- `{{CONFIG_PATH}}/appsettings.json` - Production settings
- `appsettings.Development.json` - Development overrides
- **Location**: `{{SOURCE_PATH}}/`

### Environment Variables (Optional)
- `ASPNETCORE_ENVIRONMENT` - Development/Production
- Connection strings can override {{CONFIG_PATH}}/appsettings.json values
...
**Purpose**: Testing/debugging {{REALTIME_TECH}} connections
...
**Config File**: `config/testing/{{TEST_FRAMEWORK}}.config.cjs`

**Server Management**:
- `PW_MODE=standalone` - {{TEST_FRAMEWORK}} does NOT start server (recommended)
- `PW_MODE=managed` - {{TEST_FRAMEWORK}} starts/stops server automatically
...
### Testing
- **PW_MODE**: `standalone` | `managed` ({{TEST_FRAMEWORK}} server management)
...
### Configuration
- **{{CONFIG_PATH}}/appsettings.json**: `{{SOURCE_PATH}}/{{CONFIG_PATH}}/appsettings.json`
- **{{TEST_FRAMEWORK}} Config**: `config/testing/{{TEST_FRAMEWORK}}.config.cjs`
- **ESLint Config**: `config/testing/eslint.config.js`
- **TypeScript Config**: `config/testing/tsconfig.json`

### Code
- **Controllers**: `{{SOURCE_PATH}}/Controllers/*.cs`
- **Services**: `{{SOURCE_PATH}}/Services/*.cs`
- **Hubs**: `{{SOURCE_PATH}}/Hubs/*.cs`
- **Components**: `{{SOURCE_PATH}}/Components/**/*.razor`
- **Pages**: `{{SOURCE_PATH}}/Components/Pages/*.razor`

### Tests
- **E2E ({{TEST_FRAMEWORK}})**: `{{TEST_PATH}}UI/*.spec.ts`
- **Unit Tests**: `{{TEST_PATH}}Unit/*.cs`

### Documentation
- **Architecture**: `.github/instructions/Links/{{PROJECT_NAME}}_ARCHITECTURE.MD`
...
- **Annotation System** - DELETED in annotation removal commit (2025-01-11)
  ...
  - No `AnnotationHub` {{REALTIME_TECH}} hub
...
- **v1.0.0** (2025-01-11): Initial creation
  - Extracted from live {{CONFIG_PATH}}/appsettings.json, Program.cs, Controllers
  - Added Session 212 canonical test data
  - Documented all API endpoints and {{REALTIME_TECH}} hubs
