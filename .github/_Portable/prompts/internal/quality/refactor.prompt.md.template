...
- **When Analyzers Flag Issues**: {{ANALYZER_TOOLS}}, StyleCop, or .NET analyzers report violations
...
- **Reads From**: 
  ...
  - AnalyzerConfig.MD ({{ANALYZER_TOOLS}}, StyleCop, .NET Analyzers)
...
- All tests passing (unit, integration, {{TEST_FRAMEWORK}})
...
- AFTER refactoring: include Work Requested (with key), Refactorings completed ([x]), Validation results (build/{{TEST_PATH}}analyzers), the attachments note, and **Next Actions (2-4 clear options)**.
...
- **CONTINUOUS VALIDATION**: Run `{{BUILD_COMMAND}}` after every significant change to catch issues immediately.  
...
- Apply analyzers from **`.github/instructions/Links/AnalyzerConfig.MD`** including:  
  - **{{ANALYZER_TOOLS}}** ({{LANGUAGES}} static analysis and refactoring)  
  - .NET Analyzers (`Microsoft.CodeAnalysis.NetAnalyzers`)  
  - StyleCop (with suppression rules)  
  - JavaScript/TypeScript linting (`eslint`, `eslint-plugin-{{TEST_FRAMEWORK}}`)  
...
- **Integration Points**: Assess {{REALTIME_TECH}}, API contracts, and cross-service communications
...
- **Dependency Security**: Review {{PACKAGE_MANAGER}} packages and npm dependencies for vulnerabilities  
...
- **Configuration Redundancy Detection**: Identify and consolidate duplicate or redundant configuration entries
  - **Connection String Consolidation**: Scan all `appsettings*.json` files for redundant connection strings
    ...
    - Consolidate to single `{{CONNECTION_STRING_KEY}}` entry where possible
  ...
  - **Duplicate Configuration Entries**: Find configuration sections duplicated across files
    ...
    - Move common settings to base `{{CONFIG_PATH}}/appsettings.json`
  ...
  - **Cross-Project Configuration Sync**: Ensure consistent configuration patterns across projects
    - Compare `{{SOURCE_PATH}}` vs `Tools/HostProvisioner` appsettings
...
- **Phase 5**: Integration points ({{REALTIME_TECH}} hubs, cross-feature dependencies)
...
3. **Phase Execution**
   ...
   - **MANDATORY**: Run `{{BUILD_COMMAND}}` after each file modification
...
4. **Phase Validation** (MANDATORY - ZERO TOLERANCE)
   ...
   - Run all tests (unit, integration, {{TEST_FRAMEWORK}} functional, Percy visual if UI changes)
...
├─ Validation: Build clean, {{TEST_FRAMEWORK}} tests passing
...
- **MANDATORY BUILD VALIDATION (After Every Change):**  
  - Run `{{BUILD_COMMAND}}` immediately after each file modification.  
...
- **Quality Assurance Pipeline:**  
  - Execute {{ANALYZER_TOOLS}} via `run-{{ANALYZER_TOOLS}}.ps1` and ensure ZERO diagnostics.  
...
- **MANDATORY BUILD VERIFICATION:**  
  - Execute `{{BUILD_COMMAND}} --configuration Release --verbosity normal` for complete validation.  
  - Execute `{{BUILD_COMMAND}} --configuration Debug --verbosity normal` for debug validation.  
...
- **Comprehensive Validation Pipeline:**  
  ...
  - Execute `Workspaces/CodeQuality/run-{{ANALYZER_TOOLS}}.ps1` and verify ZERO unresolved diagnostics.  
  - Run `{{LINT_COMMAND}} --verify-no-changes` to ensure consistent formatting.  
  - Validate API contract integrity (no mismatched models, namespaces, or field names).  
  - Ensure {{TEST_FRAMEWORK}} tests pass for impacted components.  
...
- **CONTINUOUS BUILD MONITORING**: After each individual change, immediately run:  
  - `{{BUILD_COMMAND}} --verbosity minimal` to catch issues instantly  
...
- **Instruction File Updates:**  
  ...
  - Modify `.github/instructions/Links/{{PROJECT_NAME}}_ARCHITECTURE.MD` to reflect architectural improvements.  
...
- **CRITICAL**: Execute `{{BUILD_COMMAND}} --configuration Release` with **ZERO ERRORS AND ZERO WARNINGS**.  
- **CRITICAL**: Execute `{{BUILD_COMMAND}} --configuration Debug` with **ZERO ERRORS AND ZERO WARNINGS**.  
- **CRITICAL**: Run `{{LINT_COMMAND}} --verify-no-changes` with no formatting issues.  
- **CRITICAL**: Execute `Workspaces/CodeQuality/run-{{ANALYZER_TOOLS}}.ps1` with ZERO unresolved diagnostics.  
- All analyzers, linters, and {{ANALYZER_TOOLS}} checks must pass with **ZERO** issues (not just "no blocking" issues).  
- All automated tests (unit, integration, {{TEST_FRAMEWORK}}) must pass with 100% success rate.  
...
- **BUILD COMMANDS FOR VALIDATION:**  
  - `dotnet clean && {{BUILD_COMMAND}} --configuration Release --verbosity normal`  
  - `dotnet clean && {{BUILD_COMMAND}} --configuration Debug --verbosity normal`  
  - `{{LINT_COMMAND}} --verify-no-changes --verbosity diagnostic`  
- **Instruction files must be consistent** with refactored codebase:  
  ...
  - `{{PROJECT_NAME}}_ARCHITECTURE.MD` matches current system design  
...
3. **Static Analysis Validation:**
   ```powershell
   .\Workspaces\CodeQuality\run-{{ANALYZER_TOOLS}}.ps1
   ```

4. **Test Suite Validation:**
   ```powershell
   {{TEST_COMMAND}} --configuration Release --verbosity normal
   ```
...
- Confirm mappings in `SystemStructureSummary.md` and `{{PROJECT_NAME}}_ARCHITECTURE.MD`.  
