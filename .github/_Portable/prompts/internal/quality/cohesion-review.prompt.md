...
Get-Process -Name dotnet -ErrorAction SilentlyContinue | Where-Object { 
    $_.MainWindowTitle -like '*Kestrel*' -or $_.Path -like '*{{PROJECT_NAME}}*' 
} | Stop-Process -Force
```
...
4. **Pattern Integration Points**:
   ...
   - **refactor.prompt.md**: Add code quality patterns from {{ANALYZER_TOOLS}} reports
   - **healthcheck.prompt.md**: Add validation patterns from failure logs
   - **test-generation.prompt.md**: Add test patterns from {{TEST_FRAMEWORK}} successes
...
4. **Spot validation duplication**: Count "{{BUILD_COMMAND}}" occurrences
...
- Replace verbose inline protocols with references to shared docs:
   ...
   - `shared/{{TEST_FRAMEWORK}}-test-generation.md` and `shared/test-orchestration-patterns.md` (for Step 6 test generation and orchestration)
...
```powershell
cd "D:\PROJECTS\{{PROJECT_NAME}}\Workspaces\Scripts"
...
**Script Actions**:
1. Query {{DATABASE_NAME}} for actual database schema
...
- {{TEST_FRAMEWORK}}QuickRef.md
- ValidationFramework.md
- API-Contract-Validation.md
- AnalyzerConfig.MD
- {{TEST_FRAMEWORK}}Config.MD
- {{TEST_FRAMEWORK}}TestPaths.MD
- FunctionalityRegistry.md
- SelfAwareness.instructions.md

**Consolidation Opportunities**:
- Combine similar files (e.g., {{TEST_FRAMEWORK}}QuickRef.md + {{TEST_FRAMEWORK}}Config.MD + {{TEST_FRAMEWORK}}TestPaths.MD → single comprehensive file)
...
2. **Combine related configurations** ({{TEST_FRAMEWORK}} files, Analyzer files)
...
- ✅ Files were merged/consolidated (e.g., {{TEST_FRAMEWORK}} files combined)
...
7. ✅ **QuickRef files updated** (InfrastructureQuickRef.md, {{TEST_FRAMEWORK}}QuickRef.md)
...
1. **InfrastructureQuickRef.md**
   - Database connection strings (verify against {{CONFIG_PATH}}/appsettings.json)
   - API endpoints (verify against Architecture.md)
   - {{REALTIME_TECH}} hubs (verify against code)
   - External dependencies ({{DATABASE_TYPE}}, Kestrel ports)
   - Schema access rules (canvas.* vs dbo.*)

2. **{{TEST_FRAMEWORK}}QuickRef.md**
   - Test patterns (verify against {{TEST_FRAMEWORK}}Config.MD)
...
2. **Compare with source of truth**:
   - {{CONFIG_PATH}}/appsettings.json for database connections
   - Architecture.md for API endpoints
   - {{TEST_FRAMEWORK}}.config.cjs for test configuration
   - Actual test files for patterns
   - **ACTUAL DATABASE SCHEMA** (query {{DATABASE_NAME}}) ⚠️ MANDATORY
...
```sql
-- Connect to {{DATABASE_NAME}} and list all tables
USE {{DATABASE_NAME}};
...
```powershell
# Search for database table references in {{LANGUAGES}} code
grep -r "{{SCHEMA_READONLY}}.TableName" --include="*.cs" Data/ Services/ Controllers/

# Search for {{FRAMEWORKS}} DbSet definitions
grep -r "DbSet<" --include="*.cs" Data/

# Search for SQL queries in migration scripts
grep -r "FROM {{SCHEMA_READONLY}}." --include="*.sql" Scripts/ Migrations/
```
...
**Step 1: Read Actual {{CONFIG_PATH}}/appsettings.json**
```powershell
# Extract connection strings
Get-Content {{CONFIG_PATH}}/appsettings.json | Select-String -Pattern "ConnectionStrings" -Context 0,10

# Extract specific settings
Get-Content {{CONFIG_PATH}}/appsettings.json | Select-String -Pattern "AzureAdB2C" -Context 0,5
```
...
**Script Actions**:
1. **Queries {{DATABASE_NAME}}** for actual canvas.* and dbo.* tables
...
4. **Searches codebase** for table references in {{LANGUAGES}} files
...
- Verify {{TEST_FRAMEWORK}}QuickRef.md matches test-generation.prompt.md patterns
