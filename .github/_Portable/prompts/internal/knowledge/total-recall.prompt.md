...
2) Commands
- .NET: parse `.csproj`/Directory.Build.props; common commands: `{{BUILD_COMMAND}}`, `{{TEST_COMMAND}}`, `{{RUN_COMMAND}}`, `{{LINT_COMMAND}}`.
...
3) Database
- Detect ORM/configs: EF Core (Migrations/, {{CONFIG_PATH}}/appsettings.json ConnectionStrings), Sequelize/Prisma, SQLAlchemy, Hibernate/JPA, ActiveRecord, etc.
...
- Realtime: {{REALTIME_TECH}}, Socket.IO, WebSockets, ActionCable, etc.
