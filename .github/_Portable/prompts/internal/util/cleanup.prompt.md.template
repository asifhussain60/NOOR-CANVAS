...
- Modify configuration files (`{{CONFIG_PATH}}/appsettings.json`, `web.config`, `*.csproj`)
...
**Example:** `Scripts,Docs,Workspaces/TEMP,{{TEST_FRAMEWORK}}/tests`
...
#### 4. Misplaced Files
- Test files in non-test folders (`*.spec.ts` outside `{{TEST_PATH}}`)
...
- 📦 MOVE: `Scripts/test-debug-panel.spec.ts` → `{{TEST_PATH}}UI/test-debug-panel.spec.ts`
...
- 🗑️ DELETE: `Workspaces/TEMP/old-{{TEST_PATH}}` (empty folder)
...
✅ [5/15] MOVE Scripts/test-debug-panel.spec.ts → {{TEST_PATH}}UI/ (3 references updated)
✅ [6/15] MOVE Scripts/deployment-notes.md → Docs/ (2 references updated)
...
✅ [15/15] DELETE Workspaces/TEMP/old-{{TEST_PATH}} (empty folder removed)
...
$buildResult = {{BUILD_COMMAND}} {{SOURCE_PATH}}/{{PROJECT_NAME}}.csproj --verbosity quiet
...
if (Test-Path "{{PROJECT_NAME}}.sln") {
    $slnBuild = {{BUILD_COMMAND}} {{PROJECT_NAME}}.sln --verbosity quiet
...
$criticalFiles = @(
    "{{SOURCE_PATH}}/{{PROJECT_NAME}}.csproj",
    "{{SOURCE_PATH}}/Program.cs",
    "{{SOURCE_PATH}}/{{CONFIG_PATH}}/appsettings.json",
    ".github/instructions/SelfAwareness.instructions.md",
    "package.json",
    "{{TEST_FRAMEWORK}}.config.cjs"
)
...
- ✅ MOVED: `Scripts/test-debug-panel.spec.ts` → `{{TEST_PATH}}UI/`
...
- ✅ DELETED: `Workspaces/TEMP/old-{{TEST_PATH}}` (empty folder)
...
### Build Validation
```
{{BUILD_COMMAND}} {{SOURCE_PATH}}/{{PROJECT_NAME}}.csproj
Build succeeded.
```
...
Changes to be committed:
  ...
  renamed:    Scripts/test-debug-panel.spec.ts → {{TEST_PATH}}UI/test-debug-panel.spec.ts
...
**If tests created** → Include `{{TEST_PATH}}UI/`, `{{TEST_FRAMEWORK}}/{{TEST_PATH}}`  
...
@workspace /cleanup target_folders="Scripts,{{TEST_PATH}}UI,.github/prompts.keys/user-landing" scope=analyze-and-propose dry_run=true github-branch=development
```
...
```csharp
// Before
using {{PROJECT_NAME}}.Services.OldLocation;

// After
using {{PROJECT_NAME}}.Services.NewLocation;
```
