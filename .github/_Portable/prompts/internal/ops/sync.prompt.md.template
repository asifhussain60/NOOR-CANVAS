...
- **Configuration Updates**: Refresh AnalyzerConfig.MD, {{TEST_FRAMEWORK}}Config.MD, ValidationFramework.md
...
### How to Invoke
```
@workspace /sync key=system-docs notes="update architecture documentation after {{REALTIME_TECH}} changes"
...
- **All {{TEST_FRAMEWORK}} and Percy tests executed only after server is started in a separate admin PowerShell window (not VS Code terminal)**
...
- Clean up temporary test files in `Workspaces/TEMP/` (keep production tests in {{TEST_PATH}}UI/)
...
⚠️ **ABSOLUTE MANDATE: ALL {{TEST_FRAMEWORK}} TESTS REQUIRE ORCHESTRATION SCRIPTS** ⚠️

**Before running any {{TEST_FRAMEWORK}} or Percy automated tests as part of sync or cleanup operations, you MUST use orchestration scripts. Direct execution of `npx {{TEST_FRAMEWORK}} test` is PROHIBITED.**
...
- ✅ Set `$env:ASPNETCORE_ENVIRONMENT = 'Development'` and `$env:ASPNETCORE_URLS = 'https://localhost:9091'` before `{{RUN_COMMAND}}`
- ✅ Health check with retry logic (10 attempts, 3-second delays)
- ✅ Automated cleanup after tests complete
- ❌ NEVER run `npx {{TEST_FRAMEWORK}} test` directly from terminal
...
- **InfrastructureQuickRef.md** ⭐ - Database ({{DATABASE_NAME}} schema rules), API, {{REALTIME_TECH}}, test infrastructure
...
- **Synchronization:**  
  ...
  - Replace `[PLACEHOLDER]` blocks with live repo data (AnalyzerConfig, {{TEST_FRAMEWORK}}Config, etc.).  
  ...
  - **Consolidate similar data streams using pattern matching:**
    ...
    - Files with similar purposes (e.g., multiple {{TEST_FRAMEWORK}} configuration files)
  ...
  - **Pattern Matching Rules:**
    ...
    - Match related terms: "quick ref" ↔ "paths" ↔ "config" (if {{TEST_FRAMEWORK}}-related)
    - Preserve unique information during consolidation
    - Update all references to point to consolidated files
  - **Consolidation Examples:**
    - Merge: {{TEST_FRAMEWORK}}QuickRef.md + {{TEST_FRAMEWORK}}Config.MD + {{TEST_FRAMEWORK}}TestPaths.MD → {{TEST_FRAMEWORK}}Reference.md
...
- **Ground Truth Validation (MANDATORY):**
  - **Execute validation script before finalizing sync:**
    ...
    ```
  - **Script validates:**
    - ✅ Database schema (query {{DATABASE_NAME}} for actual tables)
    - ✅ Codebase references (grep for actual usage in {{LANGUAGES}} files)
...
- **Confirm all {{TEST_FRAMEWORK}} and Percy tests were executed only after server was started in a separate admin PowerShell window (not VS Code terminal).**
