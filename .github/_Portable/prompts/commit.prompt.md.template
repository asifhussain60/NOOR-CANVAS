...
1. **Search for Debug Markers**:
   ...
   $debugFiles = Get-ChildItem -Path "SPA/", "{{TEST_PATH}}/", "Scripts/" -Recurse -Include "*.cs", "*.razor", "*.js", "*.ts" |
...
4. **Verify Cleanup**:
   ...
   $remainingMarkers = Get-ChildItem -Path "SPA/", "{{TEST_PATH}}/", "Scripts/" -Recurse -Include "*.cs", "*.razor", "*.js", "*.ts" |
       Where-Object { (Get-Content $_.FullName -Raw) -match '\[DEBUG-WORKITEM:.*\] ;CLEANUP_OK' }
