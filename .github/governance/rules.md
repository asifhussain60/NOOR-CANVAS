...
**Companion:** `.github/KDS-DESIGN.md` (human-readable)
...
pre_commit_validation:
  ...
  check_files: .github/** ONLY
  forbidden_files: SPA/**, Tests/**, Scripts/** (outside .github)
...
error_messages:
  ...
  non_kds_files: "❌ KDS branch ONLY for .github/ changes"
```
...
output_format: |
  ...
  **Next Command:**
  @workspace /execute #file:.github/keys/{key}/handoffs/{next}.json
...
error_handling:
  ...
  error_report_format: |
    ...
    Next Command:
    @workspace /execute #file:.github/keys/{key}/handoffs/fix-build.json
```
...
post_request_cleanup:
  step_1: Scan .github/*.md (exclude README.md, KDS-DESIGN.md)
...
mandatory_sequence:
  ...
  step_4_reorganization:
    ...
    actions:
      - Scan .github/*.md (exclude README.md, KDS-DESIGN.md)
  ...
  step_5_kds_verification:
    ...
    checks:
      ...
      performance:
        ...
        - Count .github files (soft limit: 70, hard limit: 80)
...
validation_workflow:
  step_1_analyze_request:
    ...
    checks:
      - Does request modify .github/ files?
...
enforcement:
  when_to_challenge:
    - ANY request affecting .github/ structure
...
def validate_kds_branch():
    ...
    non_kds_files = [f for f in changed_files if not f.startswith(".github/")]
...
- [ ] Rule #5: Check files = .github/** only
...
**Companion Doc:** `.github/KDS-DESIGN.md` (human-readable)
