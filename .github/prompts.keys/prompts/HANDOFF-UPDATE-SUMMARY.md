...
Updated `plan.prompt.md` to enable **automatic handoff execution** from plan agent to task agent.

### Problem Addressed

**Before:** Plan agent documented the handoff command but didn't execute it
...
**After:** Plan agent now explicitly executes the handoff command
...
1. **Main Update:** `[prompts] Update plan.prompt.md to execute automatic handoff`
   - Modified `.github/prompts/plan.prompt.md`
...
- `.github/prompts/plan.prompt.md` (4 sections updated)
...
**Before:**
1. User: "proceed"
2. Plan agent: "Here's the command, copy and paste it"
...
**After:**
1. User: "proceed"
2. Plan agent: "✓ Plan finalized. Invoking task agent now..."
3. Plan agent: [sends @workspace /task command]
...
User Request
    ↓
Plan Agent (creates plan files)
    ↓
Plan Agent (sends @workspace /task command) ← NEW: Actual execution
    ↓
...
3. ✅ **@workspace /task command sent by plan agent**
4. ✅ Task agent receives and processes command
5. ✅ Phase 1 implementation begins

### Failure Modes Prevented

❌ Plan agent shows command but doesn't execute → User confused  
❌ Plan agent documents command only → Manual copy/paste required  
❌ Plan agent stops after writing files → Workflow breaks  
...
1. Verify plan agent writes files
2. Verify plan agent sends @workspace /task command
...
- ✅ Plan agent handles orchestration
