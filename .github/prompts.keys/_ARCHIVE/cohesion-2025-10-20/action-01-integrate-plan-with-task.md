# Action 01: Integrate feature.prompt.md with task.prompt.md
...
feature.prompt.md (added 2025-10-20, 3740 lines) is a new Feature Planning Agent that hands off to task.prompt.md. However, task.prompt.md doesn't document receiving handoffs from feature.prompt.md, creating confusion for users.
...
**Current State**:
- feature.prompt.md generates comprehensive plans and invokes task.prompt.md
- task.prompt.md has no documentation about plan handoffs
- No mention of feature.prompt.md in task.prompt.md's "Integration with Other Agents" section
...
- **Called By**: 
  - **feature.prompt.md** (Feature Planning Agent) - Receives comprehensive execution plans
...
- **When to Use**:
  - **Use feature.prompt.md first** if:
...
**Add**:
```markdown
### feature planning agent Handoff Protocol

When invoked by feature.prompt.md, task agent receives:
...
6. Report completion back to user (feature planning agent monitors)
```
...
**Add example**:
```markdown
### Example: Via feature planning agent Handoff

**User initiates planning**:
```
@workspace /feature key=canvas-sharing user_request="Add share button to canvas" github-branch=development
```

**feature planning agent generates plan, user approves, plan invokes task**:
```
...
**Responsibilities**:
- **feature.prompt.md**: Creates plan, gets approval, initiates handoff
...
- [ ] Read feature.prompt.md handoff section (Step 6)
- [ ] Identify current gaps in documentation

### Post-Implementation Checks
- [ ] task.prompt.md documents plan handoff in "Integration" section
- [ ] task.prompt.md has "feature planning agent Handoff Protocol" section
...
- Action 02: Add feature.prompt.md to SystemIndex.md
- Action 03: Extract shared modules from feature.prompt.md
