...
### 2. plan.prompt.md Integration

**Location**: `.github/prompts/plan.prompt.md` (lines ~2900-2950)
...
2. Plan agent identifies affected folders (Scripts/, Tests/, Docs/, prompts.keys/)
3. Plan agent automatically invokes cleanup agent with target folders
...
| `github-branch` | string | `development` | Target branch (validated same as plan agent) |
...
```
User: @workspace /plan key=user-landing user_request="Add registration guard" github-branch=development
...
1. Plan agent completes final phase
2. Plan agent identifies affected folders
3. Plan agent invokes cleanup agent automatically
...
- Same branch validation as plan agent
...
└── plan.prompt.md (MODIFIED - added cleanup integration)
```
...
✅ plan.prompt.md integration complete  
...
- **plan.prompt.md** - Integrated cleanup invocation (MODIFIED)
