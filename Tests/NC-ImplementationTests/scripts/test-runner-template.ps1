...
Executes complete test pipeline: lint → build → tests → integration

Phases:
  0. Lint - HTML documentation and project files
  1. Verify - Ensure all completed issues have test coverage
  2. Build - Build solution and check for compilation errors
  3. Test - Run xUnit unit tests and bUnit component tests
  4. Integration - Run API integration tests (optional with -Integration flag)
  5. Report - Generate test coverage and results summary

Usage:
    .\test-runner.ps1                    # Standard pipeline (lint→verify→build→test)
...
Param(
    [switch]$Integration,     # Include integration tests requiring database
    [switch]$SkipLint,       # Skip HTML linting phase
...
try {
    ...
    if (-not $SkipLint) {
        ...
        # Lint implementation tracker HTML (if exists)
    ...
    Write-TestError "Test pipeline failed: $($_.Exception.Message)"    # Mark current phase as failed
