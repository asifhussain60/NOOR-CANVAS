...
test.describe('Infrastructure Validation Test', () => {
    test('resilient server connectivity test with retry logic', async ({ page }) => {
        ...
        for (attempts = 1; attempts <= maxAttempts; attempts++) {
            try {
                console.log(`📍 Attempt ${attempts}/${maxAttempts}: Testing server connectivity...`);
                ...
                if (healthResponse?.ok()) {
                    ...
                    const landingResponse = await page.goto(`${baseUrl}/user/landing/TEST123A`, {
                        timeout: 8000,
                    ...
                    if (landingResponse?.ok()) {
                        ...
                        console.log(`🎯 SUCCESS: Server is stable and responding on attempt ${attempts}`);
        ...
        console.log(`💥 All ${maxAttempts} attempts failed. Server appears to be unstable.`);
        ...
        console.log(`   • Recommended next steps: Review service registration duplication`);
    ...
    test('document infrastructure fixes completion', async () => {
        ...
        console.log('   • Kestrel connection limits configured (100 concurrent connections)');
        ...
        console.log('   • Duplicate logging messages (indicates config duplication)');
