...
test.describe('Complete User Experience - Multi-User SignalR Validation', () => {
    ...
    test.beforeAll(async ({ browser }) => {
        ...
        for (let i = 0; i < testUsers.length; i++) {
            ...
            console.log(`Created browser context ${i + 1} for user: ${testUsers[i].name}`);
        }
    });
    ...
    test('should register 5 users and validate real-time SignalR participant updates', async () => {
        const registrationResults: { userName: string; success: boolean; waitingRoomLoaded: boolean }[] = [];
        ...
        for (let i = 0; i < testUsers.length; i++) {
            ...
            try {
                ...
                await page.waitForSelector('[data-testid="session-name"]', { timeout: 15000 });
                ...
                console.log(`User ${i + 1}: Form filled with name="${user.name}", email="${user.email}", country="${user.country}"`);
                ...
                await page.waitForSelector('[data-testid="countdown-display"]', { timeout: 15000 });
                ...
                if (i < testUsers.length - 1) {
                    console.log(`User ${i + 1}: Waiting 2 seconds before next registration...`);
        ...
        const successfulUsers = registrationResults.filter(r => r.success && r.waitingRoomLoaded);
        expect(successfulUsers.length).toBeGreaterThan(0); // At least some users should succeed

        console.log(`Successfully registered users: ${successfulUsers.length}/${testUsers.length}`);
        ...
        for (let i = 0; i < pages.length; i++) {
            ...
            if (!registrationResults[i].success) {
                console.log(`Skipping participant validation for failed user: ${user.name}`);
                continue;
            }
            ...
            try {
                ...
                const participantElements = await page.$$('[data-testid="participants-list"] > div:not(:has-text("No participants"))');
                console.log(`User ${user.name}: Found ${participantElements.length} participants in list`);
                ...
                for (const element of participantElements) {
                    ...
                    const flagImg = await element.$('img[data-testid="participant-flag"]');
                ...
                const hasCurrentUser = participantData.some(p => p.name.includes(user.name));
                ...
                if (successfulUsers.length > 1) {
                    ...
                    console.log(`User ${user.name}: Can see ${participantData.length} total participants`);
                }

            } catch (error) {
                console.error(`Error validating participants for user ${user.name}:`, error);
            }
        }
        ...
        for (let i = 0; i < pages.length; i++) {
            ...
            try {
                ...
                const dateElement = await page.locator('div:has(.fa-calendar-days) + div p:last-child');
                ...
                const isCurrentDate = dateText?.includes('Today') || dateText?.includes(currentMonth);
                expect(isCurrentDate).toBeTruthy();

                // 2. Validate timer is running (not stuck at 00:00)
                const timerElement = await page.locator('[data-testid="countdown-display"]');
                ...
                console.log(`User ${user.name}: Timer after 2 seconds "${updatedTime}"`);

                // Timer should either be counting down or show session started
                const timerWorking = (initialTime !== updatedTime) ||
                ...
                const sessionNameElement = await page.locator('h2:has-text("Loading")').first();
                const sessionNameCount = await sessionNameElement.count();
                expect(sessionNameCount).toBe(0); // No "Loading..." text should remain

                console.log(`User ${user.name}: ✅ All UI elements validated successfully`);
        ...
        for (let i = 0; i < pages.length; i++) {
            ...
            try {
                ...
                console.log(`User ${user.name}: Sees ${participantElements.length} participants`);
            } catch (error) {
                console.error(`Error checking participant count for ${user.name}:`, error);
            }
        }
        ...
        if (participantCounts.length > 1) {
            ...
            console.log(`Participant count range: ${minCount}-${maxCount} (difference: ${countDifference})`);
        ...
        console.log('\n=== TEST COMPLETE ===');
        console.log(`✅ Successfully tested ${successfulUsers.length}/${testUsers.length} users`);
    ...
    test('should validate session token reuse works for all users', async () => {
        ...
        for (let i = 0; i < Math.min(3, pages.length); i++) { // Test with first 3 users for speed
            const page = pages[i];
            const user = testUsers[i];
            ...
            try {
                await page.waitForSelector('[data-testid="countdown-display"]', { timeout: 10000 });
