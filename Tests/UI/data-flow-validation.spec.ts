...
test.describe('Data Flow Validation - UserLanding to SessionWaiting', () => {
    ...
    test('Should validate complete data flow: URL token → API → UserLanding → SessionWaiting', async ({ page }) => {
        console.log('🔄 Starting comprehensive data flow validation test...');

        // Step 1: Navigate to UserLanding with real KSESSIONS token
        console.log(`📍 Step 1: Navigating to UserLanding with token ${TEST_CONFIG.tokens.valid}`);
        await page.goto(`${TEST_CONFIG.baseUrl}/user/landing/${TEST_CONFIG.tokens.valid}`, {
        ...
        await page.waitForSelector(sessionNameSelector, { timeout: TEST_CONFIG.timeout });
        ...
        console.log(`🔍 Expected: "${TEST_CONFIG.expectedData.sessionTitle}", Got: "${sessionName}"`);

        // Step 3: Verify registration form is visible (token validation successful)
        console.log('📍 Step 3: Verifying registration form appears after token validation');

        const registrationFormSelector = 'div:has-text("REGISTRATION")';
        await expect(page.locator(registrationFormSelector)).toBeVisible({ timeout: TEST_CONFIG.timeout });
        ...
        await page.waitForURL(/\/session\/waiting\/.*/, { timeout: TEST_CONFIG.timeout });
        console.log(`✅ Successfully navigated to SessionWaiting: ${page.url()}`);

        // Step 6: Validate SessionWaiting displays real KSESSIONS data
        console.log('📍 Step 6: Validating SessionWaiting displays real KSESSIONS data');

        // Wait for session data to load in waiting room
        await page.waitForSelector('h1:has-text("Waiting Room")', { timeout: TEST_CONFIG.timeout });

        // Verify session title appears correctly
        const waitingRoomSessionName = await page.textContent('h2, h3, .session-title, [class*="session"], [class*="title"]');
        ...
        const instructorElements = await page.locator('text=/instructor|teacher|speaker/i').all();
        ...
        for (const element of instructorElements) {
            ...
            if (text && text.includes(TEST_CONFIG.expectedData.instructorName)) {
                ...
                console.log(`✅ Found expected instructor: ${TEST_CONFIG.expectedData.instructorName}`);
                break;
            }
        }

        // Verify session timing information appears
        const dateElements = await page.locator('text=/date|time|duration|start/i').all();
        let timingDataFound = false;

        for (const element of dateElements) {
            const text = await element.textContent();
            console.log(`🔍 Timing element text: "${text}"`);
            if (text && !text.includes('TBD') && !text.includes('Loading') && text.length > 3) {
        ...
        const participantsList = page.locator('[class*="participant"], [data-testid*="participant"]');
        ...
        console.log('🎉 Comprehensive data flow validation completed successfully!');
    });
    ...
    test('Should validate API response format matches DTO expectations', async ({ page }) => {
        console.log('🔄 Testing API response format validation...');

        // Intercept API call to validate response structure
        const apiResponsePromise = page.waitForResponse(
            response => response.url().includes('/api/participant/session/') && response.url().includes('/validate')
        );
...
test.describe('Performance and Reliability Checks', () => {
    ...
    test('Should load UserLanding within acceptable time limits', async ({ page }) => {
