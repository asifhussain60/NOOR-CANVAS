...
test.describe('Complete User Journey E2E', () => {
    ...
    test('Complete User Registration Journey', async () => {
        ...
        const registerButton = page.locator('button:has-text("Join Waiting Room")').or(
        ...
        await Promise.race([
            ...
            page.waitForSelector('[data-testid="participants-list"], [data-testid="countdown-display"]', { timeout: 10000 })
        ]);
        ...
        const frankInList = await page.locator('text=Frank Adams').isVisible({ timeout: 5000 });

        if (frankInList) {
            console.log('✅ SUCCESS: Frank Adams found in participants list!');
        } else {
            // If not visible, let's debug what's happening
            console.log('⚠️  Frank Adams not immediately visible, checking page content...');

            // Get page content for debugging
            const pageContent = await page.content();
            const hasParticipantSection = pageContent.includes('participant') || pageContent.includes('waiting');
            ...
            const participantElements = await page.locator('.participant, [data-participant], .user-list li').all();
            console.log(`📊 Found ${participantElements.length} potential participant elements`);
        ...
        await expect(page.locator('text=Frank Adams')).toBeVisible({ timeout: 10000 });
        console.log('🎉 COMPLETE SUCCESS: Frank Adams registration and display verified!');
    });
    ...
    test('Verify Registration API Integration', async () => {
        ...
        const response = await request.post('/api/participant/register-with-token', {
        ...
        const participantsResponse = await request.get(`/api/participant/session/${testTokenData.userToken}/participants`);
    ...
    test('Verify Waiting Room Loads Participants Correctly', async () => {
        ...
        const participantCount = await page.locator('text=/Participants \\(\\d+\\)/').textContent().catch(() => null);
        console.log('📊 Participant count display:', participantCount);

        // Verify at least one participant is shown in the participants list
        const participants = await page.locator('[data-testid="participants-list"] > div').all();
        console.log(`📊 Found ${participants.length} participant elements in waiting room`);

        expect(participants.length).toBeGreaterThan(0);

        // Check for our test participant
        const testUserVisible = await page.locator('text=Test User API').isVisible();
        if (testUserVisible) {
...
test.describe('User Journey Edge Cases', () => {
    test('Handle Invalid Tokens Gracefully', async ({ page }) => {
        ...
        const hasError = await page.locator('[data-testid="error-message"]').isVisible({ timeout: 5000 });
    ...
    test('Handle Empty Sessions Correctly', async ({ page, request }) => {
        ...
        const noParticipantsMessage = await page.locator('text=/No participants yet|first|waiting for participants/i').isVisible({ timeout: 5000 });
        const participantCount = await page.locator('text=/Participants \\(\\d+\\)/').textContent().catch(() => '(0)');
        ...
        expect(noParticipantsMessage || (participantCount && participantCount.includes('(0)'))).toBeTruthy();
