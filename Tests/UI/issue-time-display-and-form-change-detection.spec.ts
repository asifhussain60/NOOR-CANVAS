...
test.describe('NOOR Canvas - Time Display and Form Change Detection Issues', () => {
    ...
    test.describe('Issue 1: SessionWaiting.razor Time Display', () => {
        ...
        test('should display correct countdown for future scheduled session', async ({ page }) => {
            ...
            const countdownText = await page.locator('[data-testid="countdown-display"]').textContent();
            ...
            const startTimeElement = page.locator('text=Start Time').locator('..').locator('p').nth(1);
        ...
        test('should update countdown dynamically every second', async ({ page }) => {
            await page.goto('/session/waiting/E5R7TLXK');
        ...
        test('should show appropriate status messages based on time remaining', async ({ page }) => {
            await page.goto('/session/waiting/E5R7TLXK');
            await page.waitForSelector('[data-testid="countdown-display"]');

            // Get the status message
            const statusMessage = await page.locator('text=Waiting for host to begin').or(page.locator('text=Starting soon')).or(page.locator('text=Starting very soon')).first().textContent();

            // Verify we have a meaningful status message
            expect(statusMessage).toBeTruthy();
            expect(['Waiting for host to begin the session.', 'Waiting for session to begin.', 'Starting soon...', 'Starting very soon...']).toContain(statusMessage);
    ...
    test.describe('Issue 2: Host-SessionOpener.razor Form Change Detection', () => {
        ...
        test('should hide session URL panel when form fields change', async ({ page }) => {
            ...
            let sessionUrlVisible = await page.locator('text=Session URL:').isVisible();
        ...
        test('should invalidate session URL when date changes', async ({ page }) => {
            await page.goto('/host/session-opener/ADMIN123');
            ...
            await page.fill('#session-date', dayAfterTomorrow.toISOString().split('T')[0]);

            // Verify panel is hidden (URL invalidated)
            const sessionUrlVisible = await page.locator('text=Session URL:').isVisible();
            expect(sessionUrlVisible).toBe(false);

            console.log(`✓ Session URL invalidated after date change: ${originalUrl}`);
        });

        test('should invalidate session URL when time changes', async ({ page }) => {
            await page.goto('/host/session-opener/ADMIN123');
        ...
        test('should invalidate session URL when duration changes', async ({ page }) => {
            await page.goto('/host/session-opener/ADMIN123');
        ...
        test('should allow creating new session URL after form changes', async ({ page }) => {
            await page.goto('/host/session-opener/ADMIN123');
    ...
    test.describe('Integration Tests: Both Issues Combined', () => {
        ...
        test('should create session with correct time and verify waiting room displays proper countdown', async ({ page, browser }) => {
            ...
            const tokenMatch = sessionUrl?.match(/\/user\/userlanding\/([A-Z0-9]{8})/);
            ...
            console.log(`✓ Waiting room shows correct countdown: ${countdownText} (${totalSeconds} seconds)`);
        });
    });
});