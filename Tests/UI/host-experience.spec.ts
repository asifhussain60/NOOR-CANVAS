...
test.describe('Host Experience Test', () => {
    ...
    test('Complete Host Experience - Session Creation to Waiting Room', async ({ page, request }) => {
        ...
        try {
            ...
            const openSessionBtn = page.locator('#openSessionBtn, button:has-text("Open Session")');
            ...
            page.on('response', async (response) => {
                if (response.url().includes('/api/host/create-session') || response.url().includes('session')) {
                    try {
                        const responseText = await response.text();
                        console.log('📊 Session API response:', responseText.substring(0, 200));
            ...
            await page.screenshot({ path: './screenshots/after-session-creation.png', fullPage: true });
            ...
            for (const selector of sessionUrlPanelSelectors) {
                ...
                if (count > 0) {
                    console.log(`✅ Found session URL panel with selector: ${selector} (${count} elements)`);
            ...
            const userLandingPattern = /https?:\/\/localhost:9091\/user\/landing\/([A-Z0-9]{8})/i;
            ...
            if (!participantToken) {
                ...
                const waitingRoomBtn = page.locator('button:has-text("Open Waiting Room"), a:has-text("Open Waiting Room"), [data-testid="open-waiting-room"]');
                const waitingRoomCount = await waitingRoomBtn.count();

                if (waitingRoomCount > 0) {
                    console.log(`✅ Found "Open Waiting Room" button/link (${waitingRoomCount} elements)`);
                    ...
                    for (let i = 0; i < waitingRoomCount; i++) {
                        ...
                        console.log(`   Element ${i}: href="${href}", onclick="${onclick}"`);

                        // Extract token from href or onclick
                        const tokenMatch = (href + onclick).match(/([A-Z0-9]{8})/);
                        if (tokenMatch) {
                            participantToken = tokenMatch[1];
                            sessionUrl = href.startsWith('http') ? href : `${BASE_URL}/user/landing/${participantToken}`;
                            console.log(`✅ Extracted token from waiting room link: ${participantToken}`);
                            break;
                        }
                    }
                }
            ...
            if (participantToken && sessionUrl) {
                ...
                const waitingRoomBtn = page.locator('button:has-text("Open Waiting Room"), a:has-text("Open Waiting Room")');
                ...
                if (hasWaitingRoomBtn) {
                    ...
                    console.log('🔗 No "Open Waiting Room" button found - navigating directly to session URL...');
                ...
                await page.screenshot({ path: './screenshots/waiting-room-result.png', fullPage: true });

                const pageText = await page.textContent('body');
                const hasSessionError = pageText?.includes('Session Error') || pageText?.includes('Session not found') || pageText?.includes('Invalid') || pageText?.includes('Expired');
                const hasWaitingRoom = pageText?.includes('Waiting Room') || pageText?.includes('Session begins') || pageText?.includes('Participants');

                if (hasSessionError) {
                    console.log('🎯 ISSUE REPRODUCED: Session Error found in waiting room!');
                    console.log('📄 Error content:', pageText?.substring(0, 800));

                    // This confirms the user's reported issue
                    console.log('✅ CONFIRMED: Host experience workflow generates session URL successfully');
                    console.log('❌ CONFIRMED: Participant token leads to "Session Error" in waiting room');
                    console.log('🔍 ROOT CAUSE: Session creation appears successful but participant access fails');

                } else if (hasWaitingRoom) {
                    console.log('🎉 SUCCESS: Waiting room loaded correctly - no session error!');
                ...
                console.log(`🏁 Session Error Found: ${hasSessionError ? 'YES ❌' : 'NO ✅'}`);
                ...
                console.log(`   ${hasSessionError ? '❌' : '✅'} Waiting room access: ${hasSessionError ? 'FAILS with Session Error' : 'WORKING'}`);
                console.log('');
                console.log(`🏆 HOST EXPERIENCE TEST: ${hasSessionError ? 'ISSUE CONFIRMED' : 'PASSED'}`);

            } else {
                console.log('❌ CRITICAL: No participant token found after session creation');
                ...
                console.log('   ✅ Cascading dropdown selection: WORKING');
                console.log('   ✅ Form completion (date/time/duration): WORKING');
                console.log('   ✅ Session creation button: WORKING (enabled and clickable)');
                console.log('   ❌ Session URL generation: FAILING (no participant token generated)');
                console.log('   ❓ Waiting room access: CANNOT TEST (no session URL)');
                console.log('');
                console.log('🏆 HOST EXPERIENCE TEST: SESSION CREATION FAILURE DETECTED');

                throw new Error('Session creation failed - no participant token generated');
            }

        } catch (error) {
            console.error('❌ Host Experience Test failed:', error);
            await page.screenshot({ path: './screenshots/host-experience-error.png', fullPage: true });
            throw error;
        }
    });
    ...
    test('Quick Session Error Validation', async ({ page }) => {
        ...
        const pageText = await page.textContent('body');
        const hasError = pageText?.includes('Session Error') || pageText?.includes('not found');

        console.log(`✅ Invalid token correctly shows error: ${hasError ? 'YES' : 'NO'}`);

        await page.screenshot({ path: './screenshots/invalid-token-test.png', fullPage: true });
