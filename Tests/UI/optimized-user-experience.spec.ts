        ...
        try {
            ...
            const registrationPromises = pages.map(async (page, index) => {
                try {
                    ...
                    const hasRegistrationForm = await page.isVisible('input[placeholder*="name"], input[id*="name"], [data-testid*="name"]');
                    console.log(`📝 User ${index + 1}: Registration form visible: ${hasRegistrationForm}`);

                    if (hasRegistrationForm) {
                        // Fill registration form
                        await page.fill('input[placeholder*="name"], input[id*="name"], [data-testid*="name"]', `Test User ${index + 1}`);
                    ...
                    console.error(`❌ User ${index + 1}: Registration failed:`, errorMessage);
                    return {
                        success: false,
                        userId: index + 1,
                        error: errorMessage,
            ...
            results.forEach(result => {
                if (result.success) {
                    console.log(`✅ User ${result.userId}: SUCCESS - Final URL: ${result.finalUrl}`);
                } else {
                    console.log(`❌ User ${result.userId}: FAILED - Error: ${result.error}`);
                }
            });
