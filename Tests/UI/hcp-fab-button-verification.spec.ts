...
test.describe('HCP FAB Button Verification', () => {
    ...
    test('FAB button appears and broadcasts transcript successfully', async ({ page }) => {
        ...
        // Step 2: Click "Transcript Canvas" button (ROBUST ID-based selector)
        ...
        // Step 3: Click "Start Session" button to trigger transcript render (ROBUST ID-based selector)
    ...
    test('FAB button hidden when not in broadcast mode', async ({ page }) => {
        ...
        // Select Asset Canvas (NOT Transcript Canvas) - ROBUST ID-based selector
        console.log('📍 Selecting Asset Canvas (not transcript)...');
        const assetCanvasButton = page.locator('#reg-asset-canvas-btn');
