...
test.describe('Host Authentication with Friendly Token URL Flow', () => {
    ...
    test('should authenticate host and load Host-SessionOpener with friendly token in URL', async ({ page }: { page: Page }) => {
        console.log('=== Host Authentication Flow: Loading with Friendly Token ===');
        ...
        try {
            ...
            const brandingElement = page.locator('[data-testid="host-branding"], .host-branding, .canvas-branding');
            if (await brandingElement.isVisible({ timeout: 5000 }).catch(() => false)) {
            ...
            // Album dropdown
            const albumDropdown = page.locator('select[name*="album"], select[id*="album"], #SelectedAlbum, [data-testid="album-select"]');
            if (await albumDropdown.isVisible({ timeout: 10000 }).catch(() => false)) {
                ...
                if (albumOptions > 1) { // More than just the default "Select..." option
                    console.log('✅ Albums loaded successfully');
                }
            } else {
                console.log('⚠️ Album dropdown not found - checking alternative selectors');
            }

            // Category dropdown
            const categoryDropdown = page.locator('select[name*="category"], select[id*="category"], #SelectedCategory, [data-testid="category-select"]');
            if (await categoryDropdown.isVisible({ timeout: 5000 }).catch(() => false)) {
                console.log('✅ Category dropdown found');
            }

            // Session dropdown
            const sessionDropdown = page.locator('select[name*="session"], select[id*="session"], #SelectedSession, [data-testid="session-select"]');
            if (await sessionDropdown.isVisible({ timeout: 5000 }).catch(() => false)) {
                console.log('✅ Session dropdown found');
            }

            // Step 5: Check for any error messages that would indicate authentication failure
            const errorElements = page.locator('.alert-danger, .error, .validation-error, [data-testid="error"]');
    ...
    test('should verify host token authentication via API before UI navigation', async ({ request }) => {
        ...
        try {
            ...
            const albumsResponse = await request.get(`/api/host/albums?guid=${TEST_HOST_TOKEN}`);

            console.log(`Albums API Response Status: ${albumsResponse.status()}`);

            if (albumsResponse.ok()) {
                const albums = await albumsResponse.json();
                console.log(`✅ Host token authenticated successfully - ${albums.length} albums loaded`);

                // Log first few albums for verification
                if (albums.length > 0) {
                    console.log('Sample albums:');
                    albums.slice(0, 3).forEach((album: any, index: number) => {
                        console.log(`  ${index + 1}. ${album.GroupName} (ID: ${album.GroupId})`);
                    });
                }
    ...
    test('should perform complete host authentication and cascading dropdown interaction', async ({ page }) => {
        console.log('=== Complete Host Authentication + Cascading Dropdown Flow ===');
        ...
        try {
            ...
            const albumSelect = page.locator('select[name*="album"], #SelectedAlbum').first();
            ...
            const categorySelect = page.locator('select[name*="category"], #SelectedCategory').first();
            ...
            const sessionSelect = page.locator('select[name*="session"], #SelectedSession').first();
            ...
            const sessionDate = page.locator('input[name*="date"], #SessionDate').first();
            if (await sessionDate.isVisible().catch(() => false)) {
                await sessionDate.fill('2025-09-18');
                console.log('✅ Set session date');
            }

            const sessionTime = page.locator('input[name*="time"], #SessionTime').first();
            ...
            console.log('✅ Complete host authentication and interaction flow successful');
            ...
            console.log('⚠️ UI interaction had issues but host authentication succeeded');
        }
    });
});