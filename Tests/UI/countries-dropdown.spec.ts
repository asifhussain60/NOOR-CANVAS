...
test.describe('Countries Dropdown Implementation', () => {
    ...
    test('should load countries from API endpoint', async ({ request }: { request: APIRequestContext }) => {
        console.log('Testing countries API endpoint...');

        // Test the countries API endpoint directly
        const response = await request.get(`/api/host/countries?guid=${hostToken}`, {
            headers: {
        ...
        if (countries.length > 0) {
            ...
            console.log(`Sample country: ${firstCountry.CountryName} (${firstCountry.ISO2})`);
        }
    ...
    test('should display countries dropdown in UI', async ({ page }: { page: Page }) => {
        ...
        await expect(defaultOption).toHaveText(/Select Country|Loading countries.../);
        ...
        const options = await countrySelect.locator('option:not(:first-child)').allTextContents();
    ...
    test('should handle countries loading states properly', async ({ page }: { page: Page }) => {
    ...
    test('should integrate with form validation', async ({ page }: { page: Page }) => {
        ...
        console.log(`Open Session button enabled with no country: ${isButtonEnabled}`);
    ...
    test('should include country in session creation data', async ({ page }: { page: Page }) => {
        ...
        const sessionCreationRequests: { request: Request; postData: string | null }[] = [];
        page.on('request', (req: Request) => {
        ...
        if (await openSessionButton.isEnabled()) {
            ...
            const lastRequest = sessionCreationRequests[sessionCreationRequests.length - 1];
            if (lastRequest.postData) {
                const sessionData: SessionDataWithCountry = JSON.parse(lastRequest.postData);
                ...
                console.log('Session creation data includes country:', sessionData.SelectedCountry);
            ...
            console.log('Open Session button is disabled - checking validation state');

            // Log current form state for debugging
            const albumValue = await page.locator('select#album-select').inputValue();
            const categoryValue = await page.locator('select#category-select').inputValue();
            const sessionValue = await page.locator('select#session-select').inputValue();
            const countryValue = await countrySelect.inputValue();
            const dateValue = await page.locator('input[type="date"]').inputValue();
            const timeValue = await page.locator('input[placeholder*="time"]').inputValue();
    ...
    test('should handle countries API error gracefully', async ({ page }: { page: Page }) => {
        ...
        const errorMessage = page.locator('text=/Failed to load countries|Error loading countries/');
        if (await errorMessage.isVisible()) {
            console.log('Error message displayed correctly');
        }
    });

    test('should display country name correctly in status section', async ({ page }: { page: Page }) => {
        ...
        const selectedOptionText = await countrySelect.locator('option:checked').textContent();
        const selectedCountryId = await countrySelect.inputValue();

        console.log(`Selected country: ${selectedOptionText} (ID: ${selectedCountryId})`);
