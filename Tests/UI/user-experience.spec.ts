...
test.describe('User Experience - Complete Waiting Room Journey', () => {
    ...
    test('should display correct current date format and working timer', async () => {
        ...
        await page.waitForSelector('button[data-testid="join-session-button"]', { timeout: 10000 });
        ...
        const sessionDateText = await page.textContent('.fa-calendar-days').then(() =>
            page.locator('div:has(.fa-calendar-days) + div p:last-child').textContent()
        );
    ...
    test('should have functional countdown timer that updates', async () => {
        ...
        console.log(`✅ Timer functionality validated: ${initialTime} → ${updatedTime}`);
    });
    ...
    test('should display complete participant names without truncation', async () => {
        ...
        await page.waitForSelector('[data-testid="participants-list"]', { timeout: 15000 });

        // Wait a moment for participant data to populate via API
        await page.waitForTimeout(2000);

        // Get all participant names
        const participantNames = await page.locator('[data-testid="participants-list"] span').allTextContents();
        ...
        const fullNameFound = participantNames.some(name => name.includes('Asif Hussain'));
        ...
        console.log(`✅ Participant names validation passed: Full names displayed correctly`);
    });
    ...
    test('should display all required session information correctly', async () => {
        ...
        const instructorElement = await page.locator('div:has(.fa-user-tie) + div p:last-child').textContent();
        ...
        const startTimeElement = await page.locator('div:has(.fa-clock) + div p:last-child').textContent();
        ...
        const durationElement = await page.locator('div:has(.fa-hourglass-half) + div p:last-child').textContent();
    ...
    test('should show proper waiting room status and messaging', async () => {
        ...
        const hasWaitingMessage = statusMessages.some(msg =>
        ...
        const countdownTime = await page.textContent('[data-testid="countdown-display"]');
        if (countdownTime === '00:00') {
            const sessionStartedMessage = statusMessages.some(msg => msg.includes('Session is starting now!'));
    ...
    test('should have responsive UI elements and proper styling', async () => {
        ...
        await expect(page.locator('button:has-text("Sort by Country")')).toBeVisible();
