...
test.describe("Phase 1: ParticipantController Session Timing Fix", () => {
  test("should retrieve session timing from KSESSIONS database", async ({
    page,
  }) => {
    ...
    await page.waitForSelector('[data-testid="session-waiting"]', {
      timeout: 10000,
    });

    // Get the session data via API call to verify backend behavior
    const response = await page.request.get(
      `http://localhost:9090/api/participant/validate/${testToken}`,
    );
    ...
    const fiveMinutesAfterCreated = new Date(
    ...
    const timeDifference = Math.abs(
      startTime.getTime() - fiveMinutesAfterCreated.getTime(),
    );
    ...
    if (sessionData.session.InstructorName) {
      ...
      console.log(
        "Instructor from KSESSIONS:",
        sessionData.session.InstructorName,
      );
    }
    ...
    if (sessionData.session.StartedAt) {
      ...
      expect(Math.abs(startedAt.getTime() - startTime.getTime())).toBeLessThan(
        60000,
      ); // Within 1 minute
    }
  });

  test("should handle fallback when KSESSIONS data is unavailable", async ({
    page,
  }) => {
    ...
    if (response.status() === 200) {
      ...
      const timeDifference = Math.abs(
        startTime.getTime() - expectedFallback.getTime(),
      );
      expect(timeDifference).toBeLessThan(5000); // Within 5 seconds of expected fallback

      console.log(
        "Fallback timing verified - StartTime matches CreatedAt + 5 minutes",
      );
  ...
  test("should display session information correctly in UI", async ({
    page,
  }) => {
    ...
    const sessionTitle = await page
      .locator('.session-title, [data-testid="session-title"]')
      .first();
    ...
    const instructorInfo = await page
      .locator('.instructor-info, [data-testid="instructor-name"]')
      .first();
    ...
    const timingElements = await page
      .locator('[data-testid*="time"], .time-display, .countdown')
      .all();
