...
test.describe('Issue-122: Host Control Panel Button', () => {
    ...
    test('Load control panel button routes correctly to host control panel', async ({ page }) => {
        // Navigate to host session opener
        await page.goto('http://localhost:9091/host');
        ...
        await expect(page.locator('h1, h2, h3')).toContainText(/Control Panel|Session Management/i);
    });

    test('Button remains disabled when no session is created', async ({ page }) => {
        // Navigate to host session opener
        await page.goto('http://localhost:9091/host');
        ...
        if (!isDisabled) {
            const opacity = await button.evaluate((btn) => getComputedStyle(btn).opacity);
            const pointerEvents = await button.evaluate((btn) => getComputedStyle(btn).pointerEvents);
    ...
    test('Button icon change from hourglass to gear', async ({ page }) => {
        ...
        const hourglassIcon = page.locator('#loadControlPanelBtn i.fa-hourglass-half');
    ...
    test('Control panel loads with session data after routing', async ({ page }) => {
        // Navigate to host session opener
        await page.goto('http://localhost:9091/host');
        ...
        const hasControlPanelContent = pageContent && (
