...
test.describe('Asset Sharing via Simulation Harness', () => {
    ...
    test.beforeAll(async ({ browser }) => {
        ...
        page.on('console', msg => {
            const text = msg.text();
            if (text.includes('NOOR-TEST') || text.includes('NOOR-HUB-SHARE') ||
                text.includes('NOOR-CANVAS-SHARE') || text.includes('ShareAsset') ||
                text.includes('AssetShared') || text.includes('HOST') ||
    ...
    test('should simulate asset sharing workflow', async () => {
        ...
        await page.evaluate(() => {
            ...
            testButton.onclick = () => {
                ...
                console.log('[SIMULATION] NOOR-HUB-SHARE: ShareAsset method called with sessionId=SIMULATION');
