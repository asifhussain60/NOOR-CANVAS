...
**Version**: 2.0.0  
**Purpose:** Parse user invocation to extract target, request, and parameters (CASE-INSENSITIVE)

**Used by:** route.prompt.md (Step -1)

---

## Key Feature: Case-Insensitive Parameters

**All parameter names are case-insensitive**:
- `key:`, `Key:`, `KEY:` → normalized to `key:`
- `target=`, `Target=`, `TARGET=` → normalized to `target=`
- `auto-execute=`, `Auto-Execute=`, `AUTO-EXECUTE=` → normalized to `auto-execute=`
...
5. **Case-insensitive:** `/route Key: my-key` or `/route KEY: my-key` (both work)

---

## Parsing Rules

**Step 0: Normalize Parameter Names (NEW)**
- Replace all variations of parameter names with lowercase:
  - `/\bKey:\s*/gi` → `key: `
  - `/\bKEY:\s*/gi` → `key: `
  - `/\bTarget=/gi` → `target=`
  - `/\bTARGET=/gi` → `target=`
  - `/\bAuto-Execute=/gi` → `auto-execute=`
...
- If contains `=` or `:` → named parameters
...
**Valid targets:**
- plan, task, todo, test, ask, healthcheck, drift, cohesion, test-prep

**Step 4: Extract request**
- Positional: text in quotes or after target
- Named: value after `request=`
- Default: entire input after `/route` and parameters

**Step 5: Extract optional parameters**
- key: `key=value` OR `key: value` (case-insensitive)
