...
**Requirements:**
- ✅ ZERO Roslynator diagnostics (errors or warnings)
...
**Accepted Baseline:**
- 39 ESLint errors in SignalR globals, Playwright contexts, fixtures, and catch patterns
...
- Controllers (any file in `SPA/NoorCanvas/Controllers/`)
- DTOs (any DTO, request, or response model)
- API endpoints (additions, modifications, or removals)
- SignalR hubs (any file in `SPA/NoorCanvas/Hubs/`)
...
3. **Playwright Tests** (if UI changes)
...
```bash
# Playwright tests
npx playwright test [test-file-pattern] --headed
npx playwright test [test-file-pattern] --project=chromium
npx playwright test [test-file-pattern] --project=firefox
npx playwright test [test-file-pattern] --project=webkit
```

**Requirements:**
- ✅ All relevant unit tests pass
- ✅ All relevant integration tests pass
- ✅ All relevant Playwright tests pass across all browsers
...
✅ **Analyzer Clean:** All Roslynator and .NET analyzers pass  
...
- [NOOR-CANVAS_ARCHITECTURE.MD](NOOR-CANVAS_ARCHITECTURE.MD) - System architecture
