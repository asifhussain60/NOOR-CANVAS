# Handoff Protocol Update - feature planning agent Automatic Execution
...
Updated the `feature.prompt.md` to clarify that the feature planning agent must **actively execute** the handoff command, not just document it.
...
2. feature planning agent writes all required files ({key}.plan.md, {key}.plan.json, work-log.md)
3. **feature planning agent MUST send a new message containing the @workspace /task command**
...
1. **feature planning agent behavior:** Must actively send the @workspace /task command
2. **User experience:** No manual copy/paste required
3. **Workflow:** Plan → Approve → **Automatic execution begins**

### What This Doesn't Change

- feature planning agent still doesn't modify source files directly
- feature planning agent still doesn't run builds/tests directly
...
❌ **Mistake 1:** feature planning agent only documents the command but doesn't execute it
- User sees plan files created but no task agent activation
- User has to manually copy/paste the command

❌ **Mistake 2:** feature planning agent shows the command to user and asks them to run it
- Defeats the purpose of automatic handoff
- Adds unnecessary friction

❌ **Mistake 3:** feature planning agent stops after writing files
...
- ✅ feature planning agent handles handoff automatically
...
- `.github/prompts/feature.prompt.md` - Updated with execution protocol
...
1. ✅ feature planning agent now executes handoff automatically
