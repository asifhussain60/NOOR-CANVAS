...
test.describe('Issue-129 & Issue-130: Multi-User Participant List Testing', () => {
    ...
    const testUsers = [
        { name: 'Ahmed Al-Rashid', email: 'ahmed@test.com', country: 'Saudi Arabia', countryCode: 'sa' },
        { name: 'Fatima Khan', email: 'fatima@test.com', country: 'Pakistan', countryCode: 'pk' },
        { name: 'Omar Hassan', email: 'omar@test.com', country: 'Egypt', countryCode: 'eg' },
        { name: 'Aisha Mahmoud', email: 'aisha@test.com', country: 'Morocco', countryCode: 'ma' },
        { name: 'Yusuf Ibrahim', email: 'yusuf@test.com', country: 'Turkey', countryCode: 'tr' }
    ];
    ...
    test('Issue-130: Multi-user participant registration and list addition', async ({ browser }) => {
        ...
        try {
            ...
            console.log(`Created ${testUsers.length} browser contexts for multi-user testing`);
            ...
            for (let i = 0; i < testUsers.length; i++) {
                ...
                await page.goto(`http://localhost:9090/user/landing/${testSessionToken}`);

                // Wait for page to load
                await expect(page.getByText('Join Session')).toBeVisible({ timeout: 15000 });
                ...
                const participantCountText = await page.textContent('h2:has-text("Participants")');
                const currentCount = parseInt(participantCountText?.match(/\d+/)?.[0] || '0');
                ...
                const flagImg = page.locator(`img[alt*="${user.countryCode}"], img[src*="${user.countryCode}"]`);
                await expect(flagImg.or(page.locator('img[data-testid="participant-flag"]'))).toBeVisible();
            ...
            // Verify total count
            const finalCountText = await finalPage.textContent('h2:has-text("Participants")');
            const finalCount = parseInt(finalCountText?.match(/\d+/)?.[0] || '0');
            expect(finalCount).toBe(testUsers.length);

            console.log(`✅ Final participant count: ${finalCount}/${testUsers.length}`);
    ...
    test('Issue-129: Session countdown display with future start time', async ({ page }) => {
        // Navigate to waiting room
        await page.goto(`http://localhost:9090/session/waiting/${testSessionToken}`);

        // Wait for page load
        await expect(page.getByText('Waiting Room')).toBeVisible({ timeout: 15000 });

        // With future start time, should show countdown
        const hasCountdown = await page.getByText('Session begins in:').isVisible().catch(() => false);
        const hasActiveSession = await page.getByText('Session is starting now!').isVisible().catch(() => false);
    ...
    test('Issue-130: Participant list sorting with multiple countries', async ({ page }) => {
        ...
        if (participantCount >= 2) {
            ...
            const namesSorted = await participantsList.locator('span').allTextContents();
            ...
            const countriesSorted = await participantsList.locator('span').allTextContents();
            ...
            console.log(`Only ${participantCount} participants found - sorting test requires 2+ participants`);
        }
    });

    test('Issue-129 & Issue-130: Real-time updates verification', async ({ browser }) => {
        ...
        try {
            ...
            await viewerPage.goto(`http://localhost:9090/session/waiting/${testSessionToken}`);
            await expect(viewerPage.getByText('Waiting Room')).toBeVisible({ timeout: 15000 });

            // Get initial participant count
            const initialCountText = await viewerPage.textContent('h2:has-text("Participants")');
            ...
            await registrantPage.goto(`http://localhost:9090/user/landing/${testSessionToken}`);
            await expect(registrantPage.getByText('Join Session')).toBeVisible({ timeout: 15000 });

            await registrantPage.fill('[name="Model.Name"]', newUser.name);
            await registrantPage.fill('[name="Model.Email"]', newUser.email);
            await registrantPage.selectOption('[name="Model.Country"]', newUser.country);
            await registrantPage.click('button[type="submit"]');

            await expect(registrantPage.getByText('Waiting Room')).toBeVisible({ timeout: 15000 });
            ...
            const updatedCountText = await viewerPage.textContent('h2:has-text("Participants")');
            ...
            await expect(viewerPage.getByText(newUser.name)).toBeVisible({ timeout: 5000 });
