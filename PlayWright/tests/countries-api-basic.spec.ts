...
test.describe("Countries API Direct Tests", () => {
  test("should respond to countries endpoint with generated token", async ({
    request,
  }: {
    request: APIRequestContext;
  }) => {
    console.log(
      "Testing countries API endpoint with generated authentication token...",
    );
    ...
    const response = await request.get(
      `/api/host/countries?guid=${tokenData.hostToken}`,
      {
        headers: {
    ...
    if (response.status() === 200) {
      ...
      if (countries.length > 0) {
        ...
        console.log(
          `Sample country: ${(firstCountry as any).countryName} (${(firstCountry as any).isO2})`,
        );
        console.log("Countries API endpoint is working correctly");
      } else {
        console.log(
          "Countries API returned empty array - this may be expected if no active countries",
        );
      }
    } else {
  ...
  test("should have functional albums endpoint for comparison", async ({
    request,
  }: {
    request: APIRequestContext;
  }) => {
    ...
    const response = await request.get(
      `/api/host/albums?guid=${tokenData.hostToken}`,
      {
        headers: {
          Accept: "application/json",
        },
