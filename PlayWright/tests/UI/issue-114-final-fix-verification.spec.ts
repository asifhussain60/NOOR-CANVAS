...
test.describe("Issue-114 Final Fix: Countries API with GUID Parameter", () => {
  ...
  test("should load countries dropdown with correct API call including guid parameter", async ({
    page,
  }: {
    page: Page;
  }) => {
    console.log(
      "🔧 FINAL FIX TEST: Verifying countries API with guid parameter...",
    );
    ...
    page.on("response", (response) => {
      if (response.url().includes("/api/host/countries")) {
        console.log(
          `🌐 Countries API Response: ${response.status()} ${response.url()}`,
        );
      }
    });
    ...
    console.log(
      "🎉 SUCCESS: Countries dropdown loading with GUID parameter is working!",
    );
  });

  test("should show proper error handling if countries API fails", async ({
    page,
  }: {
    page: Page;
  }) => {
    console.log(
      "🔧 ERROR TEST: Testing error handling for countries API failure...",
    );
    ...
    const errorMessage = page.locator(
      'text="Failed to load countries from database."',
    );
