...
test.describe("Issue-106: Direct Cascading Dropdown Validation", () => {
  test("should validate cascading dropdown implementation per ncIssueTracker.MD resolution", async ({
    page,
  }: {
    page: Page;
  }) => {
    ...
    await expect(page.locator('h1:has-text("NOOR Canvas")')).toBeVisible({
      timeout: 10000,
    });
  ...
  test("should validate 2-second cascading delays as specified in Issue-106", async ({
    page,
  }: {
    page: Page;
  }) => {
    ...
    page.on("console", (msg) => {
      if (
        msg.text().includes("ISSUE-106") ||
        msg.text().includes("cascading") ||
        msg.text().includes("SetCascadingDefaultValues")
      ) {
        cascadingLogs.push({
          time: Date.now(),
          message: msg.text(),
    ...
    if (cascadingLogs.length > 0) {
      console.log(
        `✅ Issue-106: Captured ${cascadingLogs.length} cascading log entries`,
      );
      ...
      console.log(
        "ℹ️ No cascading logs captured - may need valid token for full testing",
      );
    }
  });

  test("should validate Open Session URL generation per Issue-106 requirements", async ({
    page,
  }: {
    page: Page;
  }) => {
    ...
    page.on("request", (request) => {
      if (
        request.url().includes("/api/host/create-session") ||
        request.url().includes("/api/host/")
      ) {
        apiCalls.push({
          url: request.url(),
    ...
    console.log(
      `✅ Issue-106: Monitored ${apiCalls.length} API calls during test`,
    );

    if (apiCalls.length > 0) {
      apiCalls.forEach((call, index) => {
        console.log(
          `API Call ${index + 1}: ${call.method} ${call.url} - Status: ${call.status || "pending"}`,
        );
      });
    }
  });

  test("should validate Issue-106 SessionURL panel visibility requirements", async ({
    page,
  }: {
    page: Page;
  }) => {
    ...
    for (const selector of sessionUrlSelectors) {
      try {
        ...
        console.log(
          `Selector "${selector}": exists=${exists}, visible=${isVisible}`,
        );
      } catch (e) {
...
test.describe("Issue-106: Environment and API Validation", () => {
  test("should validate Issue-106 API endpoints availability", async ({
    page,
    request,
  }) => {
