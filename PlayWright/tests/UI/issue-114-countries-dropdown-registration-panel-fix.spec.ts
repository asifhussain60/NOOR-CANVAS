...
test.describe("Issue-114 Enhanced: Countries Dropdown Registration Panel Fix", () => {
  ...
  test("should show token entry panel initially with NO countries dropdown visible", async ({
    page,
  }: {
    page: Page;
  }) => {
    ...
    const countryDropdown = page
      .locator("select", { hasText: "country" })
      .or(page.locator('select:has(option:has-text("United States"))'))
      .or(page.locator('select[name*="Country"]'))
      .or(
        page.locator(
          'label:has-text("Country") + select, label:has-text("Country") ~ div select',
        ),
      );

    const isCountryVisible = await countryDropdown
      .first()
    ...
    console.log(
      "✅ Countries dropdown correctly NOT visible during token entry phase",
    );
  });

  test("should load countries dropdown in registration panel after valid token submission", async ({
    page,
  }: {
    page: Page;
  }) => {
    console.log(
      "🧪 Test 2: Verify countries load in registration panel after token validation...",
    );
  ...
  test("should show loading state while countries are being fetched", async ({
    page,
  }: {
    page: Page;
  }) => {
    console.log(
      "🧪 Test 3: Verify loading indicator during countries fetch...",
    );
    ...
    const hasLoadingText = await loadingOption
    ...
    if (hasLoadingText) {
      ...
      console.log(
        "⚠️ Loading was too fast to detect, checking if countries loaded...",
      );
    }
    ...
    const options = countryDropdown.locator(
      'option:not(:has-text("Select")):not(:has-text("Loading"))',
    );
    ...
    console.log(
      `✅ Countries loaded successfully - ${optionCount} countries available`,
    );
  });

  test("should handle countries API failure gracefully", async ({
    page,
  }: {
    page: Page;
  }) => {
    console.log(
      "🧪 Test 4: Verify error handling for countries API failure...",
    );
    ...
    console.log(
      "Registration panel loaded, checking countries API error handling...",
    );
    ...
    if (!errorFound) {
      ...
      if ((await countryDropdown.count()) > 0) {
        ...
        if (isDisabled) {
          console.log(
            "✅ Countries dropdown appropriately disabled on API failure",
          );
        } else {
          ...
          const optionCount = await options.count();
          if (optionCount <= 2) {
            // Just placeholder and maybe loading
            console.log(
              "✅ Countries dropdown shows minimal options on API failure",
            );
          } else {
  ...
  test("should maintain dropdown state throughout registration form interaction", async ({
    page,
  }: {
    page: Page;
  }) => {
    console.log(
      "🧪 Test 5: Verify countries dropdown maintains state during form interaction...",
    );
    ...
    console.log(
      `Countries dropdown still has ${optionCount} countries after form interaction`,
    );
    ...
    if ((await canadaOption.count()) > 0) {
      ...
      expect(selectedValue).toBeTruthy();
      console.log(
        "✅ Successfully selected Canada from dropdown after form interaction",
      );
    } else {
      ...
      if ((await usOption.count()) > 0) {
        ...
        expect(selectedValue).toBeTruthy();
        console.log(
          "✅ Successfully selected United States from dropdown after form interaction",
        );
      }
    }

    console.log(
      "✅ Countries dropdown maintains functionality throughout registration form interaction",
    );
  });

  test("should debug countries API call and response", async ({
    page,
  }: {
    page: Page;
  }) => {
