...
test.describe('Session Canvas Routing', () => {
  test('should navigate from SessionWaiting to SessionCanvas when StartSession is clicked', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="start-session-btn"]', { timeout: 10000 });
    ...
    await expect(page.locator('text=Session Canvas')).toBeVisible({ timeout: 5000 });
  });

  test('should display SessionCanvas page with basic info', async ({ page }) => {
    ...
    const hasSessionContent = await page.locator('text=Session Canvas').count() > 0;
  ...
  test('should have Start Session button visible on waiting page', async ({ page }) => {
