...
test.describe("Issue-124: Session Name Display Fix", () => {
  test('should display actual session name from API instead of "Unknown Session"', async ({
    page,
  }) => {
    ...
    await page.waitForResponse(
      (response) =>
        response
          .url()
          .includes(`/api/participant/session/${testToken}/validate`) &&
        response.status() === 200,
    );
    ...
    const sessionName = await page
      .locator('[data-testid="session-name"]')
      .textContent();
    ...
    if (sessionName?.includes("Need For Messengers")) {
      console.log(
        "✅ SUCCESS: Session name correctly displays API title:",
        sessionName,
      );
    }

    // Verify session description is also populated
    const sessionDescription = await page
  ...
  test('should handle invalid token gracefully without showing "Unknown Session"', async ({
    page,
  }) => {
    ...
    await page.waitForResponse((response) =>
      response
        .url()
        .includes(`/api/participant/session/${invalidToken}/validate`),
    );
  ...
  test("should verify API response structure matches SessionValidationResponse model", async ({
    page,
  }) => {
