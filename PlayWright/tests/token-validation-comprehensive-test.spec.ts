...
test.describe('Token Validation Comprehensive Test', () => {
    ...
    test('should validate token through manual entry and show registration form', async ({ page }) => {
        ...
        await expect(page.locator('h3:has-text("ENTER TOKEN")')).toBeVisible({ timeout: 5000 });
        ...
        await Promise.all([
            page.waitForResponse(response => 
                response.url().includes(`/api/participant/session/${TEST_TOKEN}/validate`) && 
                response.status() === 200,
                { timeout: 10000 }
            ),
            submitButton.click()
        ...
        await expect(page.locator('h3:has-text("ENTER TOKEN")')).not.toBeVisible({ timeout: 5000 });
        ...
        const sessionName = page.locator('p').filter({ hasText: /Islamic Art|Session|Unknown/ }).first();
        ...
        console.log('✅ Comprehensive token validation test completed successfully!');
    });
    ...
    test('should handle token validation via URL parameter', async ({ page }) => {
        ...
        await page.waitForSelector('h3:has-text("ENTER TOKEN"), input[placeholder*="name" i]', { timeout: 10000 });
        ...
        if (await nameInput.isVisible()) {
            console.log('✅ Token auto-validated successfully, showing registration form');
    ...
    test('should make correct API call and handle response format', async ({ page }) => {
        ...
        page.on('response', async response => {
            if (response.url().includes(`/api/participant/session/${TEST_TOKEN}/validate`)) {
                try {
        ...
        console.log(`  - Has 'valid' property: ${apiResponse?.valid !== undefined}`);
        console.log(`  - Has 'sessionId' property: ${apiResponse?.sessionId !== undefined}`);
    ...
    test('should debug specific UserLanding component behavior', async ({ page }) => {
        console.log('🔍 Debugging UserLanding component behavior...');
        
        // Enable console logging from the page
        page.on('console', msg => {
            if (msg.text().includes('NOOR-DEBUG') || msg.text().includes('UserLanding')) {
        ...
        console.log(`  - Contains "ENTER TOKEN": ${pageContent.includes('ENTER TOKEN')}`);
        console.log(`  - Contains "Invalid Token": ${pageContent.includes('Invalid Token')}`);
