...
test.describe('Countries Dropdown UI Tests', () => {
    ...
    test('should display countries dropdown and load data', async ({ page, request }: { page: Page, request: any }) => {
        console.log('Testing countries dropdown UI with generated authentication token...');
        ...
        console.log(`Loaded countries: ${options.slice(0, 5).join(', ')}${options.length > 5 ? '...' : ''}`);
    ...
    test('should include country in form status display', async ({ page }: { page: Page }) => {
    ...
    test('should be integrated with form validation', async ({ page }: { page: Page }) => {
    ...
    test('should work alongside existing cascading dropdowns', async ({ page }: { page: Page }) => {
        ...
        const countryOptions = await countrySelect.locator('option:not(:first-child)').count();

        console.log(`Dropdown options - Albums: ${albumOptions}, Categories: ${categoryOptions}, Sessions: ${sessionOptions}, Countries: ${countryOptions}`);
        ...
        console.log(`Countries dropdown enabled independently: ${countryEnabledState}`);
        expect(countryEnabledState).toBe(true);

        console.log('Countries dropdown works correctly alongside cascading dropdowns');
    });
});