...
test.describe('Issue-114 Diagnostic: Countries API Loading Debug', () => {
    ...
    test('should debug countries API loading after token validation', async ({ page }: { page: Page }) => {
        console.log('🔍 DIAGNOSTIC: Starting countries API loading debug...');

        // Monitor all network requests
        const networkRequests: Array<{ method: string, url: string, status?: number, response?: string }> = [];
            ...
            if (requestIndex !== -1) {
                ...
                if (response.url().includes('/api/host/countries')) {
                    try {
                        ...
                        console.log(`🌐 COUNTRIES API RESPONSE: ${response.status()} - ${responseText.substring(0, 200)}...`);
                    } catch (error) {
                        console.log(`🌐 COUNTRIES API ERROR: Could not read response - ${error}`);
                    }
                }
            }
        ...
        const countriesRequests = networkRequests.filter(req => req.url.includes('/api/host/countries'));
        ...
        if (countriesRequests.length > 0) {
            countriesRequests.forEach((req, index) => {
                ...
                if (req.response) {
                    console.log(`    Response Preview: ${req.response.substring(0, 100)}...`);
                }
            });
        } else {
        ...
        const tokenValidationRequests = networkRequests.filter(req =>
            req.url.includes('/api/participant/session') && req.url.includes('/validate')
        );
        ...
        if (tokenValidationRequests.length > 0) {
            tokenValidationRequests.forEach((req, index) => {
                console.log(`  Token Validation ${index + 1}: ${req.status} ${req.url}`);
            });
        }
        ...
        console.log(`${countriesRequests.length > 0 ? '✅' : '❌'} Countries API called: ${countriesRequests.length > 0 ? 'YES' : 'NO'}`);
        console.log(`${optionCount > 2 ? '✅' : '❌'} Countries loaded: ${optionCount > 2 ? 'YES' : 'NO'} (${optionCount} options)`);

        if (countriesRequests.length === 0) {
            console.log('🔥 ROOT CAUSE: LoadCountriesAsync() is not being called after token validation!');
        } else if (countriesRequests.some(req => req.status && req.status >= 400)) {
            console.log('🔥 ROOT CAUSE: Countries API is failing!');
        } else if (optionCount <= 2) {
            console.log('🔥 ROOT CAUSE: API succeeds but countries are not being added to dropdown!');
        }
    });

    test('should test token validation workflow step by step', async ({ page }: { page: Page }) => {
        ...
        const countriesLoadTest = await page.evaluate(() => {
            ...
            return {
                blazorComponents: document.querySelectorAll('[data-blazor-component-id]').length,
                selects: document.querySelectorAll('select').length,
                hasCountrySelect: document.querySelector('select option[value="US"]') !== null,
                selectsWithOptions: Array.from(document.querySelectorAll('select')).map(select => ({
