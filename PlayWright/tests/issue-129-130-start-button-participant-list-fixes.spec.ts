...
test.describe('Issue-129 & Issue-130: Start Button and Participant List Fixes', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.getByText('Loading Session...')).toBeHidden({ timeout: 10000 });
  });

  test('Issue-129: Start Session button should not be visible in waiting room', async ({ page }) => {
  ...
  test('Issue-130: Participant list should add participants instead of replacing', async ({ page }) => {
    ...
    const participantElements = await participantsList.locator('[data-testid="participant-flag"]').count();

    // Verify participant data structure
    if (participantElements > 0) {
      // Check first participant has proper flag and name
      const firstParticipant = participantsList.locator('[data-testid="participant-flag"]').first();
      ...
      const participantNames = await participantsList.locator('span').allTextContents();
      expect(participantNames.length).toBeGreaterThanOrEqual(1);

      console.log(`Found ${participantElements} participants:`, participantNames);
    } else {
      // If no participants yet, verify the empty state message
      await expect(page.getByText('No participants yet. You\'re the first!')).toBeVisible();
    }
  });

  test('Issue-130: Participant list persistence - names should not be replaced', async ({ page }) => {
    // Wait for participants to load
    await expect(page.getByText('Participants')).toBeVisible();

    // Get current participants
    const participantsList = page.getByTestId('participants-list');
    const initialParticipants = await participantsList.locator('span').allTextContents();
    ...
    const laterParticipants = await participantsList.locator('span').allTextContents();
    ...
    if (initialParticipants.length > 0) {
      expect(laterParticipants.length).toBeGreaterThanOrEqual(initialParticipants.length);
  ...
  test('Issue-129 & Issue-130: Waiting room layout without start button', async ({ page }) => {
    ...
    await expect(page.getByText('Click to enter the session canvas')).toBeHidden();
  });

  test('Issue-130: Participant sorting functionality works correctly', async ({ page }) => {
    // Wait for participants to load
    await expect(page.getByText('Participants')).toBeVisible();
  ...
  test('Issue-129: Navigation flow without start button interference', async ({ page }) => {
