...
test.describe('User Authentication Flow', () => {
    ...
    test('should display user landing page for no token', async ({ page }: { page: Page }) => {
    ...
    test('should display token entry form for invalid token', async ({ page }: { page: Page }) => {
    ...
    test('should handle valid session token and show session details', async ({ page, request }: { page: Page; request: APIRequestContext }) => {
...
test.describe('Session Waiting Room', () => {
    ...
    test('should display waiting room with countdown timer', async ({ page }: { page: Page }) => {
        ...
        const hasWaitingRoom: boolean = await waitingElements.first().isVisible({ timeout: 5000 });
...
test.describe('Active Session Canvas', () => {
    ...
    test('should display canvas interface for active session', async ({ page }: { page: Page }) => {
        ...
        const hasCanvas: boolean = await canvasElements.first().isVisible({ timeout: 5000 });
        ...
        if (hasCanvas) {
            ...
            const hasTools: boolean = await tools.first().isVisible({ timeout: 2000 });
...
test.describe('End-to-End User Journey', () => {
    ...
    test('complete user flow: token entry -> waiting room -> active session', async ({ page, request }: { page: Page; request: APIRequestContext }) => {
...
test.describe('Issue-102: Routing Logic Fixes', () => {
    ...
    test('should show token entry form for no token (not registration)', async ({ page }: { page: Page }) => {
        ...
        const hasRegistration: boolean = await registrationElements.first().isVisible({ timeout: 2000 });
    ...
    test('should show token entry form for invalid token (not registration)', async ({ page }: { page: Page }) => {
    ...
    test('should handle API validation failures correctly', async ({ page }: { page: Page }) => {
