...
test.describe('Panel Spacing Verification', () => {
    test('Verify margin between golden border panels', async ({ page }) => {
        ...
        if (panelCount === 1) {
            console.log('✅ Only one panel visible (session panel). Need to open session to see link panel.');
            ...
            console.log(`After opening session: ${updatedPanelCount} golden border panels`);

            if (updatedPanelCount >= 2) {
                // Get the second panel (link panel) and check its margin
                const linkPanel = page.locator('div[style*="border:2px solid #C5B358"]:has-text("Session URL")');
                const marginTop = await linkPanel.evaluate(el => window.getComputedStyle(el).marginTop);
            ...
            if (await linkPanel.count() > 0) {
                ...
                await page.screenshot({ path: 'Tests/UI/screenshots/panel-spacing-debug.png', fullPage: true });
            }
        }
    });
});