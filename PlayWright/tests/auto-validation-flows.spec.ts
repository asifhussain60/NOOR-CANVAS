...
test.describe("Auto-Validation Flows", () => {
  ...
  test.describe("UserLanding Auto-Validation", () => {
    test("should automatically validate token and show registration panel when token is in URL", async ({
      page,
    }) => {
      ...
      console.log(
        '✅ Session name loaded automatically: "Need For Messengers"',
      );
      ...
      console.log(
        `✅ Countries loaded automatically: ${countryOptions} options`,
      );

      console.log("🎉 UserLanding auto-validation test passed!");
    });

    test("should automatically validate token from query parameter", async ({
      page,
    }) => {
      console.log(
        "🧪 Testing UserLanding auto-validation with query parameter",
      );
    ...
    test("should show token input panel when no token is provided", async ({
      page,
    }) => {
  ...
  test.describe("HostLanding Auto-Validation", () => {
    test("should automatically validate and route to control panel when host token is in URL", async ({
      page,
    }) => {
      ...
      if (isLoading) {
        console.log(
          "⚡ Loading state detected - auto-authentication in progress",
        );
      ...
      const hasRedirected =
        currentUrl.includes("session-opener") ||
        currentUrl.includes("control-panel");
      const hasSessionInfo = await page
        .locator("text=Need For Messengers")
        .isVisible();
    ...
    test("should show manual authentication form when no token is provided", async ({
      page,
    }) => {
      ...
      const tokenInput = page.locator(
        'input[placeholder="Enter your Host GUID Token"]',
      );
      await expect(tokenInput).toBeVisible();
      console.log(
        "✅ Manual authentication form displayed when no token provided",
      );
  ...
  test.describe("Cross-Browser Auto-Validation Compatibility", () => {
    test("should work consistently across page refreshes", async ({ page }) => {
      ...
      const firstLoadResult = await page
        .locator('h3:has-text("REGISTRATION")')
        .isVisible();
      console.log(
        `First load auto-validation: ${firstLoadResult ? "SUCCESS" : "FAILED"}`,
      );
      ...
      const secondLoadResult = await page
        .locator('h3:has-text("REGISTRATION")')
        .isVisible();
      console.log(
        `Second load auto-validation: ${secondLoadResult ? "SUCCESS" : "FAILED"}`,
      );
  ...
  test.describe("Error Handling in Auto-Validation", () => {
    test("should handle invalid tokens gracefully", async ({ page }) => {
      ...
      const isManualMode = await page
        .locator('h3:has-text("ENTER TOKEN")')
        .isVisible();
