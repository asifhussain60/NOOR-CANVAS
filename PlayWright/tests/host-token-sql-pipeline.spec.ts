...
test.describe("Host Token Validation", () => {
  test("should load Host Control Panel with database token from SQL pipeline", async ({
    page,
  }) => {
    ...
    await expect(page.locator("text=Loading Session")).toBeHidden({
      timeout: 10000,
    });
    ...
    console.log(
      "✅ Host token 67HATXRR successfully validated through database pipeline",
    );
  });

  test("should display session transcript from KSessions database", async ({
    page,
  }) => {
    ...
    try {
      ...
      console.log(
        "✅ Session transcript successfully loaded from KSessions database",
      );
    } catch {
      // If no transcript exists in database, that's also valid
      await expect(noTranscriptMessage).toBeVisible();
      console.log(
        "ℹ️  No transcript found in database for session 212 (expected)",
      );
    }
  });
  ...
  test("should not use hardcoded token mappings", async ({ page }) => {
    ...
    page.on("console", (msg) => {
      const text = msg.text();
      if (
        text.includes("Using hardcoded mapping") ||
    ...
    console.log(
      "✅ No hardcoded token mappings used - database queries working correctly",
    );
  });
});
