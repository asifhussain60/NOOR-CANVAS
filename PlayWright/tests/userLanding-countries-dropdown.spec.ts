...
test.describe("Issue-108: UserLanding Countries Dropdown", () => {
  ...
  test("Should load countries dropdown dynamically with correct count from SQL database", async ({
    page,
  }) => {
    ...
    const countriesSelect = page.locator(
      '#country-select, select[name="Country"], .countries-dropdown select',
    );
    ...
    const loadingIndicator = page.locator(
      '.loading, .spinner, text="Loading countries..."',
    );
    ...
    const options = page.locator(
      '#country-select option, select[name="Country"] option, .countries-dropdown select option',
    );
    ...
    await expect(
      page.locator('option:has-text("United Kingdom")'),
    ).toBeVisible();
  ...
  test("Should display loading state while countries are being fetched", async ({
    page,
  }) => {
  ...
  test("Should validate SQL database count matches dropdown count", async ({
    page,
  }) => {
    ...
    const isCountValid =
      dropdownCount === apiCount || dropdownCount === apiCount + 1;
    ...
    // Log success message
    console.log(
      `✅ SQL Database validation passed: ${apiCount} countries in database match dropdown functionality`,
    );
  });
  ...
  test("Should handle API errors gracefully", async ({ page }) => {
    ...
    const errorMessage = page.locator(
      '.error, .alert-danger, text="Error loading countries"',
    );
  ...
  test("Should maintain performance during countries loading", async ({
    page,
  }) => {
    ...
    if (loadTime > 5000) {
      console.warn(
        `⚠️ Countries loading took ${loadTime}ms, consider optimization`,
      );
    }
  });
});
