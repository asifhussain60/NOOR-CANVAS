...
test.describe("Issue-120: Host Control Panel Token-Based Routing Fix", () => {
  ...
  test("should navigate Host Landing → Host Session Opener → Host Control Panel with token", async ({
    page,
  }: {
    page: Page;
  }) => {
    console.log(
      "🔧 ROUTING-TEST: Testing complete host workflow with token BIIVCFDY...",
    );
    ...
    console.log(
      "🎯 Step 2: Navigating to Host Session Opener with token BIIVCFDY...",
    );
    await page.goto(
      `${baseURL}/host/session-opener/${SESSION_212_DATA.hostToken}`,
    );
    await page.waitForLoadState("networkidle");

    // Verify Host Session Opener loaded with correct token
    await expect(page).toHaveURL(
      new RegExp(`/host/session-opener/${SESSION_212_DATA.hostToken}`),
    );
    ...
    if (!controlPanelLink) {
      console.log(
        "⚠️ No direct control panel link found, testing direct URL navigation...",
      );
      await page.goto(
        `${baseURL}/host/control-panel/${SESSION_212_DATA.hostToken}`,
      );
    } else {
    ...
    await expect(page).toHaveURL(
      new RegExp(`/host/control-panel/${SESSION_212_DATA.hostToken}`),
    );
    console.log(
      "✅ Host Control Panel URL uses token BIIVCFDY (not session ID 10227)",
    );
    ...
    expect(currentUrl).toContain(
      `/host/control-panel/${SESSION_212_DATA.hostToken}`,
    );

    console.log(
      "🎯 ROUTING-TEST COMPLETED: Host Control Panel token-based routing verified!",
    );
  });

  test("should handle token mapping correctly for session 212", async ({
    page,
  }: {
    page: Page;
  }) => {
    console.log(
      "🔄 TOKEN-MAPPING: Testing GetSessionIdFromTokenAsync functionality...",
    );
    ...
    console.log(
      "🎯 TOKEN-MAPPING TEST COMPLETED: GetSessionIdFromTokenAsync working correctly!",
    );
  });

  test("should maintain backward compatibility with session ID routing", async ({
    page,
  }: {
    page: Page;
  }) => {
    console.log(
      "🔄 BACKWARD-COMPATIBILITY: Testing session ID routing still works...",
    );

    // Test that the old session ID routing still works for backward compatibility
    await page.goto(
      `${baseURL}/host/control-panel/${SESSION_212_DATA.canvasSessionId}`,
    );
    await page.waitForLoadState("networkidle");
