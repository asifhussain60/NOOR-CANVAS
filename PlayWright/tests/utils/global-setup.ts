...
async function globalSetup() {
  ...
  try {
    ...
    if (!dbAccessible) {
      throw new Error(
        "KSESSIONS_DEV database is not accessible or has no session data",
      );
    }

    // Get available sessions for testing
    const availableSessions =
      await DatabaseTokenManager.getAvailableSessionTokens();
    console.log(
      `📊 Found ${availableSessions.length} sessions with tokens available for testing`,
    );

    if (availableSessions.length === 0) {
      console.warn(
        "⚠️ No sessions with tokens found - creating test session...",
      );
      const testSession = await DatabaseTokenManager.createTestSession(
        "Playwright Test Session",
      );
      if (testSession) {
        console.log(
          `✅ Created test session ${testSession.sessionId} with tokens: ${testSession.hostToken}/${testSession.userToken}`,
        );
      } else {
      ...
      console.log(
        "✅ Database validation passed - Sessions and tokens available",
      );
    }
  } catch (error) {
