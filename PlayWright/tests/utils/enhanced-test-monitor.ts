...
export class EnhancedTestMonitor {
    ...
    static async quickHealthCheck(): Promise<boolean> {
        try {
            ...
            const healthCommand = 'try { Invoke-WebRequest -Uri https://localhost:9091 -UseBasicParsing -SkipCertificateCheck -TimeoutSec 3 | Out-Null; Write-Output "HEALTHY" } catch { try { Invoke-WebRequest -Uri http://localhost:9090 -UseBasicParsing -TimeoutSec 3 | Out-Null; Write-Output "HEALTHY" } catch { Write-Output "UNHEALTHY" } }';

            const { stdout } = await execAsync(`powershell.exe -Command "${healthCommand}"`, {
    ...
    static async quickStartApplication(): Promise<boolean> {
        try {
            ...
            for (let attempt = 1; attempt <= 6; attempt++) {
                ...
                console.log(`⏳ Waiting for application startup... attempt ${attempt}/6`);
            }

            throw new Error('Application failed to start within 18 seconds using nc.ps1');
        } catch (error) {
     ...
     */
    static async handleTestFailure(testInfo: any, error: Error): Promise<boolean> {
        ...
        if (isAppFailure) {
            ...
            if (this.testHealth.consecutiveFailures <= this.MAX_CONSECUTIVE_FAILURES) {
     ...
     */
    private static async logFailureForAnalysis(testInfo: any, error: Error, isAppFailure: boolean): Promise<void> {
        const failureLog = {
        ...
        if (isAppFailure && this.testHealth.consecutiveFailures >= this.MAX_CONSECUTIVE_FAILURES) {
            console.log('🚨 Multiple application failures detected - may need manual intervention');
        }
    }
 ...
 */
export const enhancedBeforeEach = test.beforeEach(async ({ page }, testInfo) => {
...
export const enhancedAfterEach = test.afterEach(async ({ page }, testInfo) => {
    ...
    if (testInfo.status === 'failed' && testInfo.error) {
        ...
        const recovered = await EnhancedTestMonitor.handleTestFailure(testInfo, error);
    ...
    console.log(`📊 Test completed: ${testInfo.title} - Status: ${testInfo.status} - Duration: ${EnhancedTestMonitor.testHealth.testExecutionTime}ms`);
});
