...
export class DatabaseTokenManager {
   ...
   */
  static async getSessionByHostToken(
    hostToken: string,
    ...
    try {
      ...
      console.error(
        `❌ Failed to retrieve session for host token ${hostToken}:`,
        error,
      );
      return null;
    }
  ...
  static async getRandomActiveSession(): Promise<SessionTokenData | null> {
    ...
    if (activeSessions.length === 0) {
      console.warn(
        "⚠️ No active sessions found, using permanent test session 212",
      );
      return this.PERMANENT_TEST_TOKENS[0];
    }
  ...
  static async validateDatabaseAccess(): Promise<boolean> {
    try {
      ...
      if (this.useFallbackMode) {
        console.log(
          "🎭 Database validation: Using fallback mode with mock tokens",
        );
  ...
  static async createTestSession(
    sessionTitle: string,
    createdBy: string = "Playwright Tests",
  ): Promise<SessionTokenData | null> {
    await this.initializeConnection();
