...
test.describe("Issue-114: Countries Dropdown Loading After Submit", () => {
  ...
  test("should NOT load countries dropdown during initial token entry phase", async ({
    page,
  }: {
    page: Page;
  }) => {
    console.log(
      "Testing Issue-114: Countries should NOT be loaded during token entry...",
    );
    ...
    const countryDropdown = page.locator(
      'select[name*="Country"], select:has(option:has-text("United States"))',
    );

    // Countries dropdown should not be visible during token entry
    const isCountryDropdownVisible = await countryDropdown
      .first()
    ...
    if ((await countryDropdown.count()) > 0) {
      ...
      console.log(
        "✅ Countries dropdown exists but has no countries loaded (as expected)",
      );
    }
  });

  test("should load countries dropdown ONLY after successful token submission", async ({
    page,
  }: {
    page: Page;
  }) => {
    console.log(
      "Testing Issue-114: Countries should load ONLY after token validation...",
    );
    ...
    console.log(
      `✅ Countries dropdown loaded ${optionCount} countries after token validation`,
    );
  });

  test("should show loading indicator while countries are being fetched", async ({
    page,
  }: {
    page: Page;
  }) => {
    console.log(
      "Testing Issue-114: Loading indicator during countries fetch...",
    );
    ...
    const loadingText = page.locator(
      'option:has-text("Loading countries"), text="Loading countries"',
    );
    ...
    if (hasLoadingText) {
      ...
      console.log(
        "⚠️ Loading might have been too fast to catch, but that's acceptable",
      );
    }
  ...
  test("should handle countries API failure gracefully after token validation", async ({
    page,
  }: {
    page: Page;
  }) => {
    console.log(
      "Testing Issue-114: Error handling for countries API failure...",
    );
    ...
    const errorMessage = page.locator(
      'text="Failed to load countries", text="Error loading countries", .error',
    );
  ...
  test("should maintain countries dropdown state during registration form interaction", async ({
    page,
  }: {
    page: Page;
  }) => {
