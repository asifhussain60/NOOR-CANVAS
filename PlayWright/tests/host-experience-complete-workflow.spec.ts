...
} from "./utils/database-token-manager";
...
test.describe("Host Experience - Complete Workflow", () => {
  ...
  test("Complete Host Experience - Authentication to Control Panel", async ({
    page,
  }) => {
    ...
    if (!isHealthy) {
      ...
      if (!startSuccess) {
        throw new Error(
          "❌ Failed to start application for host experience test",
        );
      }
    }
    ...
    await test.step("Navigate directly to host session opener", async () => {
      ...
      console.log(
        `✅ Successfully navigated to session opener with token: ${sessionData!.hostToken}`,
      );
    });
    ...
    await test.step("Complete cascading dropdown selection", async () => {
      ...
      await expect(albumOptions.nth(0)).toHaveAttribute("value", /.+/, {
        timeout: 10000,
      });
      ...
      if (firstAlbumValue) {
        ...
        const categoryOptions = categorySelect.locator(
          'option:not([value=""])',
        );
        await expect(categoryOptions.nth(0)).toHaveAttribute("value", /.+/, {
          timeout: 10000,
        });
        ...
        if (firstCategoryValue) {
          ...
          const sessionOptions = sessionSelect.locator(
            'option:not([value=""])',
          );
          await expect(sessionOptions.nth(0)).toHaveAttribute("value", /.+/, {
            timeout: 10000,
          });

          // Select the first available session
          const firstSessionValue = await sessionOptions
            .first()
            .getAttribute("value");
    ...
    await test.step("Fill session details (date, time, duration)", async () => {
      ...
      const timeInput = page.locator(
        'input[id*="time"], input[placeholder*="time" i]',
      );
    ...
    await test.step("Open session and validate URL generation", async () => {
      ...
      await expect(page.locator("text=Session URL")).toBeVisible({
        timeout: 15000,
      });

      // Verify the generated URL contains a user token
      const sessionUrlElement = page
        .locator("text*=https://localhost:9091/user/landing/")
        .first();
      ...
      expect(sessionUrlText).toMatch(
        /https:\/\/localhost:9091\/user\/landing\/[A-Z0-9]{8}/,
      );

      // Extract the user token from the URL
      const userTokenMatch = sessionUrlText?.match(
        /\/user\/landing\/([A-Z0-9]{8})/,
      );
    ...
    await test.step("Load control panel and validate data", async () => {
      ...
      const controlPanelButton = page.locator(
        'button:has-text("Load Control Panel")',
      );
      ...
      await expect(
        page.locator(
          'h2:has-text("Host Control Panel"), h1:has-text("Host Control Panel")',
        ),
      ).toBeVisible();
    ...
    await test.step("Validate control panel data and functionality", async () => {
      ...
      await expect(
        page.locator(
          'button:has-text("Start Session"), button:has-text("Begin Session")',
        ),
      ).toBeVisible();

      // Verify participants section (may be empty initially)
      const participantsSection = page.locator(
        'text*="Participants", text*="Participant List"',
      );
      ...
      const questionsSection = page.locator(
        'text*="Questions", text*="Question List"',
      );
      ...
      const transcriptArea = page.locator(
        '[class*="transcript"], [id*="transcript"], text*="Transcript"',
      );
      ...
      const sessionStatus = page.locator(
        'text*="Status", text*="Active", text*="Waiting"',
      );
    ...
    await test.step("Verify complete workflow success", async () => {
      ...
      console.log(
        `✅ Complete host experience workflow validated successfully`,
      );
...
export class HostExperienceTestUtils {
   ...
   */
  static async waitForDropdownOptions(
    page: Page,
    selectIndex: number,
    ...
    try {
      ...
      await expect(
        select.locator('option:not([value=""])').first(),
      ).toBeVisible({ timeout });
      return true;
    } catch {
      return false;
