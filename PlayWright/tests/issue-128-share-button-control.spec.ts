...
test.describe('Issue-128: Share Button Injection Control', () => {
    ...
    test('should NOT show share buttons in transcript when session is not active', async ({ page }) => {
        ...
        if (await transcriptSection.count() > 0) {
            ...
            const shareButtons = transcriptSection.locator('button[data-share-button="asset"]');
            await expect(shareButtons).toHaveCount(0);
            
            // Also check for any buttons containing "SHARE" text
            const shareTextButtons = transcriptSection.locator('button:has-text("SHARE")');
            await expect(shareTextButtons).toHaveCount(0);
            
            console.log('✅ PASS: No share buttons found in transcript when session is not active');
        } else {
            console.log('ℹ️  INFO: No transcript content found - test case not applicable');
        }
    });

    test('should show share buttons in transcript after session is started', async ({ page }) => {
        ...
        if (await startButton.count() > 0 && await startButton.isEnabled()) {
            ...
            if (await transcriptSection.count() > 0) {
                ...
                if (buttonCount > 0) {
                    console.log(`✅ PASS: Found ${buttonCount} share buttons in transcript after session start`);
                    
                    // Verify button properties
                    const firstButton = shareButtons.first();
                    await expect(firstButton).toHaveAttribute('data-share-button', 'asset');
                    await expect(firstButton).toContainText('SHARE');
                } else {
                    console.log('ℹ️  INFO: No share buttons found - may indicate no assets in transcript');
                }
            } else {
                console.log('ℹ️  INFO: No transcript content found after session start');
            }
        } else {
            console.log('ℹ️  INFO: Start Session button not available or not enabled - session may already be active');
        }
    });

    test('should verify session status controls share button visibility', async ({ page }) => {
        ...
        const initialShareButtons = page.locator('.session-transcript-content button:has-text("SHARE")');
        ...
        if (await startButton.isVisible() && await startButton.isEnabled()) {
            ...
            const finalShareButtons = page.locator('.session-transcript-content button:has-text("SHARE")');
            ...
            if (finalCount > initialCount) {
                ...
                console.log('ℹ️  INFO: No change in share button count - may indicate no shareable assets');
            }
        } else {
            console.log('ℹ️  INFO: Session may already be active or Start button not available');
        }
    });
});