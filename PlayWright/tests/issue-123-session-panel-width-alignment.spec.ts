...
test.describe("Issue-123: Session Panel Width Alignment", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(
      page.locator('h2:has-text("Need For Messengers")'),
    ).toBeVisible();
    await expect(page.locator('h3:has-text("SESSION CONTROLS")')).toBeVisible();
  });

  test("should have consistent width alignment between session panels", async ({
    page,
  }) => {
    // Get the session details panel (contains "Need For Messengers")
    const sessionDetailsPanel = page
      .locator('h2:has-text("Need For Messengers")')
      .locator("..");

    // Get the session controls panel container
    const sessionControlsContainer = page
      .locator('h3:has-text("SESSION CONTROLS")')
      .locator("../..");
    ...
    const widthDifference = Math.abs(
      detailsPanelBox!.width - controlsPanelBox!.width,
    );
    ...
    const horizontalDifference = Math.abs(
      detailsPanelBox!.x - controlsPanelBox!.x,
    );
    expect(horizontalDifference).toBeLessThan(5);

    console.log(
      `Session Details Panel: width=${detailsPanelBox!.width}px, x=${detailsPanelBox!.x}px`,
    );
    console.log(
      `Session Controls Panel: width=${controlsPanelBox!.width}px, x=${controlsPanelBox!.x}px`,
    );
    console.log(`Width difference: ${widthDifference}px`);
    console.log(`Horizontal alignment difference: ${horizontalDifference}px`);
  });

  test("should have panels contained within the same parent container", async ({
    page,
  }) => {
    // Verify both panels share the same parent container structure
    const commonContainer = page
      .locator("div")
      .filter({
    ...
    const parentContainer = page
      .locator("div")
      .filter({ hasText: "HOST CONTROL PANEL" })
      .first();
  ...
  test("should maintain consistent visual styling between panels", async ({
    page,
  }) => {
    // Get panel elements
    const sessionDetailsPanel = page
      .locator('h2:has-text("Need For Messengers")')
      .locator("..");
    const sessionControlsPanel = page
      .locator('h3:has-text("SESSION CONTROLS")')
      .locator("..");

    // Verify both panels have border radius styling
    const detailsBorderRadius = await sessionDetailsPanel.evaluate(
      (el) => getComputedStyle(el).borderRadius,
    );
    const controlsBorderRadius = await sessionControlsPanel.evaluate(
      (el) => getComputedStyle(el).borderRadius,
    );
    ...
    const detailsPadding = await sessionDetailsPanel.evaluate(
      (el) => getComputedStyle(el).padding,
    );
    const controlsPadding = await sessionControlsPanel.evaluate(
      (el) => getComputedStyle(el).padding,
    );
  ...
  test("should preserve panel content and functionality after width alignment", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("text=we look at the purpose of sending messengers"),
    ).toBeVisible();
