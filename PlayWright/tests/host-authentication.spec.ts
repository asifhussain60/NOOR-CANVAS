...
test.describe('Host Authentication Flow', () => {
    ...
    test('should display host landing page with proper branding', async ({ page }: { page: Page }) => {
        ...
        const accessButton = page.locator('button:has-text("Access Host Control Panel")');
        await expect(accessButton).toBeVisible();
    });

    test('should show validation error for empty token', async ({ page }: { page: Page }) => {
    ...
    test('should show validation error for invalid token format', async ({ page }: { page: Page }) => {
    ...
    test('should generate new host token via API', async ({ page, request }: { page: Page; request: APIRequestContext }): Promise<void> => {
    ...
    test('should handle expired token gracefully', async ({ page }: { page: Page }) => {
    ...
    test('should maintain NOOR Canvas styling throughout flow', async ({ page }: { page: Page }) => {
        // Verify color scheme consistency with NOOR Canvas design system
        const card = page.locator('.card, [class*="card"]').first();
        const cardStyles = await card.evaluate((el: Element) => getComputedStyle(el));
