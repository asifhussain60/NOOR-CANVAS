...
export async function generateTestToken(
  ...
  const sessionResponse = await request.post(
    "https://localhost:9091/api/session/create",
    {
  ...
  try {
    await request.post(
      "https://localhost:9091/api/participant/test/create-token-pair",
      {
        ignoreHTTPSErrors: true,
        data: {
          sessionId: sessionId,
    ...
    console.warn(
      "Failed to create token pair via API, tokens may not validate:",
      error,
    );
  }
