...
test.describe('Issue-119: SignalR Real-Time Participant Updates', () => {
    ...
    test.beforeEach(async ({ page }) => {
        ...
        page.on('console', msg => {
            if (msg.text().includes('NOOR-SIGNALR') || msg.text().includes('SignalR')) {
                console.log(`[Browser Console]: ${msg.text()}`);
            }
        });
    });

    test('Real-time participant updates should be visible within 3 seconds', async ({ page, context }) => {
        ...
        await page.waitForSelector('[data-testid="participant-count"]', { timeout: 10000 });
        ...
        await registrationPage.goto(`${BASE_URL}/user/landing/${TEST_SESSION_TOKEN}`);
        await registrationPage.fill('input[placeholder*="name"], input[name*="name"]', testParticipants[0].name);
        
        // Select country
        const countrySelector = 'select[name*="country"], select[id*="country"]';
        await registrationPage.selectOption(countrySelector, testParticipants[0].country);
        ...
        await page.waitForFunction((participantName) => {
            const participantElements = document.querySelectorAll('[data-testid="participant-name"]');
            return Array.from(participantElements).some(el => el.textContent?.includes(participantName));
        }, testParticipants[0].name, { timeout: 5000 });
        ...
        const participantNames = await page.$$eval('[data-testid="participant-name"]', 
            elements => elements.map(el => el.textContent?.trim()));
        expect(participantNames).toContain(testParticipants[0].name);

        // 6. Verify participant count updated
        const newCount = await page.textContent('[data-testid="participant-count"]');
        expect(parseInt(newCount)).toBeGreaterThan(parseInt(initialCount));

        await newContext.close();
    });

    test('Multiple users registering concurrently should all appear immediately', async ({ page, context }) => {
        ...
        const initialCount = parseInt(await page.textContent('[data-testid="participant-count"]'));
        ...
        for (let i = 0; i < 3; i++) {
            ...
            const registrationPromise = (async () => {
                await registrationPage.goto(`${BASE_URL}/user/landing/${TEST_SESSION_TOKEN}`);
                await registrationPage.fill('input[placeholder*="name"], input[name*="name"]', testParticipants[i + 1].name);
                await registrationPage.selectOption('select[name*="country"], select[id*="country"]', testParticipants[i + 1].country);
                await registrationPage.click('button[type="submit"], .btn-success');
                
                // Wait for successful registration
                await registrationPage.waitForURL('**/session/waiting/**', { timeout: 10000 });
            })();
        ...
        await page.waitForFunction((expectedCount) => {
            const countText = document.querySelector('[data-testid="participant-count"]')?.textContent;
            return countText && parseInt(countText) >= expectedCount;
        }, initialCount + 3, { timeout: 10000 });
        ...
        const finalCount = parseInt(await page.textContent('[data-testid="participant-count"]'));
    ...
    test('SignalR connection failure should fallback to polling gracefully', async ({ page }) => {
        ...
        await page.goto(`${BASE_URL}/session/waiting/${TEST_SESSION_TOKEN}`);
        await page.waitForSelector('[data-testid="participant-count"]', { timeout: 15000 });

        // 3. Verify basic functionality still works
        const participantCount = await page.textContent('[data-testid="participant-count"]');
    ...
    test('Session-scoped events should only affect correct waiting room', async ({ page, context }) => {
        ...
        const session1InitialCount = parseInt(await page.textContent('[data-testid="participant-count"]'));
        const session2InitialCount = parseInt(await session2Page.textContent('[data-testid="participant-count"]'));
        ...
        await registrationPage.fill('input[placeholder*="name"], input[name*="name"]', 'Session1 Participant');
        await registrationPage.selectOption('select[name*="country"], select[id*="country"]', 'United States');
        await registrationPage.click('button[type="submit"], .btn-success');
        ...
        }, session1InitialCount, { timeout: 5000 });

        // 5. Verify session 1 count increased
        const session1FinalCount = parseInt(await page.textContent('[data-testid="participant-count"]'));
        ...
        const session2FinalCount = parseInt(await session2Page.textContent('[data-testid="participant-count"]'));
        expect(session2FinalCount).toBe(session2InitialCount);

        console.log(`✅ Session isolation working: Session1 ${session1InitialCount}→${session1FinalCount}, Session2 unchanged at ${session2FinalCount}`);

        await session2Context.close();
        await registrationContext.close();
    });

    test('API call frequency should be significantly reduced with SignalR', async ({ page }) => {
        console.log('🧪 Testing API call frequency optimization...');

        // Track API calls
        const apiCalls = [];
        page.on('request', request => {
            if (request.url().includes('/api/participant/session/') && request.url().includes('/participants')) {
                apiCalls.push({
                    url: request.url(),
                    timestamp: Date.now()
                });
        ...
        console.log(`📊 API calls in 30 seconds: ${callsDuring30Seconds} (should be 0 with SignalR)`);
        ...
        if (apiCalls.length > 1) {
            const timeBetweenCalls = apiCalls[apiCalls.length - 1].timestamp - apiCalls[apiCalls.length - 2].timestamp;
    ...
    test('Performance benchmark: Old vs New participant update times', async ({ page, context }) => {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            await registrationPage.goto(`${BASE_URL}/user/landing/${TEST_SESSION_TOKEN}`);
            await registrationPage.fill('input[placeholder*="name"], input[name*="name"]', participantName);
            await registrationPage.selectOption('select[name*="country"], select[id*="country"]', 'United States');
            await registrationPage.click('button[type="submit"], .btn-success');
            ...
            await page.waitForFunction((name) => {
                const participantElements = document.querySelectorAll('[data-testid="participant-name"]');
                return Array.from(participantElements).some(el => el.textContent?.includes(name));
            }, participantName, { timeout: 10000 });
        ...
        console.log(`🚀 Performance improvement: ${improvementFactor.toFixed(1)}x faster than 30-second polling`);
...
async function waitForSignalRConnection(page, timeout = 10000) {
    return await page.waitForFunction(() => {
        return window.signalRConnectionState === 'Connected' || 
...
function setupSignalRMonitoring(page) {
    return page.addInitScript(() => {
        ...
        if (window.signalR) {
            ...
            window.signalR.HubConnection.prototype.on = function(methodName, callback) {
...
function generateRandomParticipant(index) {
    const names = ['Alex', 'Sam', 'Jordan', 'Casey', 'Riley', 'Cameron', 'Blake', 'Avery'];
