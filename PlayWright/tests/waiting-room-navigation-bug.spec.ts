...
test.describe('Waiting Room Navigation Bug Test', () => {
    ...
    test('Test Enter Waiting Room Button Functionality', async ({ page }) => {
        ...
        try {
            ...
            await page.screenshot({ path: './screenshots/before-session-creation.png', fullPage: true });
            ...
            for (const selector of waitingRoomSelectors) {
                ...
                if (count > 0) {
                    console.log(`✅ Found waiting room button with selector: ${selector} (${count} elements)`);
            ...
            if (waitingRoomButton) {
                ...
                await page.screenshot({ path: './screenshots/before-waiting-room-click.png', fullPage: true });
                ...
                await page.screenshot({ path: './screenshots/after-waiting-room-click.png', fullPage: true });

                console.log(`🏁 Final URL: ${finalUrl}`);
                console.log(`📄 Final page text preview: ${finalPageText?.substring(0, 500)}`);
                ...
                for (const errorIndicator of errorIndicators) {
                    if (finalPageText?.toLowerCase().includes(errorIndicator.toLowerCase())) {
                ...
                for (const successIndicator of waitingRoomSuccessIndicators) {
                    if (finalPageText?.toLowerCase().includes(successIndicator.toLowerCase())) {
                ...
                if (foundErrors.length > 0) {
                    ...
                    console.log('');
                    console.log('🎯 BUG CONFIRMED: Enter Waiting Room button leads to error!');
                ...
                console.log('❌ CRITICAL: No Enter Waiting Room button found after session creation');
                console.log('🔍 This indicates the session creation itself may have failed');
                console.log('📋 Available elements on page:');

                // Try to find any clickable elements that might be the waiting room button
                const allButtons = page.locator('button, a, input[type="button"], input[type="submit"]');
            ...
            await page.screenshot({ path: './screenshots/waiting-room-test-error.png', fullPage: true });
            throw error;
        }
    });
});