...
test.describe('Session Token Investigation', () => {
    test('should test waiting room access with proper user token', async ({ page }) => {
        ...
        await page.goto(`https://localhost:9091/session/waiting?token=${anotherToken}`);
        await page.waitForLoadState('networkidle');
    ...
    test('should examine session creation response for waiting room URL', async ({ page }) => {
        ...
        await page.fill('textarea[name="Description"]', 'Testing session creation for token analysis');
        await page.fill('input[name="MaxParticipants"]', '10');

        // Monitor network requests during session creation
        page.on('response', async (response) => {
            if (response.url().includes('/session/create') || response.url().includes('/api/session')) {
        ...
        const waitingRoomElements = await page.$$('*:has-text("waiting room"), *:has-text("Enter Waiting Room"), *:has-text("Waiting Room")');
        console.log(`Found ${waitingRoomElements.length} waiting room related elements`);
