...
test.describe('Simple Token Validation', () => {
    test('Validate permanent Session 212 tokens work with application', async ({ page }) => {
        ...
        try {
            ...
            const tokenInput = page.locator('input[placeholder*="token" i], input[type="text"], input[name*="token" i]');
            ...
            const accessButton = page.locator('button:has-text("Access Host Control Panel"), button:has-text("Access Control Panel")').first();
            if (await accessButton.isVisible()) {
                const isEnabled = await accessButton.isEnabled();
                console.log(`🎯 Access button enabled: ${isEnabled}`);

                if (isEnabled) {
                    console.log('✅ Access button is enabled - token validation successful');
                } else {
