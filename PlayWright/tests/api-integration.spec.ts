...
test.describe("API Integration Tests", () => {
  test("should generate host token via API", async ({
    request,
  }: {
    ...
    expect(data.hostToken).toMatch(
      /^[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/i,
    );
    expect(data.userToken).toMatch(
      /^[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/i,
    );
  ...
  test("should validate host token via API", async ({
    request,
  }: {
    request: APIRequestContext;
  }) => {
    ...
    if (validateResponse.ok()) {
      const validationData: TokenValidationResponse =
      ...
      console.log(
        "Token validation endpoint status:",
        validateResponse.status(),
      );
    }
  });

  test("should reject invalid host tokens", async ({
    request,
  }: {
    request: APIRequestContext;
  }) => {
  ...
  test("should load Islamic content data via API", async ({
    request,
  }: {
    request: APIRequestContext;
  }) => {
  ...
  test("should load categories for specific album", async ({
    request,
  }: {
    request: APIRequestContext;
  }) => {
    ...
    const categoriesResponse = await request.get(
      "/api/host/categories?albumId=18",
    );
  ...
  test("should load sessions for album and category", async ({
    request,
  }: {
    request: APIRequestContext;
  }) => {
    ...
    const sessionsResponse = await request.get(
      "/api/host/sessions?albumId=18&categoryId=55",
    );
    ...
    if (sessionsResponse.ok()) {
      ...
      if (sessions.length > 0) {
        const hasTestSession: boolean = sessions.some(
          (s: SessionData) => s.id === 1281 || s.sessionId === 1281,
        );
...
test.describe("Performance & Load Tests", () => {
  test("should handle multiple concurrent token generations", async ({
    request,
  }: {
    request: APIRequestContext;
  }) => {
    // Generate multiple tokens concurrently for load testing
    const tokenPromises: Promise<any>[] = Array.from(
      { length: 5 },
      (_, i: number) =>
    ...
    const tokens: TokenGenerationResponse[] = await Promise.all(
      responses.map((r) => r.json()),
    );
    const hostTokens: string[] = tokens.map(
      (t: TokenGenerationResponse) => t.hostToken,
    );
  ...
  test("should handle rapid API calls without errors", async ({
    request,
  }: {
    request: APIRequestContext;
  }) => {
    ...
    for (let i = 0; i < 10; i++) {
      const promise: Promise<number> = request
        .get("/api/health")
        .then((r) => r.status());
...
test.describe("Security & Validation Tests", () => {
  ...
  test("should handle SQL injection attempts safely", async ({
    ...
    if (response.ok()) {
      ...
      console.log(
        "SQL injection attempt rejected with status:",
        response.status(),
      );
    }
  });
