...
test.describe('SessionWaiting Component Tests', () => {
    ...
    test.describe('Mock Data Loading Tests', () => {
        ...
        test('should load mock session data successfully', async ({ page }) => {
            ...
            await expect(page.getByText('Advanced Quranic Calligraphy Workshop')).toBeVisible();
            ...
            await expect(page.locator('[data-testid="countdown-display"]')).toBeVisible();
        ...
        test('should show live countdown timer with mock data', async ({ page }) => {
            await page.goto(`${BASE_URL}/session/waiting/${MOCK_TOKEN}`);
            ...
            const initialCountdown = await page.textContent('[data-testid="countdown-display"]');

            // Wait 2 seconds and check if countdown changed
            await page.waitForTimeout(2000);
            const updatedCountdown = await page.textContent('[data-testid="countdown-display"]');

            // Countdown should be different (decreasing)
            expect(initialCountdown).not.toBe(updatedCountdown);

            console.log(`✅ Countdown timer working: ${initialCountdown} → ${updatedCountdown}`);
        });
    ...
    test.describe('Real API Integration Tests', () => {
        ...
        test('should handle invalid token gracefully', async ({ page }) => {
            ...
            await expect(page.getByText('Please check your session link')).toBeVisible();
        ...
        test('should call participants API endpoint', async ({ page }) => {
            ...
            expect(participantsRequests[0]).toContain(`/api/participant/session/${VALID_TOKEN}/participants`);
    ...
    test.describe('Error Handling Tests', () => {
        ...
        test('should show loading state initially', async ({ page }) => {
            ...
            await expect(page.getByText('Please wait while we prepare your session')).toBeVisible();
...
test.describe('Debug Tests', () => {
    ...
    test('debug component initialization', async ({ page }) => {
        ...
        await page.screenshot({ path: './TEMP/debug-session-waiting.png', fullPage: true });

        console.log('📸 Debug screenshot saved to ./TEMP/debug-session-waiting.png');
    });
