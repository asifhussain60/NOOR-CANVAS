...
test.describe('Session Title/Description KSESSIONS Integration Verification', () => {
  ...
  test('API: SessionController returns title from KSESSIONS.SessionName', async ({ page }) => {
    ...
    const response = await page.request.get(
      `https://localhost:9091/api/session/${testUserToken}/info`,
    );
  ...
  test('API: ParticipantController validation returns KSESSIONS title', async ({ page }) => {
    ...
    const response = await page.request.get(
      `https://localhost:9091/api/participant/session/${testUserToken}/validate`,
    );
    expect(response.ok()).toBeTruthy();
  ...
  test('UI: User Landing page displays KSESSIONS title', async ({ page }) => {
    ...
    await page.goto(`https://localhost:9091/user/landing/${testUserToken}`, {
    ...
    const titleElements = page
      .locator('[data-testid*="session"], [class*="session"], h1, h2, h3, h4')
  ...
  test('UI: Host Landing page displays KSESSIONS title', async ({ page }) => {
    ...
    await page.goto(`https://localhost:9091/host/control-panel/${testHostToken}`, {
      waitUntil: 'networkidle',
    });
  ...
  test('UI: Participant Registration shows KSESSIONS title', async ({ page }) => {
    ...
    await page.goto(`https://localhost:9091/participant/register/${testUserToken}`, {
      waitUntil: 'networkidle',
    });
    ...
    const sessionTitleElement = page
      .locator('.text-primary, h4, h3, h2, h1')
      .filter({ hasText: /^(?!Session \d+$)/ });
  ...
  test('Integration: End-to-end title consistency', async ({ page }) => {
    ...
    const sessionInfoResponse = await page.request.get(
      `https://localhost:9091/api/session/${testUserToken}/info`,
    );
...
test.describe('Session Description KSESSIONS Integration Verification', () => {
  ...
  test('API: SessionController returns description from KSESSIONS.Description', async ({
    page,
  }) => {
    const response = await page.request.get(
      `https://localhost:9091/api/session/${testUserToken}/info`,
    ...
    if (
      ...
      sessionInfo.description !== 'Session description not available'
    ) {
  ...
  test('API: ParticipantController returns KSESSIONS description', async ({ page }) => {
    ...
    if (validationResponse.session?.description) {
      console.log(
        `✅ Participant validation returned description: "${validationResponse.session.description}"`,
      );
    }
  });
});
