...
async function setupTestParticipants(page: Page) {
    ...
    for (let i = 0; i < testConfig.testParticipants.length; i++) {
        ...
        try {
            console.log(`📝 Registering participant ${i + 1}/${testConfig.testParticipants.length}: ${participant.name}`);
...
async function registerParticipantAPI(participant: { name: string; email: string; country: string }): Promise<{ success: boolean; message: string }> {
    const https = require('https');
    const { URL } = require('url');
...
async function deleteParticipants(userToken: string): Promise<{ deletedCount: number; message: string }> {
    const https = require('https');
    const { URL } = require('url');
...
test('should automatically delete participants when host opens session', async ({ page }) => {
    ...
    try {
        ...
        console.log(`🗑️ DELETE API response: ${deleteResult.message}, deleted: ${deleteResult.deletedCount}`);
        ...
        if (addedCount > 0) {
            ...
            console.log(`🗑️ Cleanup result: ${cleanupResult.message}, deleted: ${cleanupResult.deletedCount}`);
            ...
            console.log(`✅ Successfully tested full cycle: added ${addedCount}, deleted ${cleanupResult.deletedCount}`);
        } else {
            console.log('⚠️ Could not add test participants (likely due to token issues), but DELETE endpoint tested successfully');
        }
        ...
        console.log('   3. CreateSessionAndGenerateTokens() calls our ClearExistingParticipants() method');
        console.log('   4. ClearExistingParticipants() calls DELETE /api/participant/session/{userToken}/participants');
