...
test.describe('Host Control Panel User Registration Link', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    console.log(
      `🎯 Test setup - SessionId: ${sessionId}, HostToken: ${redact(hostToken)}, Expected UserToken: ${redact(expectedUserToken)}`,
    );
  ...
  test('should display User Registration Link with valid token between SESSION CONTROLS and Start Session button', async ({
    page,
  }) => {
    ...
    try {
      ...
      if (sessionControlsRect && userLinkRect && startButtonRect) {
        ...
        console.log(
          '✅ User Registration Link positioned correctly between SESSION CONTROLS and Start Session button',
        );
      }
    } catch (error) {
