...
test.describe('Implementation Comparison: SignalR vs HTTP', () => {
  ...
  test('HTTP API Implementation - Question Management', async ({ page }) => {
    ...
    try {
      ...
      console.log(
        `📝 HTTP Question Submission: ${questionResponse.success ? '✅ SUCCESS' : '❌ FAILED'}`,
      );
      ...
      if (questionResponse.success && questionResponse.data?.questionId) {
        ...
        const voteResponse = await page.evaluate(
          async (config, questionId) => {
            try {
          ...
          testConfig,
          questionResponse.data.questionId,
        );
        ...
        console.log(
          `🗳️ HTTP Question Voting: ${voteResponse.success ? '✅ SUCCESS' : '❌ FAILED'}`,
        );
  ...
  test('HTTP API Implementation - Participant Management', async ({ page }) => {
    ...
    try {
      ...
      const participantResponse = await page.evaluate(async (config) => {
        try {
          const response = await fetch(
            `/api/participant/session/${config.userToken}/participants`,
            {
              method: 'GET',
              headers: {
                'Content-Type': 'application/json',
      ...
      console.log(
        `👥 HTTP Participant Loading: ${participantResponse.success ? '✅ SUCCESS' : '❌ FAILED'}`,
      );
      ...
      if (!participantResponse.success) {
        console.log(
          `   Error: ${participantResponse.error || 'HTTP ' + participantResponse.status}`,
        );
      }
      ...
      console.log(
        `🗑️ HTTP Participant Deletion: ${deleteResponse.success ? '✅ SUCCESS' : '❌ FAILED'}`,
      );
    ...
    try {
      ...
      const signalRTest = await page.evaluate(async (config) => {
        ...
        try {
          ...
          const qaConnection = new signalR.HubConnectionBuilder().withUrl('/hub/qa').build();
      ...
      console.log(
        `🔌 SignalR SessionHub: ${signalRTest.sessionHub.success ? '✅ SUCCESS' : '❌ FAILED'}`,
      );
      ...
      console.log(
        `🔌 SignalR AnnotationHub: ${signalRTest.annotationHub.success ? '✅ SUCCESS' : '❌ FAILED'}`,
      );
      ...
      console.log(
        `📨 SignalR Messaging: ${signalRTest.messaging.success ? '✅ SUCCESS' : '❌ FAILED'}`,
      );
  ...
  test('Performance and Reliability Comparison', async ({ page }) => {
    ...
    console.log(
      `  Question Submission: ${testResults.http.questionSubmission.success ? '✅' : '❌'} (${testResults.http.questionSubmission.time}ms)`,
    );
    console.log(
      `  Question Voting:     ${testResults.http.questionVoting.success ? '✅' : '❌'} (${testResults.http.questionVoting.time}ms)`,
    );
    console.log(
      `  Participant Loading: ${testResults.http.participantLoading.success ? '✅' : '❌'} (${testResults.http.participantLoading.time}ms)`,
    );
    console.log(
      `  Participant Updates: ${testResults.http.participantUpdates.success ? '✅' : '❌'} (${testResults.http.participantUpdates.time}ms)`,
    );
    console.log(`  Overall Score: ${httpScore}/100`);

    console.log(`\n🔌 SignalR Implementation:`);
    console.log(
      `  SessionHub Connect:  ${testResults.signalr.questionSubmission.success ? '✅' : '❌'} (${testResults.signalr.questionSubmission.time}ms)`,
    );
    console.log(
      `  QAHub Connect:       ${testResults.signalr.questionVoting.success ? '✅' : '❌'} (${testResults.signalr.questionVoting.time}ms)`,
    );
    console.log(
      `  AnnotationHub:       ${testResults.signalr.participantLoading.success ? '✅' : '❌'} (${testResults.signalr.participantLoading.time}ms)`,
    );
    console.log(
      `  Real-time Messaging: ${testResults.signalr.participantUpdates.success ? '✅' : '❌'} (${testResults.signalr.participantUpdates.time}ms)`,
    );
    ...
    expect(
      httpScore > 0 || signalrScore > 0,
      'At least one implementation (HTTP or SignalR) should be functional',
    ).toBeTruthy();
  });
});
