...
async function globalSetup(config: FullConfig) {
  ...
  try {
    ...
    // Create HTTP agent that ignores SSL certificate errors
    ...
    const response = await fetch(
      `${baseURLHttp}/api/host/token/${testHostToken}/validate`,
      {
        method: "GET",
        headers: {
    ...
    if (response.ok) {
      ...
      if (data.Valid) {
        console.log(
          '✅ Authentication service is working (no "service unavailable" error)',
        );
      } else {
    ...
    console.log("");
    console.log(
      "📝 Note: Tests will still run, but may fail if application is not started",
    );
    console.log(
      "To start the application, run: dotnet run (in SPA/NoorCanvas directory)",
    );
  }
