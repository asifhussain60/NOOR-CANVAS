...
async function setupUserSession(context: BrowserContext, sessionName: string): Promise<Page> {
  ...
  await expect(page.locator('[data-testid="session-canvas"], .session-canvas, #session-canvas')).toBeVisible({ timeout: 15000 });
  
  // Wait for Q&A panel to be present
  await expect(page.locator('[data-testid="qa-panel"], .qa-panel, #qa-panel, .questions-panel')).toBeVisible({ timeout: 10000 });
...
test.describe('Q&A Functionality Multi-User Test', () => {
  ...
  test('should propagate questions between multiple connected user sessions', async () => {
    ...
    const questions = [
      ...
      { user: 'User2', text: 'Can you provide more details about the implementation?', page: user2Page },
    ...
    for (const { user, page } of questions) {
      const qaPanel = page.locator('[data-testid="qa-panel"], .qa-panel, #qa-panel, .questions-panel');
    ...
    for (const { name, page } of allPages) {
      ...
      if (questionCount > 0) {
        ...
        console.log(`✅ ${name}: Found ${foundQuestions} of ${questions.length} expected questions`);
    ...
    // Submit from User1
    const questionInput = user1Page.locator([
      '[data-testid="question-input"]',
      'input[placeholder*="question" i]',
      'textarea[placeholder*="question" i]',
      '.question-input',
    ...
    const submitButton = user1Page.locator([
      '[data-testid="submit-question"]',
      'button:has-text("Submit")',
      'button:has-text("Ask")',
      'button:has-text("Send")'
    ...
    await Promise.all([
      user2Page.waitForTimeout(4000),
      user3Page.waitForTimeout(4000)
    ]);
    
    // Verify the real-time question appears in other users' panels
    for (const { name, page } of [{ name: 'User2', page: user2Page }, { name: 'User3', page: user3Page }]) {
      const qaPanel = page.locator('[data-testid="qa-panel"], .qa-panel, #qa-panel, .questions-panel');
      const panelText = await qaPanel.textContent();
      ...
      if (panelText?.includes('real-time test question')) {
        ...
        console.log(`⚠️ ${name}: Real-time question not yet visible (may need more time for SignalR)`);
      }
    }
  ...
  test('should display user identification for questions', async () => {
    ...
    const questionInput = user2Page.locator([
      '[data-testid="question-input"]',
      'input[placeholder*="question" i]',
      'textarea[placeholder*="question" i]'
    ].join(', ')).first();
    
    await fillBlazorInput(user2Page, questionInput.first().locator('xpath=.'), identifiableQuestion);
    
    const submitButton = user2Page.locator([
      '[data-testid="submit-question"]',
      'button:has-text("Submit")',
      'button:has-text("Ask")'
    ].join(', ')).first();
    ...
    const qaPanel = user1Page.locator('[data-testid="qa-panel"], .qa-panel, #qa-panel, .questions-panel');
    const panelText = await qaPanel.textContent();
    ...
    if (panelText?.includes(identifiableQuestion)) {
      ...
      const hasUserInfo = panelText?.includes('User') || 
                         panelText?.includes('Anonymous') || 
                         panelText?.includes('Participant') ||
                         await qaPanel.locator('.user-name, .question-author, [data-testid="question-user"]').count() > 0;
