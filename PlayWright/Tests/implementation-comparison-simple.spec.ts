...
test.describe('Implementation Comparison: SignalR vs HTTP', () => {
  ...
  test('HTTP API - Question Management', async ({ page }) => {
    ...
    if (userGuidTest.success && userGuidTest.userGuid) {
      ...
      const questionTest = await page.evaluate(
        async (config) => {
          try {
      ...
      console.log(
        `📝 HTTP Question Submission: ${questionTest.success ? '✅ SUCCESS' : '❌ FAILED'}`,
      );
  ...
  test('SignalR Hub Connectivity', async ({ page }) => {
    ...
    console.log(
      `🔌 SignalR AnnotationHub: ${signalRTest.annotationHub ? '✅ SUCCESS' : '❌ FAILED'}`,
    );
