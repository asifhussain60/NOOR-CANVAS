...
async function generateValidUserToken(sessionId: number = 213): Promise<string> {
    ...
    console.log(`🔧 Testing known token patterns for Session ${sessionId}...`);
    ...
    for (const token of knownTokens) {
        ...
        if (isValid) {
            console.log(`✅ Found working token: ${token.substring(0, 4)}****`);
            return token;
        }
    }

    console.log(`🔧 No known tokens work, generating new token using Host Provisioner...`);
    ...
    return new Promise((resolve, reject) => {
        const args = [
            ...
            '--dry-run', 'false',
            '--force-new', 'true' // Force new token generation even if exists
        ];
        ...
        process.on('close', (code: number) => {
            if (code !== 0) {
                ...
                if (errorOutput.includes('being used by another process') ||
                    ...
                    try {
                        ...
                        const apiUrl = new URL('https://localhost:9091/api/host/sessions/XUJEDG28/generate-tokens');
                                ...
                                if (res.statusCode === 200) {
                                    try {
                                        ...
                                        if (response.userToken) {
                                            console.log(`✅ Generated user token via API: ${response.userToken.substring(0, 4)}****`);
                                            resolve(response.userToken);
                                        } else {
                                            console.log('⚠️  No userToken in API response, falling back to test token');
                                            resolve('TEST213U'); // Final fallback
                                        }
                                    } catch (parseError) {
                                        console.log('⚠️  Failed to parse API response, falling back to test token');
                                        resolve('TEST213U');
                                    }
                                } else {
                                    console.log(`⚠️  API returned ${res.statusCode}, falling back to test token`);
                                    resolve('TEST213U');
                                }
                            });
                        });
                        ...
                        req.on('error', () => {
                            console.log('⚠️  API request failed, falling back to test token');
                            resolve('TEST213U');
                        });

                        req.end();
                    } catch (apiError) {
                        console.log('⚠️  API fallback failed, using test token');
                        resolve('TEST213U');
                    }
                    return;
                }
            ...
            if (tokenMatch && tokenMatch[1]) {
                ...
                console.log(`✅ Generated valid user token: ${userToken.substring(0, 4)}****`);
                resolve(userToken);
            } else {
                console.error('❌ Could not extract user token from Host Provisioner output');
        ...
        process.on('error', (err: Error) => {
            console.error(`❌ Failed to start Host Provisioner: ${err.message}`);
            reject(err);
        });
    });
}
...
test.describe('User Registration Multi-Instance Synchronization', () => {
    ...
    test('should register 10 users concurrently and sync participants across all instances', async () => {
        ...
        console.log(`🎯 Starting multi-user registration test with 10 superhero instances using token: ${TOKEN.substring(0, 4)}****`);
        ...
        for (let i = 0; i < 10; i++) {
            ...
            await page.waitForFunction(() => {
                ...
                return select && select.options.length > 1 && !select.options[0].textContent?.includes('Loading');
            }, { timeout: 10000 });
        ...
        for (let i = 0; i < 10; i++) {
            ...
            const submitButton = page.locator('button').filter({ hasText: /Join Waiting Room|Register|Submit/ }).first();
