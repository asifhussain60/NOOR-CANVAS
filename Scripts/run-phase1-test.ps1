# Phase 1: SessionWaiting Registration Guard - Test Orchestration Script
# 
# Purpose: Automated test execution for Phase 1 registration guard implementation
# 
# This script:
# 1. Launches the NoorCanvas application (dotnet run)
# 2. Waits for application readiness (15 seconds)
# 3. Executes Playwright test suite (phase1-session-waiting-guard.spec.ts)
...
try {
    ...
    Write-Host "Phase 1: SessionWaiting Registration Guard Test" -ForegroundColor Cyan
    Write-Host "========================================`n" -ForegroundColor Cyan
    ...
    $testArgs = "playwright", "test", "phase1-session-waiting-guard.spec.ts"
    ...
    if ($testExitCode -eq 0) {
        ...
        Write-Host "`nPhase 1 registration guard is working correctly:" -ForegroundColor Green
