<#
.SYNOPSIS
    Launches NOOR Canvas application and runs Transcript Broadcast Mode Percy visual regression tests.
...
.DESCRIPTION
    ...
    5. Tracks JavaScript console errors during test execution
    6. Cleans up application process after tests complete
    
    Visual regression tests cover:
    - Host Control Panel initial state with Share Transcript button
    - Broadcast mode activation (transcript loads in HostControlPanel)
    - Single broadcast button appearance (NO individual share buttons)
    - Transcript content with purple theme distinction
    - Participant waiting room before broadcast
    - SignalR broadcast triggering participant navigation
    - Participant TranscriptCanvas view with purple badge
    - Host remaining in HostControlPanel (no navigation)
    
.PARAMETER TestPattern
    Playwright test file pattern to run. Defaults to transcript broadcast mode test.
...
.NOTES
    Author: GitHub Copilot (task.prompt.md)
    Date: 2025-10-17
    Version: 1.0.0
...
[CmdletBinding()]
param(
    [string]$TestPattern = "Workspaces/TEMP/transcript-broadcast-mode.spec.ts",
    [switch]$SkipBuild,
...
Write-Host "  NOOR Canvas - Transcript Broadcast Mode Percy Test Runner" -ForegroundColor Cyan
Write-Host "=====================================================================" -ForegroundColor Cyan
Write-Host ""
