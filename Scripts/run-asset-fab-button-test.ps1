<#
.SYNOPSIS
    Asset FAB Button Visibility Test - Robust Orchestration
...
.DESCRIPTION
    ...
    Tests that FAB share buttons are properly injected for each asset in the transcript.
...
.EXAMPLE
    .\run-asset-fab-button-test.ps1
    Runs tests in headless mode and stops app after completion.

.EXAMPLE
    .\run-asset-fab-button-test.ps1 -Headed -KeepAppRunning
...
$testFile = 'Tests/UI/asset-fab-button-visibility.spec.ts'
$appUrl = 'https://localhost:9091'

Write-Host ''
Write-Host '════════════════════════════════════════════════════' -ForegroundColor Cyan
Write-Host '  Asset FAB Button Visibility Test Suite' -ForegroundColor Cyan
Write-Host '════════════════════════════════════════════════════' -ForegroundColor Cyan
Write-Host ''
...
try {
    ...
    if ($KeepAppRunning) {
        ...
        Write-Host "URL: $appUrl/host/control-panel/PQ9N5YWW" -ForegroundColor Gray
