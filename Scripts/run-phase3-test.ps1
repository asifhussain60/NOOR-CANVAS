# Phase 3: TranscriptCanvas Registration Guard - Test Orchestration Script
# 
# Purpose: Automated test execution for Phase 3 registration guard implementation
# 
# This script:
# 1. Launches the NoorCanvas application (dotnet run)
# 2. Waits for application readiness (15 seconds)
# 3. Executes Playwright test suite (phase3-transcript-canvas-guard.spec.ts)
...
try {
    ...
    Write-Host "Phase 3: TranscriptCanvas Registration Guard Test" -ForegroundColor Cyan
    Write-Host "========================================`n" -ForegroundColor Cyan
    ...
    $testArgs = "playwright", "test", "phase3-transcript-canvas-guard.spec.ts"
    ...
    if ($testExitCode -eq 0) {
        ...
        Write-Host "`nPhase 3 registration guard is working correctly:" -ForegroundColor Green
        ...
        Write-Host "`n[MILESTONE] Phase 3 COMPLETE - All session pages protected" -ForegroundColor Magenta
    }
    else {
