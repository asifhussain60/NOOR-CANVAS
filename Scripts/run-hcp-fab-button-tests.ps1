...
try {
    ...
    # CRITICAL: Ensure we're in project root (where Tests/UI/ exists)
    $projectRoot = (Get-Location).Path
    if ($projectRoot -match "SPA\\NoorCanvas") {
        # Navigate back to project root if we're in SPA/NoorCanvas
        Set-Location "../.."
