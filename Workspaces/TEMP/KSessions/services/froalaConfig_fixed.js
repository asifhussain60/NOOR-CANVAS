...
(function () {
    ...
    //CTRL + D
    $.FE.RegisterShortcut($.FE.KEYCODE.D, "htmlCleanerButton", null, "D", false, false);
    ...
    function registerHtmlCleanerButton() {
        ...
        $.FE.DefineIcon("htmlCleanerButton", { NAME: "paint-brush" });
        ...
        $.FE.RegisterCommand("htmlCleanerButton", {
            ...
            callback: function () {
                ...
                try {
                    ...
                    // === PROTECTION: Handle content specially BEFORE cleanup ===
                    var protectedSections = [];
                    var protectedSectionIndex = 0;

                    // 1. PROTECT poetry content FIRST - ensure it remains untouched
                    cleanedHtml = cleanedHtml.replace(/<div[^>]*class[^>]*poetry-section[^>]*>[\s\S]*?<\/div>/gi, function(match) {
                        var placeholder = '___POETRY_SECTION_' + protectedSectionIndex + '___';
                        protectedSections[protectedSectionIndex] = match;
                        protectedSectionIndex++;
                        console.log('??? [HTML-CLEANER] Protecting poetry section from cleanup - Length: $match.length');
                        return placeholder;
                    });
                    ...
                    cleanedHtml = cleanedHtml.replace(/<div[^>]*id=["']ahadees-fallback-modal["'][^>]*>[\s\S]*?<\/div>/gi, function(match) {
                        var placeholder = '___AHADEES_MODAL_' + protectedSectionIndex + '___';
                        protectedSections[protectedSectionIndex] = match;
                        protectedSectionIndex++;
                        console.log('??? [HTML-CLEANER] Protecting ahadees modal from cleanup');
                        return placeholder;
                    });
                    ...
                    cleanedHtml = cleanedHtml.replace(/<[^>]*class[^>]*(?:ahadees-modal|ahadees-panel|ahadees-popup)[^>]*>[\s\S]*?<\/[^>]+>/gi, function(match) {
                        ...
                        protectedSections[protectedSectionIndex] = match;
                        protectedSectionIndex++;
                        console.log('??? [HTML-CLEANER] Protecting ahadees modal/panel element from cleanup');
                        return placeholder;
                    });

                    // 3. REMOVE inserted ahadees divs and convert to placeholder text
                    // Target specifically the inserted ahadees divs (not the modal/panel)
                    cleanedHtml = cleanedHtml.replace(/<div[^>]*class[^>]*inserted-hadees[^>]*class[^>]*ks-ahadees-container[^>]*>[\s\S]*?<\/div>/gi, function(match) {
                        console.log('??? [HTML-CLEANER] Removing inserted ahadees div and converting to placeholder');
                        return 'AHADEES_SECTION_0AHADEES_SECTION_1AHADEES_SECTION_2';
                    });

                    // Also handle cases where the classes might be in different order
                    cleanedHtml = cleanedHtml.replace(/<div[^>]*class[^>]*ks-ahadees-container[^>]*class[^>]*inserted-hadees[^>]*>[\s\S]*?<\/div>/gi, function(match) {
                        console.log('?? [HTML-CLEANER] Removing inserted ahadees div (reverse class order) and converting to placeholder');
                        return 'AHADEES_SECTION_0AHADEES_SECTION_1AHADEES_SECTION_2';
                    });
                    ...
                    cleanedHtml = cleanedHtml
                        ...
                        .replace(/<div([^>]*class=["']([^"']*?)["'][^>]*)>\s*<div([^>]*class=["']([^"']*?)["'][^>]*)>([\s\S]*?)<\/div>\s*<\/div>/gi, function(match, outerAttrs, outerClasses, innerAttrs, innerClasses, content) {
                            ...
                            var importantClasses = ['fr-', 'ayah-card', 'golden-surah-header', 'translation-header', 'esotericBlock', 'previligedBlock', 'etymology-card', 'row', 'col-', 'poetry-section', 'froala-poetry-container'];
                        ...
                        // === NEW: Remove empty ayah-card divs with only <br> tags ===
                        .replace(/<div[^>]*class[^>]*ayah-card[^>]*>(\s*<br\s*\/?>\s*)*<\/div>/gi, '')
                        ...
                        .replace(/<span[^>]*>\s*<\/span>/gi, function (match) {
                            // Keep elements that have specific important classes that might need to be preserved
                            var importantClasses = ['fr-', 'inlineArabic', 'highlight', 'ayah-'];
                            var hasImportantClass = importantClasses.some(function (cls) {
                                return match.indexOf(cls) !== -1;
                            });
                    ...
                    // Restore all protected sections (poetry and ahadees modal/panel)
                    for (var i = 0; i < protectedSections.length; i++) {
                        // Check if it's a poetry section
                        var poetryPlaceholder = '___POETRY_SECTION_' + i + '___';
                        if (cleanedHtml.indexOf(poetryPlaceholder) !== -1) {
                            cleanedHtml = cleanedHtml.replace(poetryPlaceholder, protectedSections[i]);
                            console.log('?? [HTML-CLEANER] Restored poetry section');
                            continue;
                        }
                        
                        // Check if it's an ahadees modal
                        var modalPlaceholder = '___AHADEES_MODAL_' + i + '___';
                        if (cleanedHtml.indexOf(modalPlaceholder) !== -1) {
                            cleanedHtml = cleanedHtml.replace(modalPlaceholder, protectedSections[i]);
                            console.log('?? [HTML-CLEANER] Restored ahadees modal/panel element');
                            continue;
                        }
                    }
                    ...
                    console.log('? [HTML-CLEANER] Poetry sections and ahadees modal/panel preserved');
                    ...
                    showToastNotification('success', 'HTML Cleaned', 'HTML has been cleaned. Inserted ahadees converted to placeholder text, poetry and modal preserved.');
                    ...
                    dlg.confirmationDialog("HTML Cleaner Error", "Failed to clean HTML. Please check the console for details.", "OK", null)
    ...
    function froalaConfig(common, datacontext, quranService, dlg, $filter) {
        ...
        var service = {
            ...
            fEditorAdvancedOptions: function (events, autoSaveOptions) {
                ...
                froalaOpts.events['froalaEditor.click'] = function (e, editor, clickEvent) {
                    ...
                    // Helper function to clean translation text by removing verse numbers in square brackets
                    function cleanTranslationText(translationText) {
                        ...
                        // Remove verse numbers in square brackets at the end of the text
                        // Pattern matches: [34], [123], etc. (one or more digits in square brackets)
                        // This should handle cases like: "...became of the disbelievers. [34] ]."
                        var cleanedText = translationText;

                        // Remove verse numbers with square brackets - matches patterns like [34], [123], etc.
                        // This regex looks for square brackets with one or more digits, optionally followed by spaces and punctuation
                        cleanedText = cleanedText.replace(/\s*\[\d+\]\s*\]?\s*\.?\s*$/g, '');
                        
                        // Also handle cases where verse numbers appear in the middle or with extra brackets
                        // Pattern: [number] followed by optional ] or .
                    ...
                    if (($target.hasClass('fa-ban') || $target.hasClass('text-danger')) && 
                        ...
                        var $rowContainer = $target.closest('.row, div, .quran-block, .legacy-quran');
                        if ($rowContainer.length === 0) {
                            console.warn('?? [RED-BAN-CONVERSION] No container found, trying parent elements...');
                            $rowContainer = $target.closest('*').parent();
                        }
            ...
            if (existingButtons > 0) {
                $('.poetry-restore-btn').each(function(index) {
                    console.log('?? [POETRY-EVENT-BINDING] Button #' + index + ':', {
                        ...
                        text: $(this).text().trim(),
                        container: $(this).closest('.froala-poetry-container, .poetry-section').length > 0 ? 'FOUND' : 'NOT FOUND'
                    });
                });
            }
        ...
        function registerEtymologyButton() {
            ...
            $.FE.RegisterCommand("etymologyButton", {
                ...
                callback: function () {
                    ...
                    var resultHtml =
                        '<div class="etymology-container" style="width: 80%; margin: 40px auto; padding: 0 15px; box-sizing: border-box;">' +
        ...
        function showPoetryMetadataModal(editor, text, contentType) {
            try {
                ...
                $('input[name="content-type"]').on('change', function() {
                    var selectedType = $(this).val();
                    var $label = $('#input-label');
                    var $input = $('#poetry-metadata-input');
                    var $help = $('#input-help');
                ...
                $('#poetry-apply-btn').off('click').on('click', function() {
                    var selectedType = $('input[name="content-type"]:checked').val();
        ...
        function generateSimpleBilingualPoetryHTML(lines, metadata) {
            ...
            if (metadata && metadata.poetName && metadata.poetName.length > 0) {
                html += '        <!-- Poet Name Header with Plain Text Button -->\n';
                ...
                html += '        <!-- Poetry Restore Button Only -->\n';
        ...
        function generateArabicUrduOnlyHTML(lines, metadata) {
            ...
            // Add poet name header if provided
            if (metadata.poetName && metadata.poetName.length > 0) {
                html += '        <!-- Poet Name Header with Plain Text Button -->\n';
                ...
                // If no poet name, still show the button in a header-like area
                html += '        <!-- Poetry Restore Button Only -->\n';
                html += '        <div class="poetry-poet-header" style="display: flex; justify-content: flex-end; align-items: center; text-align: center; margin-bottom: 20px; padding: 10px; border-bottom: 2px solid #722727; background: linear-gradient(135deg, #f5f3ef 0%, #fcfbf7 100%);">\n';
                html += '            <button type="button" class="btn btn-sm btn-primary poetry-restore-btn froala-only-btn" title="Remove poetry formatting and restore plain text" style="margin: 0; flex-shrink: 0;">\n';
                html += '                <i class="fa fa-undo" style="margin-right: 5px;"></i>Plain Text\n';
                html += '            </button>\n';
                html += '        </div>\n';
            ...
            try {
                ...
                $(document).on('click.poetryRestore', '.poetry-restore-btn.froala-only-btn', function(e) {
                    ...
                    if (originalText) {
                        ...
                        var $container = $poetrySection.closest('.container');
        ...
        function showToastNotification(type, title, message) {
            try {
                ...
                console.error('? [POETRY-NOTIFICATION] Error showing notification:', error);
                // Ultimate fallback
                alert(title + '\n\n' + message);
            }
        }
        ...
        function isPoetryElement(element) {
            ...
            // Check if element has poetry-section class
            if ($(element).hasClass('poetry-section')) {
                return true;
            }
            
            // Check if element is inside a poetry section
            return $(element).closest('.poetry-section').length > 0;
        }
        ...
        function findParentPoetryElement(element) {
            ...
            return $(element).closest('.poetry-section')[0] || null;
        }
        ...
        function removePoetryFormatting(editor, poetryElement) {
            try {
                var $poetryDiv = $(poetryElement).closest('.poetry-section');
        ...
        function processQuranHtmlForTranslationCorrection(htmlContent) {
            ...
            try {
                ...
                $temp.find('.quran-ayats, .quran-ayats-single, .ayah-arabic, .arabic-text, p.ayah-arabic, div.ayah-arabic').each(function() {
                    ...
                    // Only apply existing Arabic cleaning if this is genuinely Arabic content
                    if (isArabicText(arabicText)) {
                        // PRESERVE VERSE NUMBER IMAGES: Don't use .text() which strips HTML
                        // Instead, process only text nodes to preserve <img> tags
                        $arabicElement.contents().filter(function() {
                            return this.nodeType === 3; // Text nodes only
                        }).each(function() {
                            var textNode = this;
                            if (isArabicText(textNode.nodeValue)) {
        ...
        function registerCopyToClipboardButton() {
            $.FE.DefineIcon("copyToClipboardButton", { NAME: "copy" });
            $.FE.RegisterCommand("copyToClipboardButton",
                {
                    ...
                    callback: function () {
                        ...
                        // Apply custom styling with orange icon after click
                        setTimeout(function() {
                            var $button = $('[data-cmd="copyToClipboardButton"]');
                            if ($button.length) {
                                $button.find('i').css({
                                    'color': '#ff6600'
                                });
            ...
            $.FE.RegisterCommand("pasteFromClipboardButton",
                {
                    ...
                    callback: function () {
                        ...
                        // Apply custom styling with orange icon after click
                        setTimeout(function() {
                            var $button = $('[data-cmd="pasteFromClipboardButton"]');
                            if ($button.length) {
                                $button.find('i').css({
                                    'color': '#ff6600'
                                });
