...
test.describe("Waiting Room Token Validation", () => {
  test("should access waiting room with correct UserToken format", async ({
    page,
  }) => {
    ...
    console.log(
      ...
      queryResponse?.substring(0, 300) + "...",
    );

    // Check if we get past the "Session Error - No session token provided" message
    const hasTokenError = queryResponse?.includes(
      "Session Error - No session token provided",
    );
    const hasWaitingRoom =
      queryResponse?.includes("waiting") ||
      queryResponse?.includes("Waiting") ||
      queryResponse?.includes("session") ||
      queryResponse?.includes("participant");
    ...
    console.log(
      ...
      pathResponse?.substring(0, 300) + "...",
    );

    const pathHasError = pathResponse?.includes(
      "Session Error - No session token provided",
    );
    ...
    await page.goto(
      `https://localhost:9091/session/waiting?token=${alternativeToken}`,
    );
    await page.waitForLoadState("networkidle");

    const altResponse = await page.textContent("body");
    console.log(
      "Alternative token response:",
      altResponse?.substring(0, 300) + "...",
    );

    // Test 4: Try with session parameter instead of token
    console.log(
      `Testing session parameter: /session/waiting?session=${userToken}`,
    );
    await page.goto(
      `https://localhost:9091/session/waiting?session=${userToken}`,
    );
    await page.waitForLoadState("networkidle");

    const sessionParamResponse = await page.textContent("body");
    console.log(
      "Session parameter response:",
      sessionParamResponse?.substring(0, 300) + "...",
    );

    // Final assessment
    console.log("\n=== VALIDATION RESULTS ===");
    console.log(
      "Query format (?token=) works:",
      !hasTokenError && hasWaitingRoom,
    );
    console.log("Path format (/token) works:", !pathHasError);
    console.log(
      "Session parameter works:",
      !sessionParamResponse?.includes(
        "Session Error - No session token provided",
      ),
    );
  ...
  test("should verify session creation actually generates UserToken", async ({
    page,
  }) => {
    ...
    await page.fill(
      ...
      "Testing if UserToken is returned after session creation",
    );
    ...
    console.log(
      "Final content contains UserToken pattern:",
      /[A-Z0-9]{8}/.test(finalContent || ""),
    );
