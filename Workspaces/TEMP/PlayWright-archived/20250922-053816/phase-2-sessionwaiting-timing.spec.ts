...
test.describe("Phase 2: SessionWaiting.razor Timing Fix", () => {
  test("should display real session data without hard-coded values", async ({
    page,
  }) => {
    ...
    const titleElement = await page
      .locator("h1, h2, h3")
      .filter({ hasText: /Model For Success/ })
      .first();
    ...
    if (durationElements.length === 0) {
      ...
      console.log(
        '⚠️ Found hard-coded "1 hour" duration - may need further fixes',
      );
    }
    ...
    if (loadingElements.length === 0) {
      ...
      console.log(
        '⚠️ Found "Loading..." placeholders:',
        loadingElements.length,
      );
    }

    // Check that we don't have any generic fallback values
    const fallbackElements = await page
      .getByText(/Duration not available/)
      .all();
    ...
    console.log(
      "Page loaded successfully. Content length:",
      pageContent.length,
    );
    ...
    const errorElements = await page
      .locator('.error, .alert-danger, [data-testid="error"]')
      .all();
  ...
  test("should validate real timing calculations", async ({ page }) => {
    ...
    const apiResponse = await page.request.get(
      `http://localhost:9090/api/participant/session/${testToken}/validate`,
    );
    if (apiResponse.ok()) {
    ...
    const countdownElements = await page
      .locator('[class*="countdown"], [data-testid*="time"], .time')
      .all();
