...
test.describe("Host Authentication with Friendly Token - API Only", () => {
  ...
  test("should authenticate host via API with friendly token", async ({
    request,
  }) => {
    ...
    const albumsResponse = await request.get(
      `https://localhost:9091/api/host/albums?guid=${TEST_HOST_TOKEN}`,
    );
    ...
    if (albumsResponse.ok()) {
      ...
      console.log(
        `✅ Host authenticated successfully - ${albums.length} albums loaded`,
      );
    ...
    const categoriesResponse = await request.get(
      `https://localhost:9091/api/host/categories/18?guid=${TEST_HOST_TOKEN}`,
    );
    ...
    if (categoriesResponse.ok()) {
      ...
      console.log(
        `✅ Categories loaded - ${categories.length} categories for Album 18`,
      );
    ...
    const sessionsResponse = await request.get(
      `https://localhost:9091/api/host/sessions/55?guid=${TEST_HOST_TOKEN}`,
    );
    ...
    if (sessionsResponse.ok()) {
      ...
      console.log(
        `✅ Sessions loaded - ${sessions.length} sessions for Category 55`,
      );

      // Find our test session (1281)
      const testSession = sessions.find(
        (session: any) => session.SessionID === 1281,
      );
      if (testSession) {
        console.log(
    ...
    const createSessionResponse = await request.post(
      `https://localhost:9091/api/host/create-session?token=${TEST_HOST_TOKEN}`,
      {
        data: sessionPayload,
      },
    );
    ...
    console.log(
      `Session Creation API Status: ${createSessionResponse.status()}`,
    );
  ...
  test("should verify complete cascading dropdown chain with host token", async ({
    request,
  }) => {
    ...
    console.log(
      "Validating cascading relationship: Album 18 → Category 55 → Session 1281",
    );

    // Step 1: Get Album 18
    const albumsResponse = await request.get(
      `https://localhost:9091/api/host/albums?guid=${TOKEN}`,
    );
    expect(albumsResponse.ok()).toBeTruthy();
    const albums = await albumsResponse.json();
    ...
    const categoriesResponse = await request.get(
      `https://localhost:9091/api/host/categories/18?guid=${TOKEN}`,
    );
    expect(categoriesResponse.ok()).toBeTruthy();
    const categories = await categoriesResponse.json();
    ...
    console.log(
      `✅ Category 55: ${category55.CategoryName} (belongs to Album ${category55.GroupId})`,
    );

    // Step 3: Get Sessions for Category 55, find Session 1281
    const sessionsResponse = await request.get(
      `https://localhost:9091/api/host/sessions/55?guid=${TOKEN}`,
    );
    expect(sessionsResponse.ok()).toBeTruthy();
    const sessions = await sessionsResponse.json();
    ...
    console.log(
      `✅ Session 1281: ${session1281.SessionName} (belongs to Category ${session1281.CategoryId})`,
    );
