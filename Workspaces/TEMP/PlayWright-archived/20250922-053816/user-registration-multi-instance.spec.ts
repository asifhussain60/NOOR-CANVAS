...
test.describe('User Registration Multi-Instance Synchronization', () => {
    ...
    test('should register 5 users concurrently and sync participants across all instances', async () => {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            await expect(page.locator('h1')).toContainText(['Noor Canvas', 'NOOR CANVAS', 'Dynamic Styling Test Session']);

            // Wait for session validation and form to appear
            await page.waitForSelector('input[type="text"][placeholder="Enter your display name"]', { timeout: 10000 });
        ...
        for (let i = 0; i < 5; i++) {
            ...
            await fillBlazorInput(page, 'input[type="text"][placeholder="Enter your display name"]', user.name);
