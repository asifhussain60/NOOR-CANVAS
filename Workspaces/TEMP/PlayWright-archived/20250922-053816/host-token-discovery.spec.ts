...
test.describe("Host Token Discovery", () => {
  test("Find working host token", async ({ page, request }) => {
    ...
    for (const token of possibleTokens) {
      ...
      try {
        const response = await page.goto(
          `${BASE_URL}/host/session-opener/${token}`,
          {
        ...
        if (
          pageContent &&
          !pageContent.includes("Invalid host token") &&
          !pageContent.includes("API Error")
        ) {
          ...
          await page.screenshot({
            path: `./screenshots/working-token-${token}.png`,
            fullPage: true,
          });
    ...
    try {
      ...
      const tokenListResponse = await request.get(
        `${BASE_URL}/api/host/tokens`,
        {
      ...
      if (tokenListResponse.ok()) {
        ...
        console.log(
          `   ❌ Token list API returned: ${tokenListResponse.status()}`,
        );
      }
    } catch (error) {
    ...
    console.log(
      "🔧 Method 3: Attempting token generation with various parameters...",
    );
    ...
    for (const tokenRequest of tokenRequests) {
      try {
        ...
        const tokenResponse = await request.post(
          `${BASE_URL}/api/host/generate-token`,
          {
            data: tokenRequest,
            ignoreHTTPSErrors: true,
          },
        ...
        if (tokenResponse.ok()) {
          ...
          if (tokenData.hostToken) {
            ...
            await page.goto(
              `${BASE_URL}/host/session-opener/${tokenData.hostToken}`,
            );
            await page.waitForTimeout(3000);
            ...
            if (pageContent && !pageContent.includes("Invalid host token")) {
              console.log(
                `🎯 CONFIRMED WORKING: Generated token ${tokenData.hostToken} works!`,
              );
              await page.screenshot({
                path: `./screenshots/generated-token-${tokenData.hostToken}.png`,
                fullPage: true,
              });
              return;
            }
          ...
          console.log(
            `   ❌ Token generation failed: ${tokenResponse.status()} - ${errorText}`,
          );
        }
      } catch (error) {
