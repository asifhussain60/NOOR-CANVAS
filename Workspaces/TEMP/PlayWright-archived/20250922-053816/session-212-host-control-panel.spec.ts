...
test.describe("Session 212 Host Control Panel Validation", () => {
  ...
  const EXPECTED_SESSION_DATA = {
    sessionId: 212,
    sessionName:
      "we look at the purpose of sending messengers, and their role in our spiritual awakening.",
  ...
  test("should load Host Control Panel for Session 212 successfully", async ({
    page,
  }: {
    page: Page;
  }) => {
    ...
    console.log(
      `📍 Navigating to Host Control Panel: ${HOST_CONTROL_PANEL_URL}`,
    );
    ...
    const mainContainer = page.locator(
      ".host-control-panel, .control-panel, main, .container",
    );
    ...
    const sessionInfo = page.locator(
      '[data-testid="session-info"], .session-details, .session-name',
    );
  ...
  test("should load and display session 212 details correctly", async ({
    page,
  }: {
    page: Page;
  }) => {
    ...
    const sessionNameSelector =
      'h1, h2, .session-title, .session-name, [data-testid="session-name"]';
    ...
    if (displayedSessionName) {
      ...
      const hasMessengers = displayedSessionName
        .toLowerCase()
        .includes("messengers");
      const hasSpiritual = displayedSessionName
        .toLowerCase()
        .includes("spiritual");
      ...
      if (hasMessengers || hasSpiritual || hasPurpose) {
        console.log(
          "✅ Session name contains expected content about messengers/spiritual awakening",
        );
      } else {
        console.log(
          `⚠️ Session name might be different than expected: ${displayedSessionName}`,
        );
      }
    }
    ...
    for (const selector of sessionDescriptionSelectors) {
      ...
      if (await descElement.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        console.log(
          `📝 Session Description Found: "${descText?.substring(0, 100)}..."`,
        );
        break;
      }
    }
  ...
  test("should handle participants loading and display", async ({
    page,
  }: {
    page: Page;
  }) => {
    ...
    if (participantsSection) {
      ...
      const participantItems = page.locator(
        '.participant-item, .participant, [data-testid="participant"]',
      );
      ...
      if (participantCount > 0) {
        ...
        for (let i = 0; i < maxToShow; i++) {
          ...
          console.log(
            `  👤 Participant ${i + 1}: ${participantText?.substring(0, 50)}`,
          );
        }
      } else {
      ...
      console.log(
        "ℹ️ Participants section not found - may be hidden when empty",
      );
    }
  ...
  test("should validate API endpoints for session 212", async ({
    request,
  }: {
    ...
    try {
      ...
      if (sessionResponse.ok()) {
        ...
        console.log(
          `📋 API Response: ${JSON.stringify(sessionData, null, 2).substring(0, 200)}...`,
        );
        ...
        console.log(
          `❌ Session details API failed with status: ${sessionResponse.status()}`,
        );
    ...
    try {
      ...
      console.log(
        `📡 Participants API Status: ${participantsResponse.status()}`,
      );
      ...
      if (participantsResponse.ok()) {
        ...
        console.log(
          `👥 Participants Count: ${participantsData.participantCount || 0}`,
        );
        ...
        if (
          ...
          console.log(
            `👤 First Participant: ${JSON.stringify(participantsData.participants[0], null, 2)}`,
          );
        }
      } else {
        console.log(
          `ℹ️ Participants API status: ${participantsResponse.status()} (may be expected for empty session)`,
        );
      }
    } catch (error) {
  ...
  test("should handle error states gracefully", async ({
    ...
    for (const selector of errorSelectors) {
      ...
      if (errorCount > 0) {
        errorsFound = true;
        console.log(
          `⚠️ Found ${errorCount} error element(s) with selector: ${selector}`,
        );
    ...
    if (functionalElementsCount > 0) {
      ...
      console.log(
        "⚠️ Page may not be loading correctly - no functional elements found",
      );
    }
  ...
  test("should validate responsive design and accessibility", async ({
    page,
  }: {
    page: Page;
  }) => {
