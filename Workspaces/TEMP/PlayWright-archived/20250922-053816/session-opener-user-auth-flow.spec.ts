...
test.describe("Session Opener and User Link Generation - Simplified Architecture", () => {
  ...
  test("Session Creation API with Token Embedding", async ({ request }) => {
    test.info().annotations.push({
      ...
      description:
        "Tests session creation API with embedded tokens (simplified schema)",
    });
    ...
    console.log(
      `✅ Session Created via API - ID: ${data.SessionId}, UserToken: ${data.UserToken}`,
    );
  });

  test("User Authentication Link Generation and Format", async ({
    request,
  }) => {
    test.info().annotations.push({
      type: "link-generation",
      description:
        "Tests user authentication link generation with embedded tokens",
    });
    ...
    joinLinks.forEach((link) => {
      expect(link).toMatch(
        /^https?:\/\/localhost:909[01]\/user\/landing\/[A-Z0-9]{8}$/,
      );
    });
  ...
  test("User Registration and Participant Creation - Simplified Schema", async ({
    page,
    request,
  }) => {
    test.info().annotations.push({
      ...
      description:
        "Tests user registration creating entries in Participants table",
    });
    ...
    const nameInput = page.locator(
      'input[name="name"], input[name="displayName"], input[placeholder*="name"], input[placeholder*="Name"]',
    );
    ...
    if (await nameInput.isVisible()) {
      ...
      console.log(
        "ℹ️ User registration form not found - may be auto-join flow",
      );
    }
  });
  ...
  test("Session Participant Management API", async ({ request }) => {
    test.info().annotations.push({
      type: "api-integration",
      description:
        "Tests participant management through simplified Participants table",
    });
    ...
    try {
      ...
      const response = await request.post(
        `${BASE_URL_HTTP}/api/session/${sessionId}/participants`,
        {
  ...
  test("Session Data Storage - Annotations and Questions", async ({
    request,
  }) => {
    test.info().annotations.push({
      type: "data-storage",
      description:
        "Tests SessionData table usage for annotations and questions (JSON storage)",
    });
    ...
    try {
      ...
      const response = await request.post(
        `${BASE_URL_HTTP}/api/session/${sessionId}/annotations`,
        {
          data: annotationData,
        },
      );
    ...
    try {
      ...
      const response = await request.post(
        `${BASE_URL_HTTP}/api/session/${sessionId}/questions`,
        {
          data: questionData,
        },
      );
  ...
  test("Token Expiration and Validation", async ({ request }) => {
    ...
    const hostValidation = await request.get(
      `${BASE_URL_HTTP}/api/host/token/${hostToken}/validate`,
    );
    ...
    if (hostData.Session && hostData.Session.ExpiresAt) {
      ...
      console.log(
        `✅ Token Expiration Valid - Expires: ${expiresAt.toISOString()}`,
      );
    }
  });
  ...
  test("Session Status and Lifecycle Management", async ({ request }) => {
    ...
    const statusResponse = await request.get(
      `${BASE_URL_HTTP}/api/session/${sessionId}/status`,
    );
    ...
    if (statusResponse.status() === 200) {
      ...
      if (statusData.status) {
        expect(["Active", "Waiting", "Started", "Created"]).toContain(
          statusData.status,
        );
      }
...
test.describe("Performance and Reliability - Session Operations", () => {
  test("Session Creation Performance", async ({ request }) => {
    test.info().annotations.push({
      type: "performance",
      description:
        "Measures session creation performance in simplified architecture",
    });
  ...
  test("Concurrent User Join Simulation", async ({ browser }) => {
    ...
    for (const userPage of userPages) {
      ...
      await expect(
        userPage.locator("text=Authentication service unavailable"),
      ).not.toBeVisible();
    }
