...
- **Provided:** Selective update - port only the specified prompt and its dependencies; write the updated live prompt back to `.github/prompts/{prompt}` (no `.template` extension) and its portable template to `.github/_Portable/prompts/{prompt}.template`.
...
- ✅ Live meta-prompts in `.github/prompts/` updated and ensured to have NO `.template` suffix
...
- ✅ Live prompt updated in `.github/prompts/{prompt}` without `.template`
...
7. **NO TEMPLATE SUFFIX IN LIVE PROMPTS**: Only templates in `.github/_Portable/**` end with `.template`. Prompts under `.github/prompts/` must never contain a `.template` suffix after any operation.
...
✅ Updated: .github/prompts/$promptFile
```
...
# Pseudocode:
# for each $m in ('port-instructions.prompt.md','total-recall.prompt.md'):
#   live = .github/prompts/$m
#   portable = .github/_Portable/prompts/$m.template
#   1) Read live; transform to templated content (replace project specifics with {{VARIABLES}}; add header)
#   2) Write templated to $portable
#   3) Ensure $live remains present without any .template suffix
...
- For meta-prompts (this file and `total-recall.prompt.md`), also generate the live prompt under `.github/prompts/` without a `.template` suffix during setup.
...
Additional shared project learning files:
- `Dependencies.md` (tooling map) → Use as authoritative source for dev tooling versions when available. In portable output, include a templated variant `Dependencies.md.template` with variables for versions, and generate a live `Dependencies.md` during setup based on selected stack.
...
- Include a machine-readable manifest alongside templates: `.github/_Portable/learning/tooling.manifest.json` describing expected dev tools (ESLint/Prettier/Playwright/Stylelint/TypeScript) and their config file paths. The setup script uses this to install and wire packages and configs.
...
2. **Template Variable Population:**
  ...
  - Expand all `{{VARIABLES}}` and output real files without `.template` suffixes into `.github/prompts/`, `.github/instructions/Links/`, and other destinations.
  - For meta-prompts specifically, ensure `.github/prompts/port-instructions.prompt.md` and `.github/prompts/total-recall.prompt.md` exist as live prompts (no `.template`).

3. **File Generation:**
  - Copy all `.template` files
  - Replace `{{VARIABLES}}` with actual values
  - Remove `.template` extension on generated outputs
  - Place in correct `.github/` locations
  - Maintain `_Portable` as the template store; do not overwrite live prompts with `.template` files
...
5. **Tool Installation:**
  - If project type includes Node/JS tooling, read `.github/_Portable/learning/tooling.manifest.json` and:
    1) Ensure `package.json` exists with devDependencies for:
      - Playwright and runner
      - Percy CLI and Playwright integration
      - ESLint core + TS ESLint plugin + parser + Playwright plugin
      - Prettier core
      - Stylelint core + standard config + postcss-html
    ...
    3) Add npm scripts:
      - `test`, `test:headed`, `test:debug`, `test:report`, `test:install`, `test:ui-mode`, `test:trace`, `test:percy`, `test:percy:headed`, `lint`, `format`, `format:check`, `lint:css`, `lint:css:fix`, `build:tests`.
    4) Optionally install Playwright browsers (`npx playwright install`).
  - If project type is .NET, add analyzer packages and MSBuild properties if not present (e.g., NetAnalyzers, Roslynator, StyleCop) honoring project format.

6. **Summary Generation:**
  - Create `PROJECT-SETUP-SUMMARY.md` in project root
  - List all installed components
  - Show next steps
  - Include a Tooling Sync section showing which dependencies/configs were created or verified
...
**Verify Tooling & Dependencies:**
1. ✅ `package.json` contains expected devDependencies per Dependencies manifest
2. ✅ Config files exist under `config/testing/`
3. ✅ NPM scripts exist (test/lint/format/build:tests)
4. ✅ Playwright browsers installed (optional)
5. ✅ VS Code tasks present for analyzer workflow (if applicable)
...
# Linters
npx eslint -v; npx stylelint --version; npx prettier -v
# Playwright
...
- ✅ Cross-references valid
 - ✅ Tooling synced with Dependencies manifest
...
5. Run quick checks: `npm run build:tests`; `npm run test:install` (optional)
