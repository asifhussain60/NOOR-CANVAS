...
- ✅ Automatic test creation (E2E tests generated for UI changes when configured)
...
**ALWAYS** follow `.github/instructions/SelfAwareness.instructions.md` for all operating guardrails, file organization, runtime rules, and {{ANALYZER_TOOLS}} integration.

### 🗄️ Database Access Rules (MANDATORY)
**PRIMARY DATABASE:** KSESSIONS_DEV

- When user mentions "database", assume **KSESSIONS_DEV**
- .NET Framework (Web.config): use `ConfigurationManager.ConnectionStrings["DefaultDb"].ConnectionString`
- **SCHEMA RULES:**
   - ✅ `dbo.*` - READ/WRITE allowed via repositories (Dapper)
   - ❌ All other schemas - READ-ONLY (unless explicitly allowed in InfrastructureQuickRef)
- **VIOLATION = IMMEDIATE ROLLBACK**

**See:** `.github/instructions/Links/InfrastructureQuickRef.md` for complete database rules
...
- Test Framework QuickRef (optional; use only if an E2E framework is configured)
...
- **2.9:** QuickRef Localization (cache InfrastructureQuickRef, {{TEST_FRAMEWORK}}QuickRef - first use only)
...
- Backend unit tests: NUnit (project `Source Code/Sessions.Tests`) — run when backend code changes.
- UI/E2E tests: Optional. Only delegate to `test-generation.prompt.md` if an E2E framework (e.g., Playwright/Karma) is confirmed configured for this repo.

**UI/E2E Delegation Protocol (conditional):**
1. Detect UI-impacting changes (components, pages, CSS, user interactions).
2. If E2E is configured, invoke `test-generation.prompt.md` with parameters:
   - `key`, `feature`, `scenario`, `endpoints`
   - `tokens` (optional), `multiUser` (if host/participant flows), `testType`
3. Receive generated test files and orchestration script; document in key-data-stream.

**See:** `test-generation.prompt.md` and shared E2E references (if present) for:
...
**Key Requirements (when E2E enabled):**
- **Test Location**: `.github/prompts.keys/{key}/e2e/` (within key data stream)
- **Test Registry**: `.github/prompts.keys/{key}/e2e/test-registry.md` (prevents duplication)
- **Orchestration Scripts**: `.github/prompts.keys/{key}/scripts/`
- **Naming**: `{feature}-{test-type}.spec.ts`
- **Test Data**: Session 212 tokens optional
- **Execution**: Via orchestration scripts ONLY (never run the test runner directly)
...
- E2E framework is not configured
...
2. **Run linters by file type**:
   - **C# Files** (*.cs, *.cshtml, *.razor): MSBuild/Roslyn Analyzers
   - **JavaScript/TypeScript** (*.js, *.ts, *.tsx): ESLint (optional)
   - **CSS** (*.css): Stylelint (optional)
   - **PowerShell Scripts** (*.ps1): PSScriptAnalyzer
   - **JSON Files** (*.json): JSON syntax validation (+ Prettier optional)
...
3. **Document verification results**:
   ...
   - [PASS] Constraint 1: Save button preserved
    ...
    - [PASS] Constraint 2: Colors matched exactly
       ...
       - CSS values: (example) hex "#FF5733" and "#3357FF" confirmed
   ```
...
1. **Check for active tests** in `.github/prompts.keys/{key}/{{TEST_PATH}}/`:
   ```powershell
   $testFiles = Get-ChildItem ".github/prompts.keys/{key}/{{TEST_PATH}}/*.spec.ts"
   ```

2. **For each passing test**:
   - Copy test file to production: `{{TEST_PATH}}/UI/{feature}-{test-type}.spec.ts`
...
3. **Update test registry** (`.github/prompts.keys/{key}/{{TEST_PATH}}/test-registry.md`):
   ```markdown
...
4. **Delete tests from key directory**:
   ```powershell
   Remove-Item ".github/prompts.keys/{key}/{{TEST_PATH}}/*.spec.ts"
...
- Promoted {X} test(s) to {{TEST_PATH}}/UI/
...
**Skip Conditions:**
- No tests exist in `.github/prompts.keys/{key}/{{TEST_PATH}}/`
...
- **ALWAYS include persistence tests in {{TEST_FRAMEWORK}} specs** (page refresh after mutation is mandatory)
...
- {{TEST_FRAMEWORK}} tests require persistence validation with page refresh
