...
- All other instruction files (WITHOUT .template extension)
 - `.github/prompts/` live prompts (including `port-instructions.prompt.md` and `total-recall.prompt.md`) exist WITHOUT `.template` suffix
...
- Check `.github/learning/Dependencies.md` exists (or project-level equivalent) to serve as the authoritative tooling manifest for dev/test/lint infrastructure.
- If missing: ⚠️ WARN and plan to generate a templated `Dependencies.md` during porting.

**Verify Node/Tooling Configs (if JS/TS detected):**
```powershell
Test-Path "package.json"            # devDependencies should include Playwright, ESLint, Prettier, Stylelint, TS
...
1) Read `.github/learning/Dependencies.md` and extract expected packages and configs:
     - Playwright: `playwright`, `@playwright/test`
     - Percy: `@percy/cli`, `@percy/playwright`
     - ESLint stack: `eslint`, `@typescript-eslint/eslint-plugin`, `@typescript-eslint/parser`, `eslint-plugin-playwright`, `eslint-config-prettier`
     - Prettier: `prettier`
     - Stylelint: `stylelint`, `stylelint-config-standard`, `postcss-html`
     - TypeScript: `typescript`, `ts-node`, `@types/node`
     - .NET analyzers: `Microsoft.CodeAnalysis.NetAnalyzers`, `Roslynator.Analyzers`, `StyleCop.Analyzers`
...
if (Test-Path "package.json") {
    ...
    foreach ($name in @('playwright','@playwright/test','@percy/cli','@percy/playwright','eslint','@typescript-eslint/eslint-plugin','@typescript-eslint/parser','eslint-plugin-playwright','eslint-config-prettier','prettier','stylelint','stylelint-config-standard','postcss-html','typescript','ts-node','@types/node')) {
        if (-not $dev.$name) { Write-Host "⚠️ Missing devDependency: $name" -ForegroundColor Yellow }
    }
}

# Config files
foreach ($f in @('config/testing/playwright.config.cjs','config/testing/eslint.config.js','config/testing/.prettierrc','config/testing/stylelint.config.cjs','config/testing/tsconfig.json')) {
    if (-not (Test-Path $f)) { Write-Host "⚠️ Missing config: $f" -ForegroundColor Yellow }
}

# VS Code tasks for analyzers (optional)
if (Test-Path ".vscode/tasks.json") { Write-Host "ℹ️ tasks.json present" } else { Write-Host "⚠️ VS Code tasks not found (.vscode/tasks.json)" -ForegroundColor Yellow }
```

3) Output a Tooling Audit Summary (include versions if available) and mark remediation items for the port-instructions/setup scripts.
...
## Final Checks: Live vs Template and Tooling

Perform these final validations before finishing:
- Live prompts check: ensure `.github/prompts/` contains only live `.md` files (no `.template` suffixes)
- Template store check: ensure all `.template` files are under `.github/_Portable/**`
- Tooling check: if Node/JS stack present, verify `package.json` devDependencies and `config/testing/*` configs align with `Dependencies.md`
- Report any violations with exact file paths and recommend re-running setup or port-instructions remediation.
