...
window.NoorAnnotations = (function () {
  ...
  function initialize(containerId) {
    try {
      ...
      setupEventListeners();
      console.log(
        "NOOR-ANNOTATION: Annotation system initialized for container:",
        containerId,
      );
      return true;
    } catch (error) {
      console.error(
        "NOOR-ANNOTATION: Error initializing annotation system:",
        error,
      );
      return false;
    }
  ...
  function setupEventListeners() {
    ...
    svgElement.addEventListener("touchstart", handlePointerStart, {
      passive: false,
    });
    svgElement.addEventListener("touchmove", handlePointerMove, {
      passive: false,
    });
    svgElement.addEventListener("touchend", handlePointerEnd, {
      passive: false,
    });
  ...
  function handlePointerStart(event) {
    ...
    console.log(
      "NOOR-ANNOTATION: Pointer start at",
      point,
      "with tool",
      currentTool,
    );
  ...
  function handleTextSelection(event) {
    ...
    try {
      ...
      console.log(
        "NOOR-ANNOTATION: Text selection highlight created:",
        annotation,
      );
    } catch (error) {
      console.error(
        "NOOR-ANNOTATION: Error creating highlight from selection:",
        error,
      );
    }
  }
    ...
    try {
      ...
      if (window.DotNet && window.DotNet.invokeMethodAsync) {
        window.DotNet.invokeMethodAsync(
          ...
          .then((result) => {
            console.log(
              "NOOR-ANNOTATION: Annotation created successfully:",
              result,
            );
          })
          .catch((error) => {
        ...
        console.warn(
          "NOOR-ANNOTATION: DotNet interop not available, annotation not persisted",
        );
      }
