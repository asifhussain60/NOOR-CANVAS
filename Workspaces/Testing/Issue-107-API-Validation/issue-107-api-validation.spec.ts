...
test.describe('Issue-107: API Validation Error - Session/Category/Album Null Parameters', () => {
    ...
    test('should reproduce the API validation error with exact payload structure', async ({ request }: { request: APIRequestContext }) => {
        ...
        const testAlbum = albums.find(a => a.GroupId.toString() === TEST_ALBUM_ID);
        ...
        const categoriesResponse = await request.get(`/api/host/categories/${TEST_ALBUM_ID}?guid=${TEST_HOST_TOKEN}`);
        expect(categoriesResponse.ok()).toBeTruthy();
        const categories: CategoryData[] = await categoriesResponse.json();
        console.log(`Categories loaded: ${categories.length} found for album ${TEST_ALBUM_ID}`);

        const testCategory = categories.find(c => c.CategoryID.toString() === TEST_CATEGORY_ID);
        expect(testCategory).toBeDefined();
        console.log(`Test category found: ${testCategory?.CategoryName} (ID: ${testCategory?.CategoryID})`);

        // Get sessions for test category
        const sessionsResponse = await request.get(`/api/host/sessions/${TEST_CATEGORY_ID}?guid=${TEST_HOST_TOKEN}`);
        expect(sessionsResponse.ok()).toBeTruthy();
        const sessions: SessionData[] = await sessionsResponse.json();
        console.log(`Sessions loaded: ${sessions.length} found for category ${TEST_CATEGORY_ID}`);

        const testSession = sessions.find(s => s.SessionID.toString() === TEST_SESSION_ID);
        ...
        const createResponse = await request.post(`/api/host/create-session?token=${TEST_HOST_TOKEN}`, {
            data: sessionRequest
        });
        ...
        if (createResponse.status() === 400) {
            ...
            expect(responseData.error).toBe('Selected session, category, and album are required');
    ...
    test('should validate property name case sensitivity in API payload', async ({ request }: { request: APIRequestContext }) => {
        ...
        for (const variation of variations) {
            ...
            if (response.status() === 400) {
                ...
                console.log(`${variation.name} - Received: ${JSON.stringify(errorData.received)}`);
            }
    ...
    test('should validate API parsing with debugging headers', async ({ request }: { request: APIRequestContext }) => {
        ...
        for (const [key, value] of Object.entries(responseHeaders)) {
            if (key.toLowerCase().startsWith('x-debug') || key.toLowerCase().startsWith('x-noor')) {
...
test.describe('Issue-107: Database Validation for Session/Category/Album Data', () => {
    test('should verify test data integrity in KSESSIONS_DEV database', async ({ request }: { request: APIRequestContext }) => {
        ...
        const categoriesResponse = await request.get(`/api/host/categories/18?guid=JHINFLXN`);
        expect(categoriesResponse.ok()).toBeTruthy();
        const categories: CategoryData[] = await categoriesResponse.json();

        const category55 = categories.find(c => c.CategoryID === 55);
        expect(category55).toBeDefined();
        console.log(`✅ Category 55 verified: ${category55?.CategoryName} (Album: ${category55?.GroupId})`);

        // Test session 1281 exists under category 55
        const sessionsResponse = await request.get(`/api/host/sessions/55?guid=JHINFLXN`);
        expect(sessionsResponse.ok()).toBeTruthy();
        const sessions: SessionData[] = await sessionsResponse.json();

        const session1281 = sessions.find(s => s.SessionID === 1281);
        expect(session1281).toBeDefined();
        console.log(`✅ Session 1281 verified: ${session1281?.SessionName} (Category: ${session1281?.CategoryId})`);

        // Verify the complete cascade chain
        expect(category55?.GroupId).toBe(18);
        expect(session1281?.CategoryId).toBe(55);
        console.log('✅ Complete cascade chain verified: Album 18 → Category 55 → Session 1281');
    });
});