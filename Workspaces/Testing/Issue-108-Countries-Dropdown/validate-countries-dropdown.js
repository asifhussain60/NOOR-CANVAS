...
async function validateCountriesDropdown() {
    ...
    try {
        ...
        if (await joinButton.isVisible()) {
            ...
            console.log('🔍 Join Session button not found, looking for Join as User option...');
            // Try alternative selectors
            const userButton = page.locator(':has-text("Join")').or(page.locator(':has-text("User")')).first();
        ...
        const countriesSelect = page.locator('select').filter({ hasText: 'Select your country' })
            .or(page.locator('select').filter({ hasText: 'country' }))
            .or(page.locator('select[name*="Country"]'))
            .or(page.locator('select').filter({ has: page.locator('option:has-text("United States")') }))
        ...
        if (await countriesSelect.isVisible()) {
            ...
            for (let i = 0; i < Math.min(5, optionCount); i++) {
                ...
                if (text && text !== 'Select your country' && text !== 'Loading countries...') {
            ...
            if (optionCount >= 239) {
                console.log(`✅ SUCCESS: Countries dropdown has ${optionCount} options (expected ≥239)`);
                return true;
            } else {
                console.log(`❌ FAIL: Countries dropdown has only ${optionCount} options (expected ≥239)`);
                return false;
            }
        } else {
...
async function validateAPI() {
    ...
    try {
        const response = await context.get('https://localhost:9091/api/host/countries?guid=demo-token-12345');
        ...
        if (response.ok()) {
            ...
            if (countries.length >= 239) {
                console.log(`✅ SUCCESS: API returns ${countries.length} countries (expected 239)`);
                return true;
            } else {
                console.log(`❌ FAIL: API returns only ${countries.length} countries (expected 239)`);
                return false;
            }
        } else {
