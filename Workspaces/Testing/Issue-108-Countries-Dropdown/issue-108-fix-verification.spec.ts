...
test.describe("Issue-108 Fix Verification: Token Validation", () => {
  test("should validate token TXZ25W6K successfully after HttpClient fix", async ({
    page,
  }) => {
    ...
    const errorMessage = await page
      .locator("text=Unable to validate token")
      .count();
    ...
    const clientError = await page
      .locator("text=Unable to validate token")
      .count();
    expect(clientError).toBe(0);

    console.log(
      "✅ Issue-108 Fix Verified: Token validation working correctly",
    );
  });

  test("should handle token validation API call correctly", async ({
    page,
  }) => {
