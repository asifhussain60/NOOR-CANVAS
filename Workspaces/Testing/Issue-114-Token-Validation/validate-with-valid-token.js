...
async function validateIssue114WithValidToken() {
    console.log('🧪 Issue-114 Validation: Testing countries dropdown with valid user token');
    ...
    try {
        ...
        for (const token of validTokens) {
            ...
            if (selectElements > 0) {
                const options = await page.locator('select option:not(:has-text("Select")):not(:has-text("Loading"))');
                const initialOptionCount = await options.count();
                console.log(`Initial country options: ${initialOptionCount} (should be 0 or very few)`);

                if (initialOptionCount < 10) {
                    console.log('✅ PASS: Countries not loaded during token entry phase');
                } else {
                    console.log(`❌ FAIL: ${initialOptionCount} countries loaded too early`);
            ...
            const registrationForm = page.locator('input[placeholder*="name"], input[placeholder*="email"]');
            const errorMessage = page.locator('.error, .alert-danger, text="Invalid token"');

            const hasRegistrationForm = await registrationForm.first().isVisible({ timeout: 5000 }).catch(() => false);
            ...
            if (hasRegistrationForm) {
                ...
                try {
                    ...
                    console.log('⚠️  Timeout waiting for countries to load, checking current state...');
                    const options = page.locator('select option:not(:has-text("Select")):not(:has-text("Loading"))');
                    optionCount = await options.count();
                }

                if (optionCount > 50) {
                    console.log(`✅ SUCCESS: Countries dropdown loaded ${optionCount} countries AFTER token validation!`);
                    ...
                    if (selectedValue) {
                        console.log(`✅ SUCCESS: Can select countries (selected: ${selectedValue})`);
                        console.log(`\n🎉 Issue-114 Fix VALIDATED with token: ${token}`);
                        await browser.close();
                        return true;
                    }
                } else {
                    console.log(`❌ FAIL: Only ${optionCount} countries loaded after token validation`);

                    // Debug: Check if there are loading indicators
                    const loadingText = await page.locator('text="Loading", option:has-text("Loading")').count();
                    if (loadingText > 0) {
                ...
                console.log(`⚠️  Unclear response for token ${token} - no registration form or clear error`);
                continue;
            }
        }
...
// Run the validation
validateIssue114WithValidToken().then(success => {
    if (success) {
