...
test.describe('Broadcast Template UI Verification - signalcomm', () => {
    ...
    test('Verify new template buttons and inline broadcast button are present', async ({ page }) => {
        ...
        console.log('[WORKITEM:signalcomm:test] ✅ All new UI elements are present and visible ;CLEANUP_OK');
    });
    ...
    test('Verify old buttons have been removed', async ({ page }) => {
        ...
        const oldBroadcastButtons = broadcastPanel.locator('button').filter({ hasText: 'Broadcast HTML' });
        const deleteButtons = broadcastPanel.locator('button').filter({ hasText: 'Delete' });
        const testShareButtons = broadcastPanel.locator('button').filter({ hasText: 'Test Share Asset' });

        await expect(oldBroadcastButtons).toHaveCount(0);
        await expect(deleteButtons).toHaveCount(0);
        await expect(testShareButtons).toHaveCount(0);

        console.log('[WORKITEM:signalcomm:test] ✅ All old UI elements have been successfully removed ;CLEANUP_OK');
    });
