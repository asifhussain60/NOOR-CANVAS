...
test.describe('Share Button Injection Validation', () => {
    ...
    test('should detect assets and inject share buttons with proper data attributes', async ({ page }) => {
        const testId = `sharebutton-injection-${Date.now()}`;
        console.log(`[${testId}] Testing share button injection with canonical Session 212 data`);
        ...
        // Validate that share buttons were injected
        const shareButtons = await page.locator('.ks-share-button').all();
        console.log(`[${testId}] Found ${shareButtons.length} share buttons injected`);

        // Should have at least some share buttons from Session 212 transcript
        expect(shareButtons.length).toBeGreaterThan(0);

        // Test each share button for proper data attributes
        for (let i = 0; i < shareButtons.length; i++) {
            const button = shareButtons[i];

            // Validate required data attributes exist
            const shareId = await button.getAttribute('data-share-id');
            const assetType = await button.getAttribute('data-asset-type');
            const instanceNumber = await button.getAttribute('data-instance-number');

            console.log(`[${testId}] Button ${i + 1}: shareId=${shareId}, assetType=${assetType}, instance=${instanceNumber}`);

            // Assert data attributes are properly set
            expect(shareId).toBeTruthy();
            expect(assetType).toBeTruthy();
            expect(instanceNumber).toBeTruthy();

            // Validate semantic ID format (should be meaningful, not generic)
            expect(shareId).toMatch(/^share-asset-\w+-\d+$/);

            // Validate asset type matches known AssetLookup types
            const validAssetTypes = ['ayah-card', 'inserted-hadees', 'etymology-card',
                'inserted-dua', 'islamic-table', 'inlineArabic', 'img', 'audio'];
    ...
    test('should pass lint and build validation', async ({ page }) => {
        ...
        // Verify page is still functional (can navigate and interact)
        await expect(page.locator('text=NOOR Canvas')).toBeVisible();
